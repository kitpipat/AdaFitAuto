...
function FCNnDocTmpChkPdtSpcCtlInDB($paParams,$paWhere)
{
    if(!isset($paParams['tUserSessionID']) || empty($paParams['tUserSessionID'])){
        return;
    }
    ...
    // ============= Parameter =============    
    $tUserSessionID = $paParams['tUserSessionID'];
    $tDctTable      = $paParams['tDctTable'];
    $tTableRefPK    = $paWhere['tTableRefPK'];
    $tSQLWhere      = "";

    $tUsrCode   = $_SESSION['tSesUsername'];
    $tUsrLevel  = $_SESSION['tSesRealUsrLevel'];
    switch($tUsrLevel){
        case 'HQ':
            $tSQLWhere .= " AND (PSC.FTPscAlwCmp = '1' OR PSC.FTPdtCode IS NULL OR (PSC.FTPscAlwOwner = '1' AND PDT.FTCreateBy = '".$tUsrCode."')) ";
            break;
        case 'AD':
            $tSQLWhere .= " AND (PSC.FTPscAlwAD = '1' OR PSC.FTPdtCode IS NULL OR (PSC.FTPscAlwOwner = '1' AND PDT.FTCreateBy = '".$tUsrCode."')) ";
            break;
        case 'BCH':
            $tSQLWhere .= " AND (PSC.FTPscAlwBch = '1' OR PSC.FTPdtCode IS NULL OR (PSC.FTPscAlwOwner = '1' AND PDT.FTCreateBy = '".$tUsrCode."')) ";
            break;
        case 'MER':
            $tSQLWhere .= " AND (PSC.FTPscAlwMer = '1' OR PSC.FTPdtCode IS NULL OR (PSC.FTPscAlwOwner = '1' AND PDT.FTCreateBy = '".$tUsrCode."')) ";
            break;
        case 'SHP':
            $tSQLWhere .= " AND (PSC.FTPscAlwShp = '1' OR PSC.FTPdtCode IS NULL OR (PSC.FTPscAlwOwner = '1' AND PDT.FTCreateBy = '".$tUsrCode."')) ";
            break;
    }

    $tSQL2 = "
    UPDATE TCNTDocDTTmp SET FTPunCode = CRD.FTPunCode , FTTmpRemark = '' , FTTmpStatus = '1'
        FROM (
        SELECT
            Pac.FTPunCode,
            TMP.FTPdtCode
        FROM
            TCNTDocDTTmp TMP
            LEFT JOIN TCNMPdtPackSize Pac ON TMP.FTPdtCode = Pac.FTPdtCode AND Pac.FCPdtUnitFact = 1
        WHERE
            TMP.FTSessionID = '$tUserSessionID' 
            AND TMP.FTPunCode = 'N/A'
            ) CRD
            WHERE FTSessionID = '$tUserSessionID' AND TCNTDocDTTmp.FTPdtCode = CRD.FTPdtCode
    ";
    $ci->db->query($tSQL2);

    $tSQL = "   UPDATE TCNTDocDTTmp SET TCNTDocDTTmp.FTTmpStatus = '5', TCNTDocDTTmp.FTTmpRemark = 'ไม่มีสิทธิใช้งานสินค้านี้'
                WHERE FTPdtCode NOT IN (
                    SELECT DISTINCT PSC.FTPdtCode FROM TCNTDocDTTmp DT WITH(NOLOCK) 
                    INNER JOIN TCNMPdt PDT WITH(NOLOCK) ON PDT.FTPdtCode = DT.FTPdtCode
                    INNER JOIN TCNSDocCtl_L DCT WITH(NOLOCK) ON DCT.FTDctTable = '".$tDctTable."' AND DCT.FTDctStaUse = '1' AND DCT.FNLngID = 1
                    INNER JOIN TCNMPdtSpcCtl PSC WITH(NOLOCK) ON PSC.FTDctCode = DCT.FTDctCode AND PSC.FTPdtCode = DT.FTPdtCode
                    WHERE DT.FTSessionID = '".$tUserSessionID."' 
                    AND DT.FTXthDocKey = '".$tTableRefPK."' 
                    AND DT.FTTmpStatus = '1'
                    ".$tSQLWhere."
                )
                AND TCNTDocDTTmp.FTSessionID = '".$tUserSessionID."' 
                AND TCNTDocDTTmp.FTXthDocKey = '".$tTableRefPK."' 
                AND TCNTDocDTTmp.FTTmpStatus = '1' ";
