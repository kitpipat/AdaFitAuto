<?php
    ...
    //คำนวณ prorate
    function FCNaHCalculateProrate($ptPagename,$ptDocumentNo,$paParams = array()){
        $ci = &get_instance();
        $ci->load->database();
        set_time_limit(0);
        ...
        $tSQLHDDis = "SELECT HDD.*,CASE WHEN HDD.FCXtdAmt < 0 THEN 1 ELSE 3 END AS FTXtdDisChgType FROM(
                        ...
                        FROM  $tTableHDDisTmp WITH(NOLOCK)
        ...
        if($oQueryHDDis->num_rows() > 0){
            ...
            $ci->db->delete($tTableDTDisTmp);

            //Step 03 : ไปเอาข้อมูลแต่ละรายการในตาราง Temp
            $tSQLDT = "SELECT FTPdtCode,FTXtdPdtName,FCXtdNet,FTXtdStaAlwDis,FNXtdSeqNo FROM $tTableDTTmp WITH(NOLOCK) WHERE FTXtdStaAlwDis = 1 AND FTSessionID = '$tSession_id' AND FTXthDocKey = '$ptPagename' AND FTXthDocNo ='$ptDocumentNo'  ";
            ...
            if($oQueryDT->num_rows() > 0){
                ...
                //Insert Prorate
                $tSql = "INSERT INTO $tTableDTDisTmp (
                            FTBchCode,
                            FTXthDocNo,
                            FNXtdSeqNo,
                            FTSessionID,
                        ...
                        FROM (
                            SELECT 
                                ...
                                DT.FTXtdPdtName,
                                DT.FCXtdNet,
                                ( select SUM(FCXtdNet) FROM $tTableDTTmp WITH(NOLOCK) WHERE FTSessionID = '$tSession_id' AND FTXthDocKey = '$ptPagename' AND FTXthDocNo ='$ptDocumentNo' AND FTXtdStaAlwDis = '1' GROUP BY FTXthDocNo ) AS SUMNET,
                                DT.FTXtdStaAlwDis,
                                DT.FNXtdSeqNo ,
                                DT.FTBchCode,
                                DT.FTXthDocNo
                            FROM $tTableDTTmp DT WITH(NOLOCK)
                ...
                $tSqlLastSum = "UPDATE Tmp SET Tmp.FCXtdValue = LastProrate.FCXthLastProrate
                                FROM $tTableDTDisTmp Tmp INNER JOIN ( SELECT DT.FTXthDocNo 
                                                                            , DT.FNXtdSeqNO 
                                                                            , TmpLast.FCXtdValue + ($nDiscount - (
                                                                                                                  SELECT SUM(FCXtdValue) FROM $tTableDTDisTmp  
                                                                                                                  WHERE FTSessionID = '$tSession_id'
                                                                                                                  AND FTXthDocNo ='$ptDocumentNo' 
                                                                                                                  AND FNXtdStaDis = '2' )
                                                                                                                ) AS FCXthLastProrate  
                                                                        FROM $tTableDTDisTmp DT 
