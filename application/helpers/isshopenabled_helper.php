...
function FCNxLoadConfigIsCategoryEnabled(){
    ...
    $ci->load->database();
    $tRoleCode  = $ci->session->userdata('tSesUsrRoleCodeMulti');
    $tSQL       = " SELECT USRMENU.FTRolCode , USRMENU.FTMnuCode , USRMENU.FTAutStaRead , MENULIST.FTMnuStaUse
                    FROM TCNTUsrMenu USRMENU WITH(NOLOCK)
                    LEFT JOIN TSysMenuList MENULIST WITH(NOLOCK) ON USRMENU.FTMnuCode = MENULIST.FTMnuCode
                    WHERE USRMENU.FTRolCode IN ($tRoleCode) 
                      AND USRMENU.FTMnuCode IN ('SPD011','SPD012','SPD013','SPD014','SPD015')
