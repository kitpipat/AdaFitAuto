        ...
        case "TCNMPdtBar":
            $tSQL = "UPDATE T SET 
                    ...
                    SELECT 
                        ...
                        MAS.FTBarCode
                    FROM TCNTImpMasTmp PDT
                    INNER JOIN TCNMPdtBar MAS ON PDT.FTPdtCode = MAS.FTPdtCode AND PDT.FTBarCode = MAS.FTBarCode AND PDT.FTPunCode <> MAS.FTPunCode
                    WHERE 
                    PDT.FTTmpTableKey IN ('TCNMPdt','TCNMPdtSpcBch') 
                    AND PDT.FTTmpStatus IN ('1','6')
                    AND PDT.FTSessionID = '$tUserSessionID'
                    ) C ON C.FTPdtCode = T.FTPdtCode AND C.FTSessionID = T.FTSessionID AND C.FTBarCode = T.FTBarCode AND C.FTPunCode = T.FTPunCode ";
            // echo $tSQL;
        break;
        case "TCNMPdt":
            $tSQL = "UPDATE T SET 
                    ...
                    INNER JOIN (
                        ...
                        AND PDT.FTSessionID = '$tUserSessionID'
                        AND ISNULL(PDT.FTAgnCode,'') <> ISNULL(SPC.FTAgnCode,'')
