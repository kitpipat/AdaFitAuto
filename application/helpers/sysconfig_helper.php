...
// ฟังก์ชั่น Get App Version ล่าสุด 30/08/2021
function FCNtGetAppVersion() {
    ...
    $tPath = getcwd().'/version_deploy.txt';
    $headers = @get_headers($tPath);
    if($headers[0] != 'HTTP/1.0 404 Not Found') {
        $tFilePath = fopen($tPath,'r');
        while ($tReadFile = fgets($tFilePath)) {
            $tVersion = str_replace("Version","",$tReadFile);
        }
        fclose($tFilePath);
    }else{
        $tVersion = "00.00.00.00";
    }
