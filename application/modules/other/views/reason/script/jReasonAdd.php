<script type="text/javascript">
    ...
    $('#oimBrowseMesGroup').click(function(e) {
        e.preventDefault();
        var nStaSession = JCNxFuncChkSessionExpired();
        if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
            JSxCheckPinMenuClose();
            window.oPdtBrowseMessageGroupOption = oBrowseMessageGroup({
                'tReturnInputCode': 'oetRcnGroupCode',
                'tReturnInputName': 'oetRcnGroupName',
            });
            JCNxBrowseData('oPdtBrowseMessageGroupOption');
        } else {
            JCNxShowMsgSessionExpired();
        }
    });
    ...
    //Option MessageGroup
    var oBrowseMessageGroup = function(poReturnInput) {
        var tInputReturnCode = poReturnInput.tReturnInputCode;
        var tInputReturnName = poReturnInput.tReturnInputName;


        var oOptionReturn = {
            Title: ['other/reason/reason', 'tRSGGroup'],
            Table: {
                Master: 'TSysRsnGrp_L',
                PK: 'FTRsgCode'
            },
            Where: {
                Condition: ['AND TSysRsnGrp_L.FNLngID = 1']
            },
            GrideView: {
                ColumnPathLang: 'other/reason/reason',
                ColumnKeyLang: ['tRSGGroupCode', 'tRSGGroupName'],
                ColumnsSize: ['15%', '85%'],
                WidthModal: 50,
                DataColumns: ['TSysRsnGrp_L.FTRsgCode', 'TSysRsnGrp_L.FTRsgName'],
                DataColumnsFormat: ['', ''],
                Perpage: 10,
                OrderBy: ['TSysRsnGrp_L.FTRsgCode DESC'],
            },
            CallBack: {
                ReturnType: 'S',
                Value: [tInputReturnCode, "TSysRsnGrp_L.FTRsgCode"],
                Text: [tInputReturnName, "TSysRsnGrp_L.FTRsgName"],
            },
            RouteAddNew: 'agency',
            BrowseLev: 1,
        }
