...
class mBankdeptype extends CI_Model {
    ...
    public function FSaMBDTListBDT($paData){
        ...
        $tSQL   = "SELECT c.* FROM(
                        SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , FTBdtCode ASC) AS FNRowID,* FROM
                            (SELECT DISTINCT
                                ...
                                BDT.FDCreateOn,
                                BDT.FTAgnCode       AS rtAgnCode,
                                BDH.FTBdtCode   AS rtBdtCodeLef
                            FROM [TFNMBnkDepType] BDT
                            LEFT JOIN [TFNMBnkDepType_L] BDTL ON BDT.FTBdtCode = BDTL.FTBdtCode AND BDTL.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN (SELECT DISTINCT FTBdtCode FROM TFNTBnkDplHD WITH(NOLOCK) ) BDH ON BDH.FTBdtCode  = BDT.FTBdtCode
                            WHERE 1=1 ";
        ...
        if(@$tSearchList != ''){
            $tSQL .= " AND (BDT.FTBdtCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= " OR BDTL.FTBdtName LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
