...
class mCustomerDebitCard extends CI_Model
{
    ...
    public function FSaMCstDebitCardDataList($paData){
        try {
            ...
            $tSQL           = " 
                SELECT c.* FROM(SELECT  ROW_NUMBER() OVER(ORDER BY FTCrdRefCode ASC) AS rtRowID,*
                FROM(
                    SELECT 	
                        CRDMAN.FTCrdCode,
                        CRDMAN.FTCrdRefCode,
                    ...
                    WHERE CRDMAN.FTCrdRefCode = ".$this->db->escape($tCstCode)."
            ";
    ...
    public function FSoMCstDebitCardGetPageAll($ptSearchList, $paData){
        try {
            ...
            if (isset($ptSearchList) && !empty($ptSearchList)) {
                $tSQL   .= " AND (URLObj.FNUrlID LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
            }
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    public function FSaMCstCheckCrdCode($paData)
    {
        ...
        $tSQL       = "
            SELECT 	
                CRDMAN.FTCrdCode
            FROM [TFNMCardMan] CRDMAN WITH(NOLOCK)
            WHERE CRDMAN.FTCrdRefCode = ".$this->db->escape($tRefCode)."
            AND CRDMAN.FTCrdCode = ".$this->db->escape($tCrdCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $oDetail    = $oQuery->result();
    ...
    function FSnMCstDataSumTotalCard($paData){
        $tCrdCode   = $paData['FTCrdCode'];
        $tSQL       = "
            SELECT ((Crad.CashIn + Promotion ) - Payment ) AS TotalAmount,((Crad.CashIn + Promotion ) - DepositCrd - Payment  ) AS TotalUse , DepositCrd AS TotalDepositCrd 
            FROM (
                SELECT 
                ...
                WHERE FTCrdCode  = ".$this->db->escape($tCrdCode)."
            ) AS Crad
        ";
        $oQuery = $this->db->query($tSQL);
