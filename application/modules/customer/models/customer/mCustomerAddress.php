...
class mCustomerAddress extends CI_Model{
    ...
    public function FSaMCSTAddressDataList($paDataWhere){
        $tSQL   = " 
            ...
            WHERE CSTA.FTAddGrpType   <> 4
            AND CSTA.FNLngID    = ".$this->db->escape($paDataWhere['FNLngID'])."
            AND CSTA.FTCstCode  = ".$this->db->escape($paDataWhere['FTCstCode'])."
    ...
    public function FSaMCSTAddressGetDataID($paDataWhereAddress){
        $tSQL   = " 
            SELECT
                ...
                CSTA.FTAddLatitude
            FROM TCNMCstAddress_L       CSTA WITH(NOLOCK)
            LEFT JOIN TCNMArea_L        AREL WITH(NOLOCK)   ON CSTA.FTAreCode       = AREL.FTAreCode    AND AREL.FNLngID    = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            LEFT JOIN TCNMZone_L        ZNEL WITH(NOLOCK)   ON CSTA.FTZneCode       = ZNEL.FTZneChain   AND ZNEL.FNLngID    = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            LEFT JOIN TCNMSubDistrict_L SDSTL WITH(NOLOCK)  ON CSTA.FTAddV1SubDist  = SDSTL.FTSudCode   AND SDSTL.FNLngID   = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            LEFT JOIN TCNMDistrict_L    DSTL WITH(NOLOCK)   ON CSTA.FTAddV1DstCode  = DSTL.FTDstCode    AND DSTL.FNLngID    = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            LEFT JOIN TCNMProvince_L    PVNL WITH(NOLOCK)   ON CSTA.FTAddV1PvnCode  = PVNL.FTPvnCode    AND PVNL.FNLngID    = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            WHERE CSTA.FDCreateOn <> ''
            AND CSTA.FTCstCode      = ".$this->db->escape($paDataWhereAddress['FTCstCode'])."
            AND CSTA.FNLngID        = ".$this->db->escape($paDataWhereAddress['FNLngID'])."
            AND CSTA.FTAddGrpType   = ".$this->db->escape($paDataWhereAddress['FTAddGrpType'])."
            AND CSTA.FNAddSeqNo     = ".$this->db->escape($paDataWhereAddress['FNAddSeqNo'])."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0){
    ...
    public function FSxMCSTAddressUpdateSeq($paCSTDataAddress){
        $tSQL   = " 
            ...
            INNER JOIN (
                ...
                WHERE FDCreateOn <> ''
                AND FTCstCode       = ".$this->db->escape($paCSTDataAddress['FTCstCode'])."
                AND FNLngID         = ".$this->db->escape($paCSTDataAddress['FNLngID'])."
                AND FTAddGrpType    = ".$this->db->escape($paCSTDataAddress['FTAddGrpType'])."
            ) AS DATARUNSEQ
            ON ADDRUPD.FTCstCode        = DATARUNSEQ.FTCstCode
