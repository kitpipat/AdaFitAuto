...
class mCustomerLevel extends CI_Model {
    ...
    public function FSaMCstLevSearchByID($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL       = "SELECT
                            ...
                            CstLev.FTClvStaAlwPnt   AS rtCClvAlwPnt,
                            CstLev.FCClvCalAmt   AS rtCClvCalAmt,
                            CstLev.FCClvCalPnt   AS rtCClvCalPnt,
    ...
    public function FSaMCstLevList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL       = "SELECT c.* FROM(
                       ...
                       (SELECT DISTINCT
                                ...
                                CstLev.FDCreateOn
                        FROM [TCNMCstLev] CstLev
                        LEFT JOIN [TCNMCstLev_L] CstLevL ON CstLevL.FTClvCode = CstLev.FTClvCode AND CstLevL.FNLngID = ".$this->db->escape($nLngID)."
                        LEFT JOIN TCNMPdtPriList_L  ON  TCNMPdtPriList_L.FTPplCode = CstLev.FTPplCode AND TCNMPdtPriList_L.FNLngID  = ".$this->db->escape($nLngID)."
                        LEFT JOIN ( SELECT COUNT ( CST.FTCstCode ) AS nCount, CST.FTClvCode FROM TCNMCst CST WITH ( nolock ) GROUP BY CST.FTClvCode ) t ON CstLev.FTClvCode = t.FTClvCode
                        WHERE 1=1";
        ...
        if ($tSearchList != ''){
            ...
            $tSQL .= " AND CstLev.FTClvCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR CstLevL.FTClvName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
    ...
    public function FSaMCstLevAddUpdateMaster($paData){
        try{
            ...
            $this->db->set('FTClvStaAlwPnt' , $paData['FTClvStaAlwPnt']);
            $this->db->set('FCClvCalAmt' , $paData['FCClvCalAmt']);
            $this->db->set('FCClvCalPnt' , $paData['FCClvCalPnt']);
            ...
            if($this->db->affected_rows() > 0){
                ...
                $this->db->insert('TCNMCstLev',array(
                    ...
                    'FTClvStaAlwPnt'     => $paData['FTClvStaAlwPnt'],
                    'FCClvCalAmt'     => $paData['FCClvCalAmt'],
                    'FCClvCalPnt'     => $paData['FCClvCalPnt'],
