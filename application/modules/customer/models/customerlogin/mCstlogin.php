...
class mCstlogin extends CI_Model {
    ...
    public function FSaMCSTLOGDataList($paData){
        try{
            ...
            $tSQL           = " 
                SELECT c.* FROM(SELECT  ROW_NUMBER() OVER(ORDER BY FDCstPwdStart DESC , FTCstCode DESC) AS rtRowID,*
                FROM(
                    SELECT 	
                        CSTLOGIN.FTCstCode,
                    ...
                    LEFT JOIN TCNMCst_L CSTL ON CSTLOGIN.FTCstCode = CSTL.FTCstCode AND CSTL.FNLngID = ".$this->db->escape($tFNLngID)."
                    WHERE CSTLOGIN.FTCstCode = ".$this->db->escape($tCstCode)."
            ";
    ...
    public function FSoMCSTLOGINGetPageAll($ptSearchList,$paData){
        try{
            ...
            if(isset($ptSearchList) && !empty($ptSearchList)){
                $tSQL .= " AND (CSTLOGIN.FTCstCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
            }
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    public function FSaMCSTLCheckID($paData){
        ...
        $tSQL       = "
            ...
            WHERE CSTLOGIN.FTCstLogin   = ".$this->db->escape($tCstlogin)."
            AND CSTLOGIN.FTCstCode      = ".$this->db->escape($tCstCode)."
            AND CSTLOGIN.FDCstPwdStart  = ".$this->db->escape($tPwdStart)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0){
    ...
    public function FSoMCSTLCheckDuplicate($ptCstLogin, $paCstLogType, $paCstCode, $ptPwdStart){
        ...
        $tSQL   = "
            SELECT COUNT(FTCstLogin)AS counts
            FROM TCNMCstLogin
            WHERE  1=1 
            $tSQLWhere
        ";
