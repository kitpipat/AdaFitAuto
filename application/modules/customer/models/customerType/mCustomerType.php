    ...
    public function FSaMCstTypeList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL       = "SELECT c.* FROM(
                       ...
                       (SELECT DISTINCT
                                ...
                                CstType.FTAgnCode   AS rtCstAgnCode,
                                CstType.FTCtyCode   AS rtCstTypeCode,
                                CstTypeL.FTCtyName  AS rtCstTypeName,
                                CstType.FDCreateOn
                        FROM [TCNMCstType] CstType
                        LEFT JOIN [TCNMCstType_L] CstTypeL ON CstTypeL.FTCtyCode = CstType.FTCtyCode AND CstTypeL.FNLngID = ".$this->db->escape($nLngID)."
                        --LEFT JOIN [TCNMCst] CstM ON CstM.FTCtyCode = CstType.FTCtyCode
                        LEFT JOIN (SELECT DISTINCT FTCtyCode FROM TCNMCst WITH(NOLOCK)) CstM ON CstM.FTCtyCode = CstType.FTCtyCode
                        WHERE 1=1";

        $tSearchList = $paData['tSearchAll'];
        if ($tSearchList != ''){
            $tSQL .= " AND CstType.FTCtyCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR CstTypeL.FTCtyName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
