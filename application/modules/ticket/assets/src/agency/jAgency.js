...
function JSnAddEditAgency(ptRoute) {
    ...
    //ถ้าเป็นตัวแทนขายประเภท แฟรนไซส์จำเป็นต้องกรอกลูกค้าอ้างอิง
    if($("#ocmCmpAgnFra option:selected").val() == 2){
        if($('#oetAgnFrmCstCode').val() == '' || $('#oetAgnFrmCstCode').val() == null){
            $('#oetAgnFrmCstName').focus();
            return;
        }
    }
