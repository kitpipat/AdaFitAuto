...
class cAgency extends MX_Controller {
    ...
    public function FSxCANGDataTable(){
        ...
        $aResListRelation   = $this->mAgency->FSaMAGNListRelation();
        $aGenTable  = array(
            'aDataRelation' => $aResListRelation,
            'aAlwEvent' => $aAlwEvent,
            'aDataList' => $aResList,
            'nPage'     => $nPage,
            'tSearchAll'    => $tSearchAll
    ...
    public function FSaCAGNAddEvent(){
        ...
        try{
            ...
            $tAgnFrmCstCodeOld  = $this->input->post('ohdAgnFrmCstCodeOld');
            ...
            $aDataMaster        = [
                ...
                'FTAgnType'         => $this->input->post('ocmCmpAgnFra'),
                'FTAgnRefCst'       => $this->input->post('oetAgnFrmCstCode')
            ];
            ...
            $aDataMasterCstFC = array(
                'FTCstCodeFCOld'    => $tAgnFrmCstCodeOld,
                'FTCstCode'         => $aDataMaster['FTAgnRefCst'] ,
            );
            ...
            if($nStaDup == 0){
                $this->db->trans_begin();
                $this->mAgency->FSaMAGNAddUpdateMaster($aDataMaster);
                $this->mAgency->FSaMAGNAddUpdateLang($aDataMaster);
                $this->mAgency->FSaMAGNAddUpdateSpc($aDataMasterSpc);

                if ($aDataMaster['FTAgnType'] == 2) {
                    $this->mAgency->FSaMAGNAddUpdateCstFC($aDataMasterCstFC);
                }

                $aBranchData =  $this->FSaCAGNCreateBranchDefault($aDataMaster);
                $this->FSaCAGNCreatePosDefault($aDataMaster,$aBranchData);
