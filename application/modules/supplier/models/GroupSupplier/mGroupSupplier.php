    ...
    public function FSaMSGPList($paData){
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtSgpCode DESC) AS rtRowID,* FROM
                                    (SELECT DISTINCT
                                        ...
                                        SGP.FDCreateOn
                                    FROM [TCNMSplGrp] SGP
                                    LEFT JOIN [TCNMSplGrp_L]  SGP_L ON SGP.FTSgpCode = SGP_L.FTSgpCode AND SGP_L.FNLngID = ".$this->db->escape($nLngID)."
                                    --LEFT JOIN [TCNMSpl]  SPL ON SPL.FTSgpCode = SGP.FTSgpCode
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                ...
                $tSQL .= " AND SGP.FTSgpCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR SGP_L.FTSgpName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            }
