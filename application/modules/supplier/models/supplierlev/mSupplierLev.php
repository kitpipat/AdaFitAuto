    ...
    public function FSaMSLVList($paData){
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtSlvCode DESC) AS rtRowID,* FROM
                                    (SELECT DISTINCT
                                        ...
                                        SLV.FDCreateOn
                                    FROM [TCNMSplLev] SLV
                                    LEFT JOIN [TCNMSplLev_L]  SLV_L ON SLV.FTSlvCode = SLV_L.FTSlvCode AND SLV_L.FNLngID = ".$this->db->escape($nLngID)."
                                    --LEFT JOIN [TCNMSpl]  SPL ON SPL.FTSlvCode = SLV.FTSlvCode
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                ...
                $tSQL .= " AND SLV.FTSlvCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR SLV_L.FTSlvName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            }
