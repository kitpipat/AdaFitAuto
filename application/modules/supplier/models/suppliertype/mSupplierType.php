        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtStyCode DESC) AS rtRowID,* FROM
                                    ...
                                    LEFT JOIN [TCNMSplType_L]  STY_L ON STY.FTStyCode = STY_L.FTStyCode AND STY_L.FNLngID = ".$this->db->escape($nLngID)."
                                    --LEFT JOIN [TCNMSpl]  SPL ON SPL.FTStyCode = STY.FTStyCode
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                ...
                $tSQL .= " AND STY.FTStyCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR STY_L.FTStyName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            }
    ...
    public function FSaMSTYGetDataByID($paData){
        try{
            ...
            $tSQL       = " SELECT
                                ...
                                STY.FDCreateOn  AS rDStyCreateOn
                            FROM TCNMSplType STY
