<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
    <div class="table-responsive">
        <table id="otbMNTDataList" class="table table-striped">
            <thead>
                <tr>
                    <th class="text-center xCNTextBold" ><?= language('checkdocument/checkdocument','tChkDocTbNo')?></th>
                    <th class="text-center xCNTextBold" ><?= language('checkdocument/checkdocument','วันที่ส่งข้อความ')?></th>
                    <th class="text-center xCNTextBold" ><?= language('checkdocument/checkdocument','tChkDocTbDocType')?></th>
                    <th class="text-center xCNTextBold" ><?= language('checkdocument/checkdocument','ประเภทเอกสาร')?></th>
            ...
            <tbody>
                <?php if(!empty($aDataNotiDoc)){ ?>
                    <?php foreach($aDataNotiDoc as $nKey => $aData){ ?>
                        <?php 
                        ...
                        <tr style="<?=$tStaRead?>">
                            ...
                            <?php if($aData['FNNotType'] == 1){
                                    $tTextNotType   = "ค้างอนุมัติ";
                                    $tCSSNotType    = "font-weight:bold;";
                                }else{
                                    $tTextNotType   = "อนุมัติแล้ว";
                                    $tCSSNotType    = "";
                                }
                            ?>
                            <td class='text-left xCNTextDetail2' style="<?=$tCSSNotType?>"><?=$tTextNotType?></td>
                ...
                <?php }else{ ?>
                    <tr>
                        <td class='text-center xCNTextDetail2' colspan='10'><?= language('checkdocument/checkdocument','tChkDocTbNoData')?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
