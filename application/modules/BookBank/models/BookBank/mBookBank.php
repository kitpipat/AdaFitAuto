...
class mBookBank extends CI_Model {
    ...
    public function FSaMBBKList($paData){
        ...
        $tSQL       = "     SELECT DISTINCT
                                ...
                                BBK.FDCreateOn,
                                BBK.FTBbkCode       AS rtAgnCode,
                                BDH.FTBbkCode      AS rtBbkCodeLef
                            FROM [TFNMBookBank] BBK WITH(NOLOCK)
                            LEFT JOIN [TFNMBookBank_L] BBKL WITH(NOLOCK)
                                ON BBK.FTBbkCode = BBKL.FTBbkCode AND BBK.FTBchCode = BBKL.FTBchCode AND BBK.FTBchCode = BBKL.FTBchCode AND BBKL.FNLngID =  ".$this->db->escape($nLngID)."
                            LEFT JOIN [TFNMBank_L] BNKL WITH(NOLOCK) ON BBK.FTBnkCode = BNKL.FTBnkCode AND BNKL.FNLngID =  ".$this->db->escape($nLngID)."
                            LEFT JOIN [TCNMBranch_L] BCHL WITH(NOLOCK) ON BBK.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID =  ".$this->db->escape($nLngID)."
                            LEFT JOIN (SELECT DISTINCT FTBbkCode FROM TFNTBnkDplHD WITH(NOLOCK) ) BDH ON BDH.FTBbkCode  = BBK.FTBbkCode
                            WHERE 1=1 ";

        @$tSearchList = $paData['tSearchAll'];
        if(@$tSearchList != ''){
            $tSQL .= " AND (BBK.FTBbkCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= "      OR BBK.FTBbkAccNo LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= "      OR BNKL.FTBnkName LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= "      OR BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= "      OR BBKL.FTBbkName LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
