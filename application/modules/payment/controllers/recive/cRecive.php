...
class cRecive extends MX_Controller
{
    ...
    public function FSaRCVAddEvent()
    {
        ...
        if ($tRcvFormatCode == 4 || $tRcvFormatCode == 20 ) {
            $tRcvCouponCode = $this->input->post('oetRcvCouponCode');
        }else{
            $tRcvCouponCode = '';
        }
        ...
        $aDataMaster    = [
            ...
            'FNRcvQtyCopy'          => (!empty($this->input->post('oetRcvQtyCopy'))) ? $this->input->post('oetRcvQtyCopy') : 0,
            'FCRcvPayMin'           => (!empty($this->input->post('oetRcvPayMin'))) ? str_replace(',', '', $this->input->post('oetRcvPayMin')) : 0,
            'FCRcvPayMax'           => (!empty($this->input->post('oetRcvPayMax'))) ? str_replace(',', '', $this->input->post('oetRcvPayMax')) : 0,
            'FTRcvStaReqSign'       => (!empty($this->input->post('ocbRcvStaReqSign'))) ? 1 : 2,
        ];

        $oCountDup = $this->mRecive->FSoMRCVCheckDuplicate($aDataMaster['FTRcvName'],$aDataMaster['FTAgnCode']);
