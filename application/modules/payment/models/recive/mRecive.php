...
class mRecive extends CI_Model
{
    ...
    public function FSaMRCVList($ptAPIReq, $ptMethodReq, $paData)
    {
        ...
        $tSQL   = "SELECT c.* FROM(
                        SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtRcvCode ASC) AS rtRowID,* FROM
                            (SELECT DISTINCT
                                RCV.FTRcvCode   AS rtRcvCode,
                                VDRC.FTRcvCode   AS rtRcvCodeLef,
                                RCV.FTAgnCode       AS rtAgnCode,
                                RCV.FTRcvStaUse AS rtRcvStatus,
                                RCVL.FTRcvName  AS rtRcvName,
                            ...
                            LEFT JOIN [TSysRcvFmt_L]    RCVF WITH(NOLOCK) ON RCV.FTFmtCode = RCVF.FTFmtCode AND RCVF.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN [TCNMImgObj]      IMGO WITH(NOLOCK) ON RCV.FTRcvCode = IMGO.FTImgRefID AND IMGO.FTImgTable = 'TFNMRcv' AND IMGO.FNImgSeq = 1
                            LEFT JOIN (
        ...
        if ($tSearchList != '') {
            $tSQL .= " AND (RCV.FTRcvCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= " OR RCVL.FTRcvName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= " OR RCVF.FTFmtName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
    ...
    public function FSaMRCVAddUpdateMaster($paData)
    {
        ...
        try {
            ...
            $this->db->set('FTRcvRefExt', $paData['FTRcvRefExt']);
            ...
            $this->db->set('FNRcvQtyCopy', $paData['FNRcvQtyCopy']);
            $this->db->set('FCRcvPayMin', $paData['FCRcvPayMin']);
            $this->db->set('FCRcvPayMax', $paData['FCRcvPayMax']);
            $this->db->set('FTRcvStaReqSign', $paData['FTRcvStaReqSign']);
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->db->insert('TFNMRcv', array(
                    ...
                    'FTRcvRefExt'               => $paData['FTRcvRefExt'],
                    ...
                    'FNRcvQtyCopy'              => $paData['FNRcvQtyCopy'],
                    'FCRcvPayMin'               => $paData['FCRcvPayMin'],
                    'FCRcvPayMax'               => $paData['FCRcvPayMax'],   
                    'FTRcvStaReqSign'           => $paData['FTRcvStaReqSign']
                ));
    ...
    //Return Type : object
    public function FSoMRCVCheckDuplicate($ptFTRcvName,$ptFTAgnCode)
    {

        $tSQL   = "SELECT TFNMRcv.FTRcvCode AS counts
                   FROM TFNMRcv
                   INNER JOIN TFNMRcv_L ON  TFNMRcv.FTRcvCode = TFNMRcv_L.FTRcvCode
                   WHERE TFNMRcv_L.FTRcvName LIKE '$ptFTRcvName' AND TFNMRcv.FTAgnCode='$ptFTAgnCode'";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            return $oQuery->result();
        } else {
