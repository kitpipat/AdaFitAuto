...
class mRate extends CI_Model {
    ...
    public function FSaMRTESearchByID($paData){
        ...
        $tSQL       = " 
            ...
            LEFT JOIN [TFNMRate_L] RTEL WITH(NOLOCK) ON RTE.FTRteCode = RTEL.FTRteCode AND RTEL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN [TCNMImgObj] IMGO WITH(NOLOCK) ON RTE.FTRteCode = IMGO.FTImgRefID AND IMGO.FTImgTable = 'TFNMRate' AND IMGO.FNImgSeq = 1
            WHERE RTE.FDCreateOn <> ''
        ";
        if($tRteCode != ""){
            $tSQL .= "AND RTE.FTRteCode = ".$this->db->escape($tRteCode)."";
        }
    ...
    public function FSaMRTEList($paData){
        ...
        $tSQL   = " 
            ...
            ";
        if($tSearchList != ''){
            $tSQL   .= " AND (RTE.FTRteCode COLLATE  THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR RTEL.FTRteName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
    ...
    public function FSaMRTEAddUpdateMaster($paData){
        if($paData['FTRteStaLocal']== 1){
            ...
            $tSQL   = "
                SELECT FTRteCode
                FROM TFNMRate 
                WHERE FTRteStaLocal = ".$this->db->escape($tRteStalocal)."
            ";
            $oQuery = $this->db->query($tSQL);
            if($oQuery->num_rows() > 0){
                $aData = $oQuery->result();
    ...
    public function FSnMRTEGetPageAll($ptSearchList,$ptLngID){
        $tSQL   = "
            SELECT COUNT (RTE.FTRteCode) AS counts 
            FROM TFNMRate RTE
            LEFT JOIN [TFNMRate_L] RTEL ON RTE.FTRteCode = RTEL.FTRteCode AND RTEL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE RTE.FDCreateOn <> ''
        ";
        if($ptSearchList != ''){
            $tSQL   .= " AND (   RTE.FTRteCode     COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= "      OR RTEL.FTRteName    COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
        }
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSnMRTECheckStaLocal($paData){
        ...
        $tRteStalocal   = $paData['FTRteStaLocal'];
        $tSQL           = "
            SELECT FTRteStaLocal AS counts
            FROM TFNMRate 
            WHERE FTRteStaLocal = ".$this->db->escape($tRteStalocal)."
        ";
        $oQuery = $this->db->query($tSQL);
    ...
    public function FSnMRTECheckDuplicate($ptRteCode){
        $tSQL = "
            SELECT COUNT(FTRteCode)AS counts
            FROM TFNMRate
            WHERE FTRteCode = ".$this->db->escape($ptRteCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
