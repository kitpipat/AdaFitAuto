        ...
        try{
            ...
            $tSQL           =   "   SELECT c.* FROM(
                                        SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtBntCode ASC) AS rtRowID,* FROM (
                                            ...
                                            LEFT JOIN [TFNMBankNote_L]  BNT_L   WITH(NOLOCK) ON BNT.FTBntCode   = BNT_L.FTBntCode AND BNT_L.FNLngID =  ".$this->db->escape($nLngID)."
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL   .= " AND (BNT.FTBntCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL   .= " OR BNT_L.FTBntName  LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
