...
class mReciveSpc extends CI_Model
{
        ...
        try {
            ...
            $tSQL           = " SELECT c.* FROM(SELECT  ROW_NUMBER() OVER(ORDER BY FTRcvCode ASC) AS rtRowID,*
                                FROM(
                                    ...
                                    LEFT JOIN TCNSListObj_L OBJL WITH(NOLOCK) ON RCVSPC.FTObjCode = OBJL.FTObjCode AND OBJL.FNLngID = $nFNLngID
                                    WHERE 1=1
    ...
    public function FSaMRCVSPCCheckID($paData)
    {
        ...
        $tSQL   = " SELECT
                    ...
                    LEFT JOIN TCNSListObj_L OBJL WITH(NOLOCK) ON RCVSPC.FTObjCode = OBJL.FTObjCode AND OBJL.FNLngID = $nLngID
                    WHERE 1=1
                    AND ISNULL(RCVSPC.FTRcvCode,'') = '$tRcvCode'
                    AND ISNULL(RCVSPC.FTAppCode,'') = '$tAppCode'
                    AND ISNULL(RCVSPC.FTObjCode,'') = '$tObjCode'
    ...
    public function FSaMRCVSPCUpdateMaster($paData)
    {
        ...
        $FTObjCodeold =  $paData['FTObjCodeold'];
        ...
        $tSQL .= "  WHERE FTRcvCode = '$FTRcvCode'
                    ...
                    AND ISNULL(FTObjCode,'') = '$FTObjCodeold'
    ...
    public function FSnMRCVSpcDel($paDataWhere)
    {
        ...
        $this->db->where_in('FTObjCode', $paDataWhere['FTObjCode']);
    ...
    public function FSaMRCVSPCDeleteMultiple($paDataDelete, $ptRevCodeWhere)
    {
        ...
        $this->db->where_in('FTObjCode', $paDataDelete['FTObjCode']);
    ...
    public  function FSaMRCVSPCChkDupAddMaster($paData)
    {
        ...
        $this->db->where('FTObjCode', $paData['FTObjCode']);
