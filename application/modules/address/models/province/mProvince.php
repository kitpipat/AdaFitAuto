...
class mProvince extends CI_Model
{
    ...
    public function FSaMPVNSearchByID($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL       = "
            ...
            LEFT JOIN [TCNMProvince_L] PVNL ON PVN.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN [TCNMZone_L] ZNEL ON ZNEL.FTZneCode = PVN.FTZneCode AND ZNEL.FNLngID = ".$this->db->escape($nLngID)."
            WHERE PVN.FDCreateOn <> ''
        ";
        if ($tPvnCode != "") {
            $tSQL   .= "AND PVN.FTPvnCode = ".$this->db->escape($tPvnCode)."";
        }
        ...
        $tSQL       = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY rtPvnCode ASC) AS rtRowID,* FROM (
                    ...
                    FROM [TCNMProvince] PVN
                    LEFT JOIN [TCNMProvince_L] PVNL ON PVN.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($nLngID)."
                    LEFT JOIN [TCNMZone_L] ZNEL ON PVN.FTZneCode = ZNEL.FTZneCode AND ZNEL.FNLngID = ".$this->db->escape($nLngID)."
                    WHERE PVN.FDCreateOn <> ''
        ";
        @$tWhereCode = $paData['tWhereCode'];
        if (@$tWhereCode != '') {
            $tSQL   .= " AND (PVN.FTZneCode = ".$this->db->escape($tWhereCode).")";
        }
        $tSearchList = $paData['tSearchAll'];
        if ($tSearchList != '') {
            $tSQL   .= " AND (PVN.FTPvnCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
    ...
    public function FSnMPVNGetPageAll($ptWhereCode, $ptSearchList, $ptLngID){
        $tSQL   = "
            SELECT COUNT (PVN.FTPvnCode) AS counts
            FROM [TCNMProvince] PVN
            LEFT JOIN [TCNMProvince_L] PVNL ON PVN.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID  = ".$this->db->escape($ptLngID)."
            WHERE PVN.FDCreateOn <> ''
        ";
        if (@$ptWhereCode != '') {
            $tSQL   .= " AND (PVN.FTZneCode = ".$this->db->escape($ptWhereCode).")";
        }

        if ($ptSearchList != '') {
            $tSQL   .= " AND (PVN.FTPvnCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
    ...
    public function FSnMPVNCheckDuplicate($ptPvnCode){
        $tSQL = "
            SELECT COUNT(FTPvnCode)AS counts
            FROM TCNMProvince
            WHERE FTPvnCode = ".$this->db->escape($ptPvnCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            return $oQuery->result();
