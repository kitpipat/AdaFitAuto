...
class mArea extends CI_Model {
    ...
    public function FSaMAREList($paData){
        try{
            ...
            $tSQL           = "
                SELECT c.* FROM(
                    SELECT  ROW_NUMBER() OVER(ORDER BY rtAreCode ASC) AS rtRowID,* 
                    FROM (
                        SELECT DISTINCT
                            ARE.FTAreCode   AS rtAreCode,
                            ARE_L.FTAreName AS rtAreName
                        FROM [TCNMArea] Are
                        LEFT JOIN [TCNMArea_L]  ARE_L ON ARE.FTAreCode = ARE_L.FTAreCode AND ARE_L.FNLngID = ".$this->db->escape($nLngID)."
                        WHERE Are.FDCreateOn <> ''
            ";
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL   .= " AND (ARE.FTAreCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL   .= " OR ARE_L.FTAreName LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
    ...
    public function FSoMAREGetPageAll($ptSearchList,$ptLngID){
        try{
            $tSQL = "
                SELECT COUNT (ARE.FTAreCode) AS counts
                FROM [TCNMArea] ARE
                LEFT JOIN [TCNMArea_L]  ARE_L ON Are.FTAreCode = ARE_L.FTAreCode AND ARE_L.FNLngID = ".$this->db->escape($ptLngID)."
                WHERE ARE.FDCreateOn <> ''
            ";
            if(isset($ptSearchList) && !empty($ptSearchList)){
                $tSQL   .= " AND (ARE.FTAreCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
                $tSQL   .= " OR ARE_L.FTAreName LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
            }
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    public function FSaMAREGetDataByID($paData){
        try{
            ...
            $tSQL       = "
                ...
                FROM TCNMArea ARE
                LEFT JOIN TCNMArea_L ARE_L ON ARE.FTAreCode = ARE_L.FTAreCode AND ARE_L.FNLngID = ".$this->db->escape($nLngID)." 
                WHERE ARE.FTAreCode = ".$this->db->escape($tAreCode)."
            ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0){
    ...
    public function FSnMARECheckDuplicate($ptAreCode){
        $tSQL = "
            SELECT COUNT(ARE.FTAreCode) AS counts
            FROM TCNMArea ARE 
            WHERE ARE.FTAreCode = ".$this->db->escape($ptAreCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
