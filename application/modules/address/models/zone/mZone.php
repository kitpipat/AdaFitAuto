...
class mZone extends CI_Model {
	...
	public function FSaMZNEUpdateChangeZneL($tChainNameOld,$tChainNameNew,$nlen,$tZneChain){
		$tSQL2	= "
			UPDATE TCNMZone_L SET
			FTZneChainName	= REPLACE(FTZneChainName, ".$this->db->escape($tChainNameOld).",".$this->db->escape($tChainNameNew).")
		";
	...
	public function FSaMZNECheckLangAdded($nZneCode,$nLangEdit){
		$tSQL	= "
			SELECT COUNT(FTZneCode) AS counts
			FRO[M TCNMZone_L
			WHERE FTZneCode	= ".$this->db->escape($nZneCode)." 
			AND FNLngID		= ".$this->db->escape($nLangEdit)."
		";
	...
	public function FSaMZNEUpdateZneNameAndFTZneChainNameMaster($aDataMaster){
		...
		$tSQL1	= 	"
			UPDATE TCNMZone 
			SET FTZneChain = REPLACE(FTZneChain, '$oetZneChainOld', '$oetZneChainCurrent')
			WHERE FTZneCode IN (
				SELECT ZNE.FTZneCode 
				FROM TCNMZone ZNE 
				LEFT JOIN TCNMZone_L ZNEL ON ZNE.FTZneCode = ZNEL.FTZneCode
		...
		$tSQL4	= "
			UPDATE TCNMZone_L
			SET -- FTZneName = REPLACE(FTZneName, '$oetZneNameOld', '$tZneName') ,
			FTZneName 		=  '$tZneName',
			FTZneRmk  		=  '$FTZneRmk',
		...
		$tSQL5	= "
			UPDATE TCNMZone_L
			SET FTZneChainName = REPLACE(FTZneChainName, '$old', '$new')
			WHERE FTZneCode IN (
				SELECT ZNE.FTZneCode 
				FROM TCNMZone ZNE 
				LEFT JOIN TCNMZone_L ZNEL ON ZNE.FTZneCode = ZNEL.FTZneCode
				WHERE 
				LEFT(ZNE.FTZneChain , $nLenn) = '$oetZneChainCurrent'
				OR ZNEL.FTZneCode = $tZneCode
				AND LEN(ZNE.FTZneChain
			) > $nLenn 
		)";
	...
	public function FSaMZNEGetZneSelectZneChainName($ptZneCodeSelet){
		$tSQL	= "
			SELECT FTZneChainName
			FROM TCNMZone_L
			WHERE FTZneCode = ".$this->db->escape($ptZneCodeSelet)."
		";
		$oQuery = $this->db->query($tSQL);
		if ($oQuery->num_rows() > 0) {
			return $oQuery->result();
	...
	public function FSaMZNEGetZneLevelANDZneChain($ptZneCodeSelet){
		$tSQL	= "
			SELECT FNZneLevel,FTZneParent,FTZneChain
			FROM TCNMZone 
			WHERE FTZneCode	= ".$this->db->escape($ptZneCodeSelet)."
		";
		$oQuery = $this->db->query($tSQL);
	...
	public function FSaMZNESearchByID($paData){
		...
		if(@$tCode){
			$tSQL	= "
				...
				FROM [TCNMZone] ZNE
				LEFT JOIN [TCNMZone_L] ZNEL ON ZNE.FTZneCode = ZNEL.FTZneCode AND ZNEL.FNLngID	= ".$this->db->escape($nLngID)."
				LEFT JOIN [TCNMZone_L] ZNEPARENT ON ZNE.FTZneParent = ZNEPARENT.FTZneCode AND ZNEPARENT.FNLngID = ".$this->db->escape($nLngID)."
				LEFT JOIN [TCNMArea_L] AREL ON ZNE.FTAreCode = AREL.FTAreCode AND AREL.FNLngID 	= ".$this->db->escape($nLngID)."
			";
			if ($tCode != '') {
				$tSQL	.= " WHERE ZNE.FTZneCode = ".$this->db->escape($tCode)."";
			}
			$oQuery	= $this->db->query($tSQL);
			if ($oQuery->num_rows() > 0) {
	...
	public function FSnMZNEList($paData){
		...
		$tSQL		= "
			SELECT c.* FROM(
				SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtZneCode DESC) AS rtRowID,* 
				FROM (
					SELECT DISTINCT
						ZNE.FTZneCode 		AS rtZneCode,
						ZNE.FNZneLevel 		AS rnZneLevel,
					...
					FROM TCNMZone ZNE
					LEFT JOIN TCNMZone_L ZNEL ON ZNE.FTZneCode = ZNEL.FTZneCode AND ZNEL.FNLngID = ".$this->db->escape($nLngID)."
					WHERE ZNE.FDCreateOn <> ''
		";
		@$tWhereCode 	= $paData['tWhereCode'];
		if (@$tWhereCode != '') {
			$tSQL	.= " AND (ZNE.FTAreCode = ".$this->db->escape($tWhereCode).")";
		}
		$tSearchList	= $paData['tSearchAll'];
		if ($tSearchList != '') {
			$tSQL	.= " AND (ZNE.FTZneCode COLLATE THAI_BIN	LIKE  '%".$this->db->escape_like_str($tSearchList)."%'";
			$tSQL	.= " OR ZNEL.FTZneName COLLATE THAI_BIN 	LIKE  '%".$this->db->escape_like_str($tSearchList)."%')";
		}
	...
	function JSnMZNEGetPageAll($ptWhereCode,$ptSearchList,$ptLngID){
		$tSQL	= "
			SELECT COUNT (ZNE.FTZneCode) AS counts
			FROM TCNMZone ZNE
			LEFT JOIN TCNMZone_L ZNEL ON ZNE.FTZneCode = ZNEL.FTZneCode AND ZNEL.FNLngID = ".$this->db->escape($ptLngID)."
			WHERE ZNE.FDCreateOn <> ''
		";
		if(@$ptWhereCode != ''){
			$tSQL	.= " AND (ZNE.FTAreCode	= ".$this->db->escape($ptWhereCode).")";
		}
		if ($ptSearchList != '') {
			$tSQL	.= " AND (ZNE.FTZneCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
			$tSQL 	.= " OR ZNEL.FTZneName 	LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
		}
		$oQuery = $this->db->query($tSQL);
		if ($oQuery->num_rows() > 0) {
			return $oQuery->result();
	...
	public function FSnMZNECheckduplicate($ptCode){
		$tSQL	= "
			SELECT COUNT(FTZneCode)AS counts
			FROM TCNMZone
			WHERE FTZneCode	= ".$this->db->escape($ptCode)."
		";
		$oQuery = $this->db->query($tSQL);
		if ($oQuery->num_rows() > 0) {
			return $oQuery->result();
		...
		$tSQL 		= "
			SELECT c.* FROM(
				SELECT  ROW_NUMBER() OVER(ORDER BY rtZneCode ASC) AS rtRowID,* FROM (
					...
					LEFT JOIN TCNMUser_L USRL ON USRL.FTUsrCode   = CASE WHEN ZNEO.FTZneTable = 'TCNMUser'   THEN ZNEO.FTZneRefCode ELSE NULL END AND USRL.FNLngID = ".$this->db->escape($nLngID)."
					LEFT JOIN TCNMBranch_L BCHL ON BCHL.FTBchCode = CASE WHEN ZNEO.FTZneTable = 'TCNMBranch' THEN ZNEO.FTZneRefCode ELSE NULL END AND BCHL.FNLngID = ".$this->db->escape($nLngID)."
					LEFT JOIN TCNMSpn_L SPNL ON SPNL.FTSpnCode    = CASE WHEN ZNEO.FTZneTable = 'TCNMSpn'    THEN ZNEO.FTZneRefCode ELSE NULL END AND SPNL.FNLngID = ".$this->db->escape($nLngID)."
					LEFT JOIN TCNMShop_L SHPL ON SHPL.FTShpCode   = CASE WHEN ZNEO.FTZneTable = 'TCNMShop'   THEN ZNEO.FTZneRefCode ELSE NULL END AND SHPL.FNLngID = ".$this->db->escape($nLngID)."
					LEFT JOIN TCNMPosLastNo PLN ON PLN.FTPosCode  = CASE WHEN ZNEO.FTZneTable = 'TCNMPos'    THEN ZNEO.FTZneRefCode ELSE NULL END 
					WHERE ZNE.FDCreateOn <> '' AND ZNEO.FTZneChain	= ".$this->db->escape($nZenCode)."
		";
		@$tWhereCode	= $paData['nZenCode'];
		$tSearchList	= $paData['tSearchAll'];
		if ($tSearchList != '') {
			$tSQL	.= " AND (ZNE.FTZneCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
			$tSQL 	.= " OR ZNEL.FTZneName 	LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
		}
		$tSQL	.= ") Base) AS c WHERE c.rtRowID > ".$this->db->escape($aRowLen[0])." AND c.rtRowID <= ".$this->db->escape($aRowLen[1])."";
		$oQuery  = $this->db->query($tSQL);
		if ($oQuery->num_rows() > 0) {
	...
	function JSnMZNEReferGetPageAll($ptWhereCode,$ptSearchList,$ptLngID){
		...
		";
		if(@$ptWhereCode != ''){
			$tSQL	.= " AND (ZNEO.FTZneChain = ".$this->db->escape($ptWhereCode).")";
		}
		if ($ptSearchList != '') {
			$tSQL 	.= " AND (ZNE.FTZneCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
			$tSQL 	.= " OR ZNEL.FTZneName 	LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
    ...
    public function FSnMZENOJBDel($ptZneRefCode) {
        ...
        $tSQL 			= "
			SELECT COUNT(FTZneRefCode) AS count
			FROM TCNMZoneObj
			WHERE FTZneChain	= ".$this->db->escape($tZneChain)."
		";
    ...
    public function FSnMZENOGetAllNumRow($ptZneRefCode){
        ...
        $tSQL 			= "SELECT COUNT(*) AS FNAllNumRow FROM TCNMZoneObj WHERE FTZneChain = ".$this->db->escape($tZneRefCode)."";
        $oQuery 		= $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0){
            $aResult	= $oQuery->row_array()["FNAllNumRow"];
        }else{
    ...
    public function FSnMZENCheckDuplicate($ptCpnCode){
        $tSQL	= "
			SELECT COUNT(FTZneCode)AS counts
			FROM TCNMZone
			WHERE FTZneCode = ".$this->db->escape($ptCpnCode)."
		";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
