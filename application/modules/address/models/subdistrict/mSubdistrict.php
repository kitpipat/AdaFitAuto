...
class mSubdistrict extends CI_Model {
    ...
    public function FSaMSDTSearchByID($paData){
        ...
        $tSQL       = "
            ...
            LEFT JOIN TCNMSubDistrict_L SDTL ON SDT.FTSudCode = SDTL.FTSudCode AND SDTL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMDistrict  DST ON SDT.FTDstCode = DST.FTDstCode
            LEFT JOIN TCNMDistrict_L DSTL ON SDT.FTDstCode = DSTL.FTDstCode AND DSTL.FNLngID    = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMProvince_L PVNL ON DST.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID    = ".$this->db->escape($nLngID)."
            WHERE SDT.FDCreateOn <> ''
        ";
        if($tSudCode!= ""){
            $tSQL   .= "AND SDT.FTSudCode = ".$this->db->escape($tSudCode)."";
        }
    ...
    public function FSaMSDTList($paData){
        ...
        $tSQL       = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY rtSudCode ASC) AS rtRowID,* 
                    FROM (
                        SELECT DISTINCT
                            SDT.FTSudCode       AS rtSudCode,
                            SDT.FTDstCode       AS rtDstCode,
        ...
        if($tSearchList != ''){
            $tSQL   .= " AND (SDT.FTSudCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR SDTL.FTSudName  LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR DSTL.FTDstName  LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR PVNL.FTPvnName  LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
    ...
    public function FSnMSDTGetPageAll($ptSearchList,$ptLngID){
        $tSQL   = "
            SELECT COUNT (SDT.FTSudCode) AS counts
            FROM TCNMSubDistrict SDT
            LEFT JOIN TCNMSubDistrict_L SDTL ON SDT.FTSudCode = SDTL.FTSudCode AND SDTL.FNLngID = ".$this->db->escape($ptLngID)."
            LEFT JOIN TCNMDistrict  DST ON SDT.FTDstCode = DST.FTDstCode
            LEFT JOIN TCNMDistrict_L DSTL ON SDT.FTDstCode = DSTL.FTDstCode AND DSTL.FNLngID = ".$this->db->escape($ptLngID)."
            LEFT JOIN TCNMProvince_L PVNL ON DST.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE SDT.FDCreateOn <> ''
        ";
        if($ptSearchList != ''){
            $tSQL   .= " AND (SDT.FTSudCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= " OR SDTL.FTSudName  LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= " OR DSTL.FTDstName  LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= " OR PVNL.FTPvnName  LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
        }
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSnMSDTCheckDuplicate($ptSubCode){
        $tSQL = "
            SELECT COUNT(FTSudCode)AS counts
            FROM TCNMSubDistrict
            WHERE FTSudCode = ".$this->db->escape($ptSubCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
