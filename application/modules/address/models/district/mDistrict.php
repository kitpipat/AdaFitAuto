...
class mDistrict extends CI_Model {
	...
	public function FSnMDSTGetPostCode($ptDstCode){
		$tSQL   = "
            SELECT DST.FTDstPost  AS rtDstPost
            FROM [TCNMDistrict] DST
            WHERE DST.FDCreateOn <> ''
        ";
		if(@$ptDstCode != ""){
			$tSQL   .= "AND DST.FTDstCode = ".$this->db->escape($ptDstCode)."";
		}
    ...
    public function FSaMDSTSearchByID($ptAPIReq,$ptMethodReq,$paData){
        ...
        $tSQL       = "
            ...
            FROM [TCNMDistrict] DST
            LEFT JOIN [TCNMDistrict_L]  DSTL ON DST.FTDstCode = DSTL.FTDstCode AND DSTL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN [TCNMProvince_L]  PVNL ON DST.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($nLngID)."
            WHERE DST.FDCreateOn <> ''
        ";
        if($tDstCode!= ""){
            $tSQL   .= "AND DST.FTDstCode = ".$this->db->escape($tDstCode)."";
        }
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0){
            $oDetail = $oQuery->result();
    ...
    public function FSaMDSTList($ptAPIReq,$ptMethodReq,$paData){
        ...
        if(@$tWhereCode != ''){
            $tSQL   .= " AND (DST.FTPvnCode = ".$this->db->escape($tWhereCode).")";
        }
        
        $tSearchList = $paData['tSearchAll'];
        if($tSearchList != ''){
            $tSQL   .= " AND (DST.FTDstCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
    ...
    public function FSnMDSTGetPageAll($ptWhereCode,$ptSearchList,$ptLngID){
        $tSQL   = "
            SELECT COUNT (DST.FTDstCode) AS counts
            FROM [TCNMDistrict] DST
            LEFT JOIN [TCNMDistrict_L]  DSTL ON DST.FTDstCode = DSTL.FTDstCode AND DSTL.FNLngID = ".$this->db->escape($ptLngID)."
            LEFT JOIN [TCNMProvince_L]  PVNL ON DST.FTPvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE DST.FDCreateOn <> ''
        ";
        if(@$ptWhereCode != ''){
            $tSQL   .= " AND (DST.FTPvnCode = ".$this->db->escape($ptWhereCode).")";
        }
        
        if($ptSearchList != ''){
            $tSQL   .= " AND (DST.FTDstCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
    ...
    public function FSnMDSTCheckDuplicate($ptDstCode){
        $tSQL   = "
            SELECT COUNT(FTDstCode)AS counts
            FROM TCNMDistrict
            WHERE FTDstCode = ".$this->db->escape($ptDstCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
