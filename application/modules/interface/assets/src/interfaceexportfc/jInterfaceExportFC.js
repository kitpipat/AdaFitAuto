...
var nStaInterfaceImportBrowseType   = $('#oetInterfaceExportFCFCStaBrowse').val();
var tCallInterfaceImportBackOption  = $('#oetInterfaceExportFCFCCallBackOption').val();
...
function JSxIFCCallRabbitMQ(){
    ...
    if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
        ...
        if(tSpcCheck == true){
            $.ajax({
                type    : "POST",
                url     : "interfaceexportFCCheckDocument",
                data    : {
                        ...
                        if($('.progress-bar-chekbox:checked').length == 1){
                            if(tSpcAPICode == '00030' || tSpcAPICode == '00028'){ 
                                if(tSpcAPICode == '00030'){ //ต้นทุน FIT Auto (SAP + BIGDATA) จะต้องวิ่งเข้าไปหาเอกสารตรวจนับก่อน
                                    ...
                                    $('#odvInterfaceExportFCCostIsNull').modal('show');
                                    ...
                                    //เปิด Table
                                    $('#otbInterfaceExportFCDocNotApv tbody').html('');
                                    $('#otbInterfaceExportFCDocNotApv tbody').append(tHTML);
                                    $('#odvHistoryDocumentNotApv').show();
                                }else if(tSpcAPICode == '00028'){ //ส่งออกรายได้ขายแยกตามประเภทการชำระเงิน(SAP)
                                    $('#odvInterfaceExportFCTextFile').modal('show');
                                }
                            }
                        }else{
                            if(tSpcAPICode == '00030' || tSpcAPICode == '00028'){ 
                            ...
                            $('#odvInterfaceExportFCCostIsNullButCanMoveOn').modal('show');
                        }
                        return;
                    }else{
                        JSxIFCCallRabbitMQToExport('confirm');
                    }
                },
...
function JSxIFCSendNotiForDocumentNotApv(){
}

// Call Rabbit MQ  To Export
function JSxIFCCallRabbitMQToExport(ptType){
    if(ptType == 'remove'){
    ...
    $.ajax({
        type    : "POST",
        url     : "interfaceexportFCAction",
        data    : $('#ofmInterfaceExportFC').serialize()+"&ptTypeEvent="+'getpassword',
        cache   : false,
        Timeout : 0,
        success : function(tResult){
            ...
            if(aResult.tHost == '' || aResult.tPort == ''  || aResult.tPassword == '' || aResult.tUser == '' || aResult.tVHost == ""){
                ...
                $.ajax({
                    type    : "POST",
                    url     : "interfaceexportFCAction",
                    data    : $('#ofmInterfaceExportFC').serialize()+"&ptTypeEvent="+'confirm'+'&tPassword='+tPassword,
                    cache   : false,
                    Timeout : 0,
                    success: function(tResult){
                        $('#obtInterfaceExportFCConfirm').attr('disabled', true);
                        $('#odvInterfaceExportFCSuccess').modal('show');
                        JCNxCloseLoading();
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        JCNxResponseError(jqXHR, textStatus, errorThrown);
...
// ส่งออก
function JSxIFCExportDocumentNotApv(){
    window.open('interfaceexportFCExportForDocNotApv','_blank');
}