...
class mVatrate extends CI_Model {
        ...
        $tSQL       = "
            SELECT c.* FROM(
                ...
                FROM (
                    ...
                    WHERE VAT.FDCreateOn <> ''
        ";
        if ($tVatCode != '') {
            $tSQL   .= "AND VAT.FTVatCode   = ".$this->db->escape($tVatCode)."";
        }
        $tSQL   .= ") Base) AS c ";
        $oQuery  = $this->db->query($tSQL);
    ...
    public function FSaMVATList($paData){
        ...
        if ($tSearchList != ''){
            $tSQL   .= " AND (VAT.FTVatCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR VAT.FCVatRate   LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
    ...
    public function FSaMVATGetPageAll($ptSearchList){
        $tSQL   = "
            SELECT  COUNT(DISTINCT(VAT.FTVatCode)) AS counts
            FROM TCNMVatRate VAT
            WHERE VAT.FDCreateOn <> ''
        ";
        if($ptSearchList != ''){
            $tSQL   .= " AND (VAT.FTVatCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= " OR VAT.FCVatRate   LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
        }
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSoMVATChkDup($pdDateStart){
        $tSQL = "
            SELECT COUNT(FTVatCode) AS counts
            FROM TCNMVatRate
            WHERE FDCreateOn <> ''
        ";
        if($pdDateStart != ""){
            $tSQL .= " AND FDVatStart = ".$this->db->escape($pdDateStart)."";
        }
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
