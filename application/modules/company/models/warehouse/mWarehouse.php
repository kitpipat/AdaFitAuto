...
class mWarehouse extends CI_Model
{
	...
	public function FSvMBCHGetAddress($ptData)
	{
		...
		$tSQL	= "
			SELECT  FTAddVersion,
				...
				FTAddV2Desc2
			FROM TCNMAddress_L
			WHERE FTAddRefCode	= ".$this->db->escape($tAddRefCode)."
			AND FTAddGrpType 	= ".$this->db->escape($tAddGrpType)."
			AND FTAddVersion 	= ".$this->db->escape($tAddVersion)."
			AND FNLngID			= ".$this->db->escape($nLngID)."
		";
	...
	public function FSnMWAHList($paData)
	{
		...
		$tSQL			 = "
			SELECT c.*
			FROM(
				SELECT ROW_NUMBER() OVER(ORDER BY rtBchCode DESC , rtCreateOn DESC , rtWahCode Desc) AS rtRowID,*
				FROM (
					SELECT DISTINCT
						STC.FTWahCode 		AS rtWahCodeLef,
						WAH.FTWahCode 		AS rtWahCode,
						WAH.FTBchCode 		AS rtBchCode,
						WAH.FTWahStaType 	AS rtWahStaType,
						WAH.FTWahRefCode 	AS rtWahRefCode,
					...
					LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON WAH.FTBchCode 	= BCHL.FTBchCode AND BCHL.FNLngID 	= ".$this->db->escape($nLngID)."
					LEFT JOIN TCNMWaHouse_L WAHL WITH (NOLOCK) ON WAH.FTWahCode = WAHL.FTWahCode AND WAH.FTBchCode 	= WAHL.FTBchCode AND WAHL.FNLngID = ".$this->db->escape($nLngID)."
					LEFT JOIN (SELECT DISTINCT FTWahCode,FTBchCode FROM TCNTPdtStkCrd WITH(NOLOCK) ) STC ON STC.FTWahCode = WAH.FTWahCode AND STC.FTBchCode = WAH.FTBchCode
					WHERE WAH.FDCreateOn <> ''";
		...
		if($nGetStaWasteWAH == ''){
			$tSQL	.= " AND WAH.FTWahStaType <> '10' ";
		}

		$tSearchList	= $paData['tSearchAll'];
		if ($tSearchList != '') {
			$tSQL	.= " AND (WAH.FTWahCode COLLATE THAI_BIN LIKE  '%".$this->db->escape_like_str($tSearchList)."%' ";
			$tSQL 	.= " OR WAHL.FTWahName  COLLATE THAI_BIN LIKE  '%".$this->db->escape_like_str($tSearchList)."%' ";
	...
	function JSnMWAHGetPageAll($paData, $ptLngID)
	{
		$tSQL = "
			...
			FROM TCNMWaHouse WAH WITH (NOLOCK)
			LEFT JOIN TCNMWaHouse_L WAHL WITH (NOLOCK) ON WAH.FTWahCode = WAHL.FTWahCode AND WAH.FTBchCode 	= WAHL.FTBchCode AND WAHL.FNLngID = ".$this->db->escape($ptLngID)."
			LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON  WAH.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID	= ".$this->db->escape($ptLngID)."
			WHERE WAH.FDCreateOn <> ''
		";
		if ($this->session->userdata("tSesUsrLevel") != "HQ") {
			$tBchCodeMulti = $this->session->userdata("tSesUsrBchCodeMulti");
	...
	{
		$tSQL	= "
			SELECT COUNT(FTWahCode)AS counts
			FROM TCNMWaHouse
			WHERE FTWahCode	= ".$this->db->escape($ptCode)."
			AND FTBchCode 	= ".$this->db->escape($ptBchCode)."
		";
	...
	//get all row data from pdt location
	...
	//หาว่า ระบบนี้สามารถใช้ คลังอะไรได้บ้าง
	public function FSaMWAHGetWahouseInConfig($pnGetStaWasteWAH){
		$nLangID	= $this->session->userdata("tLangEdit");
		if (empty($pnGetStaWasteWAH) && $pnGetStaWasteWAH == '') {
			$WhereWAH  = "AND OBJL.FTObjCode != '10'";
		}else{
			$WhereWAH  = "";
		}
		$tSQL 		= "
			SELECT
				OBJL.FTObjName,
				OBJL.FTObjCode
			FROM TCNSListObj OBJ
			INNER JOIN TCNSListObj_L OBJL ON OBJ.FTObjCode = OBJL.FTObjCode AND OBJL.FNLngID = ".$this->db->escape($nLangID)."
			WHERE OBJ.FTObjTable	= 'TCNMWaHouse'
			AND OBJ.FTObjStaUse 	= '1' $WhereWAH
		";
