...
class mCompany extends CI_Model{
    ...
    public function FSaMCMPList($ptAPIReq,$ptMethodReq,$paData){
        try{
            ...
            $tSQL       = "
                ...
                FROM [TCNMComp] CMP
                LEFT JOIN [TCNMComp_L]      CMPL ON CMP.FTCmpCode = CMPL.FTCmpCode  AND CMPL.FNLngID    = ".$this->db->escape($nLngID)."
                LEFT JOIN [TCNMBranch_L]    BCHL ON CMP.FTBchCode = BCHL.FTBchCode  AND BCHL.FNLngID    = ".$this->db->escape($nLngID)."
                LEFT JOIN [TCNMBranch]      BCH  ON CMP.FTBchCode = BCH.FTBchCode
                LEFT JOIN [TFNMRate_L]      RTEL ON CMP.FTRteCode = RTEL.FTRteCode  AND RTEL.FNLngID    = ".$this->db->escape($nLngID)."
    ...
    public function FSoMCMPGenViewAddress($ptSysCode,$ptSysKey){
        try{
            $tSQL = "
                ...
                FROM [TSysConfig] TCF
                WHERE FTSysCode = ".$this->db->escape($ptSysCode)." 
                AND FTSysKey = ".$this->db->escape($ptSysKey)."
            ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    public function FSoMCMPGetVatRate($ptVatCode){
        try{
            $tSQL   =   " 
                ...
                WHERE TVR.FDCreateOn <> ''
                AND TVR.FTVatCode   = ".$this->db->escape($ptVatCode)."
    ...
    public function FSaMCMPSelectAddressList($paData){
        ...
        $tSQL       =   "   
            ...
            AND ADDL.FTAddGrpType   = ".$this->db->escape($nAddGrp)."
            AND ADDL.FNLngID        = ".$this->db->escape($nLngID)."
            AND ADDL.FTAddRefCode   = ".$this->db->escape($tRefCode)." 
            AND ADDL.FTAddVersion   = ".$this->db->escape($nAddVer)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result_array();
    ...
    public function FSnMCMPCheckduplicate($ptCmpCode){
        try{
            $tSQL = "
                SELECT COUNT(FTCmpCode)AS counts
                FROM TCNMComp
                WHERE FTCmpCode = ".$this->db->escape($ptCmpCode)."
            ";
            $oQuery = $this->db->query($tSQL);
            if($oQuery->num_rows() > 0){
                return $oQuery->result();
    ...
    public function FSaMCMPAddUpdateMaster($paDataMaster){
        ...
        if($this->db->affected_rows() == 0){
            $tSQLAdd    = " 
                INSERT INTO TCNMComp (
                    FTCmpCode,FTCmpTel,FTCmpFax,FTBchcode,FTCmpWhsInOrEx,FTCmpRetInOrEx,
                    FTCmpEmail,FTRteCode,FTVatCode,FDCreateOn,FTCreateBy,FDLastUpdOn,FTLastUpdBy
                )
                VALUES (
    ...
    public function FSaMCMPAddUpdateLang($paDataMaster){
        ...
        if($this->db->affected_rows() == 0){
            $tSQLAdd    = "
                INSERT INTO TCNMComp_L (FTCmpCode,FNLngID,FTCmpName,FTCmpShop,FTCmpDirector)
                VALUES (
                    '".$paDataMaster['FTCmpCode']."',
    ...
    public function FSaMCMPGetName($paData){
        try{
            ...
            $tSQL   .= "
                ...
                LEFT JOIN [TCNMComp_L]     CMPL ON CMP.FTCmpCode    = CMPL.FTCmpCode  AND CMPL.FNLngID      = ".$this->db->escape($nLngID)."
                LEFT JOIN [TCNMImgObj]     CIMG ON CMP.FTCmpCode    = CIMG.FTImgRefID AND CIMG.FTImgTable   = 'TCNMComp'
                LEFT JOIN [TCNMAgency_L]   AGNL ON AGNL.FTAgnCode   = ".$this->db->escape($tAgnCode)." 
                AND AGNL.FNLngID    = ".$this->db->escape($nLngID)."
    ...
    public function FSaMCMPGetAddressMER(){
        ...
        $tSQL       = "
            ...
            LEFT JOIN TCNMMerchant_L MERL WITH (NOLOCK) ON MER.FTMerCode = MERL.FTMerCode AND MERL.FNLngID  = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMProvince_L PROVL WITH (NOLOCK) ON PROVL.FTPvnCode = ADDL.FTAddV1PvnCode AND PROVL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMDistrict_L DISTL WITH (NOLOCK) ON DISTL.FTDstCode = ADDL.FTAddV1DstCode AND DISTL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMSubDistrict_L SUBDISTL WITH (NOLOCK) ON SUBDISTL.FTSudCode = ADDL.FTAddV1SubDist AND SUBDISTL.FNLngID = ".$this->db->escape($nLangID)."
            WHERE ADDL.FTAddGrpType = 7 
            AND ADDL.FTAddRefNo     = 1 
            AND ADDL.FTAddRefCode   = ".$this->db->escape($tSessionMER)."
        ";
        $oQuery         = $this->db->query($tSQL);
    ...
    public function FSaMCMPGetAddressAD(){
        ...
        $tSQL           = " 
            SELECT TOP 1 
                ...
                SUBDISTL.FTSudName
            FROM TCNMAddress_L ADDL
            LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON ADDL.FTAddRefCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($nLangID)." 
            LEFT JOIN TCNMAgency AGN WITH (NOLOCK) ON AGN.FTBchCode = BCHL.FTBchCode
            LEFT JOIN TCNMAgency_L AGNL WITH (NOLOCK) ON AGN.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMProvince_L PROVL WITH (NOLOCK) ON PROVL.FTPvnCode = ADDL.FTAddV1PvnCode AND PROVL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMDistrict_L DISTL WITH (NOLOCK) ON DISTL.FTDstCode = ADDL.FTAddV1DstCode AND DISTL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMSubDistrict_L SUBDISTL WITH (NOLOCK) ON SUBDISTL.FTSudCode = ADDL.FTAddV1SubDist AND SUBDISTL.FNLngID = ".$this->db->escape($nLangID)."
            WHERE ADDL.FTAddGrpType = 1 
            AND ADDL.FTAddRefNo     = 1 
            AND ADDL.FTAddRefCode   = ".$this->db->escape($tBchCode)."
        ";
        $oQuery         = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0) {
            $aItems = $oQuery->row_array();
    ...
    public function FSaMCMPGetAddressBCH(){
        $nLangID        = $this->session->userdata("tLangEdit");
        $tSQLFindCOM    = "
            SELECT TOP 1 COM.FTCmpCode , COML.FTCmpName , COM.FTCmpEmail
            FROM TCNMComp COM  
            LEFT JOIN TCNMComp_L COML ON COM.FTCmpCode = COML.FTCmpCode 
            AND COML.FNLngID    = ".$this->db->escape($nLangID)."
        ";
        ...
        $tSQL           = " 
            ...
            AND ADDL.FTAddRefCode   = ".$this->db->escape($tSessionBCH)."
        ";
        $oQuery         = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0) {
            $aItems = $oQuery->row_array();
    ...
    public function FSaMCMPGetAddressCOM(){
        $nLangID        = $this->session->userdata("tLangEdit");
        $tSQLFindCOM    = "
            SELECT TOP 1 COM.FTCmpCode , COM.FTBchCode , COM.FTCmpEmail , COML.FTCmpName FROM TCNMComp COM
            LEFT JOIN TCNMComp_L COML ON COM.FTCmpCode = COML.FTCmpCode 
            AND COML.FNLngID = ".$this->db->escape($nLangID)."
        ";
        $oQueryCOM  = $this->db->query($tSQLFindCOM);
    ...
    public function FSaMGetDataAddress($paDataAddrWhere){
        $nLangID    = $paDataAddrWhere['FNLngID'];
        $nGrpType   = $paDataAddrWhere['FTAddGrpType'];
        $tRefCode   = $paDataAddrWhere['FTAddRefCode'];
        $nSeqNo     = $paDataAddrWhere['FNAddSeqNo'];
        $tSQL       = "
            SELECT DISTINCT
                ADDR.FNLngID,
                ADDR.FTAddGrpType,
            ...
            LEFT JOIN TCNMSubDistrict_L SUDL WITH(NOLOCK) ON ADDR.FTAddV1SubDist = SUDL.FTSudCode AND SUDL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMDistrict_L    DSTL WITH(NOLOCK) ON ADDR.FTAddV1DstCode = DSTL.FTDstCode AND DSTL.FNLngID = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMProvince_L    PVNL WITH(NOLOCK) ON ADDR.FTAddV1PvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($nLangID)."
            WHERE ADDR.FDCreateOn <> ''
            AND ADDR.FNLngID        = ".$this->db->escape($nLangID)."
            AND ADDR.FTAddGrpType   = ".$this->db->escape($nGrpType)."
            AND ADDR.FTAddRefCode   = ".$this->db->escape($tRefCode)."
            AND ADDR.FNAddSeqNo     = ".$this->db->escape($nSeqNo)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
