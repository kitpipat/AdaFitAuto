...
class mBranch extends CI_Model {
    ...
    public function FSvMBCHGetAddress($ptData) {
        ...
        $tSQL           = "
            ...
            LEFT JOIN TCNMZoneObj ZNE WITH(NOLOCK) ON ADDL.FTAddRefCode  = ZNE.FTZneRefCode
            LEFT JOIN TCNMZone_L ZNEL WITH(NOLOCK) ON ZNE.FTZneChain     = ZNEL.FTZneChain
            LEFT JOIN TCNMProvince_L PVNL WITH(NOLOCK) ON ADDL.FTAddV1PvnCode = PVNL.FTPvnCode AND PVNL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMDistrict_L DSTL WITH(NOLOCK) ON ADDL.FTAddV1DstCode = DSTL.FTDstCode AND DSTL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMSubDistrict_L SUBDSTL WITH(NOLOCK) ON ADDL.FTAddV1SubDist = SUBDSTL.FTSudCode AND SUBDSTL.FNLngID = ".$this->db->escape($nLngID)."
            WHERE 1=1
            AND ADDL.FTAddRefCode   = ".$this->db->escape($tAddRefCode)."
            AND ADDL.FTAddGrpType   = ".$this->db->escape($tAddGrpType)."
            AND ADDL.FNLngID        = ".$this->db->escape($nLngID)."
        ";
    ...
    public function FSaMBchSearchByID($paData) {
        ...
        if (@$tBchCode) {
            $tSQL   =   "
                ...
                LEFT JOIN [TCNMWaHouse_L]       WAHL    WITH(NOLOCK) ON BCH.FTWahCode = WAHL.FTWahCode AND BCH.FTBchCode    = WAHL.FTBchCode  AND WAHL.FNLngID = ".$this->db->escape($nLngID)."
                LEFT JOIN [TCNMPdtPriList_L]    PRL     WITH(NOLOCK) ON BCH.FTPplCode = PRL.FTPplCode  AND PRL.FNLngID      = ".$this->db->escape($nLngID)."
                LEFT JOIN TCNMAgency_L          AGNL    WITH(NOLOCK) ON BCH.FTAgnCode = AGNL.FTAgnCode  AND AGNL.FNLngID    = ".$this->db->escape($nLngID)."
                -- LEFT JOIN TCNMMerchant_L     MERL    WITH(NOLOCK) ON BCH.FTMerCode = MERL.FTMerCode  AND MERL.FNLngID    = ".$this->db->escape($nLngID)."
                LEFT JOIN [TCNMImgObj]          IMGO    WITH(NOLOCK) ON BCH.FTBchCode = IMGO.FTImgRefID AND IMGO.FTImgTable = 'TCNMBranch'
            ";
            if ($tBchCode != '') {
                $tSQL   .= " WHERE BCH.FTBchCode = ".$this->db->escape($tBchCode)."";
            }
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    //Return : data
    //Return Type : Array
    public function FSvCBCHCountHQCondit() {
        $tSQL   =   "   SELECT COUNT(FTBchCode) AS CHQ FROM TCNMBranch WHERE FTBchStaHQ = 1";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $aDetail = $oQuery->result();
        } else {
    ...
    function JSnMBCHGetPageAll($ptSearchList, $ptLngID) {
        $tSQL = "   
            ...
            FROM TCNMBranch BCH WITH(NOLOCK)
            LEFT JOIN TCNMBranch_L BCHL ON BCH.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE 1 = 1
        ";
        if($this->session->userdata("tSesUsrLevel") != "HQ"){
            $tBchCode    = $this->session->userdata("tSesUsrBchCodeMulti");
        ...
        if ($ptSearchList != '') {
            $tSQL   .= " AND (BCH.FTBchCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
    ...
    public function FSnMBCHCheckduplicate($ptBchCode) {
        $tSQL = "
            SELECT COUNT(FTBchCode)AS counts
            FROM TCNMBranch
            WHERE FTBchCode = ".$this->db->escape($ptBchCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            return $oQuery->result();
    ...
    public function FSaMBCHWahDefaultInsert($paDataMaster){
        ...
        array_push($aDataWahDefualt,array(
            ...
            'FTWahRefCode'          => '',
            'FTWahStaChkStk'        => '3',
            'FTWahStaPrcStk'        => '2', //ตัดสต็อค
            'FDCreateOn'            => date('Y-m-d H:i:s'),
            'FTCreateBy'            => $paDataMaster['FTCreateBy'],
            'FDLastUpdOn'           => date('Y-m-d H:i:s'),
            'FTLastUpdBy'           => $paDataMaster['FTLastUpdBy'],
            'FTWahStaAlwCntStk'     => 1,
            'FTWahStaAlwCostAmt'    => 1
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'             => $paDataMaster['FTBchCode'],
            'FTWahCode'             => '00002', //คลังจองสินค้า
            'FTWahStaType'          => 7,
            'FTWahRefCode'          => '',
            'FTWahStaChkStk'        => '1',
            'FTWahStaPrcStk'        => '2', //ตัดสต็อค
            ...
            'FTLastUpdBy'           => $paDataMaster['FTLastUpdBy'],
            'FTWahStaAlwCntStk'     => 1,
            'FTWahStaAlwCostAmt'    => 2
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'             => $paDataMaster['FTBchCode'],
            'FTWahCode'             => '00003', //คลังเคลมรับ
            'FTWahStaType'          => 8,
            'FTWahRefCode'          => '',
            'FTWahStaChkStk'        => '1',
            'FTWahStaPrcStk'        => '2', //ตัดสต็อค
        ...
        array_push($aDataWahDefualt,array(
            'FTBchCode'             => $paDataMaster['FTBchCode'],
            'FTWahCode'             => '00004', //คลังเคลมเปลี่ยน
            'FTWahStaType'          => 9,
            'FTWahRefCode'          => '',
            'FTWahStaChkStk'        => '3',
            'FTWahStaPrcStk'        => '2', //ตัดสต็อค
    ...
    public function FSaMBCHWahDefaultInsert_L($paDataMaster){
        ...
        array_push($aDataWahDefualt,array(
            ...
            'FTWahCode'         => '00002',
            'FNLngID'           => 1,
            'FTWahName'         => 'คลังจองสินค้า',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00002',
            'FNLngID'           => 2,
            'FTWahName'         => 'Reserve Warehouse',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00003',
            'FNLngID'           => 1,
            'FTWahName'         => 'คลังเคลมรับ',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00003',
            'FNLngID'           => 2,
            'FTWahName'         => 'Cliam Get Customer Warehouse',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00004',
            'FNLngID'           => 1,
            'FTWahName'         => 'คลังเคลมเปลี่ยน',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00004',
            'FNLngID'           => 2,
            'FTWahName'         => 'Cliam To Supplier Warehouse',
        ));
        array_push($aDataWahDefualt,array(
            'FTBchCode'         => $paDataMaster['FTBchCode'],
            'FTWahCode'         => '00005',
    ...
    public function FSaMCMPGetBchInfo($paParams){
        ...
        $tSQL       = "
            SELECT TOP 1 *
            FROM TCNMBranch BCH WITH (NOLOCK)
            LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON BCHL.FTBchCode = BCH.FTBchCode AND BCHL.FNLngID    = ".$this->db->escape($nLangID)."
            LEFT JOIN TCNMAddress_L ADDL WITH (NOLOCK) ON ADDL.FTAddRefCode = BCH.FTBchCode AND ADDL.FTAddGrpType = '1'
            WHERE BCH.FTBchCode = ".$this->db->escape($tBchCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0) {
    ...
    public function FSaMBCHGetTempData($paDataSearch){
        ...
        $tSQL           = " 
            ...
            LEFT JOIN TCNMPdtPriList_L	PRI_L WITH(NOLOCK) ON IMP.FTPplCode = PRI_L.FTPplCode AND PRI_L.FNLngID = ".$this->db->escape($nLngID)."
        ...
        if($tTextSearch != '' || $tTextSearch != null){
            ...
            $tSQL   .= " OR IMP.FTBchName   LIKE '%".$this->db->escape_like_str($tTextSearch)."%' ";
            ...
            $tSQL   .= " OR IMP.FTPplCode   LIKE '%".$this->db->escape_like_str($tTextSearch)."%' ";
            $tSQL   .= " OR PRI_L.FTPplName LIKE '%".$this->db->escape_like_str($tTextSearch)."%' ";
            $tSQL   .= " )";
        }
        ...
        try{
            ...
            $tSQL   = " 
                ...
                SELECT
                    ...
                    '1',
                    '1',
                    ".$this->db->escape($dBchDateStart).",
                    ".$this->db->escape($dBchDateStop).",
                    ".$this->db->escape($dBchDateStart).",
                    ".$this->db->escape($dBchDateStop).",
                    ".$this->db->escape($dDateOn).",
                    ".$this->db->escape($tUserBy).",
                    ".$this->db->escape($dDateOn).",
                    ".$this->db->escape($tUserBy)."
                FROM TCNTImpMasTmp IMP WITH(NOLOCK)
                WHERE IMP.FTSessionID   = ".$this->db->escape($tSessionID)."
            ...
            $this->db->query($tSQL);
            $tSQL   = "
                INSERT INTO TCNMBranch_L (FTBchCode,FNLngID,FTBchName)
                SELECT
                    IMP.FTBchCode,
                    ".$this->db->escape($nLngID).",
                    IMP.FTBchName
                FROM TCNTImpMasTmp IMP WITH(NOLOCK)
                WHERE IMP.FTSessionID   = ".$this->db->escape($tSessionID)."
                AND IMP.FTTmpTableKey   = ".$this->db->escape($tTableKey)."
    ...
    public function FSaMBCHImportMove2MasterAndInsWah($paDataSearch){
        try{
            ...
            $tSQL   = " 
                ...
                CROSS APPLY (
                VALUES
                    (FTBchCode,'00001',1,FTBchCode,'3','2','$dDateOn','$tUserBy','$dDateOn','$tUserBy','1','1') ,
                    (FTBchCode,'00002',7,FTBchCode,'1','2','$dDateOn','$tUserBy','$dDateOn','$tUserBy','1','2') ,
                    (FTBchCode,'00003',8,FTBchCode,'1','2','$dDateOn','$tUserBy','$dDateOn','$tUserBy','2','2') ,
                    (FTBchCode,'00004',9,FTBchCode,'3','2','$dDateOn','$tUserBy','$dDateOn','$tUserBy','1','2') ,
                    (FTBchCode,'00005',10,FTBchCode,'1','2','$dDateOn','$tUserBy','$dDateOn','$tUserBy','1','2')
            ...
            $tSQL   = " 
                INSERT INTO TCNMWaHouse_L (FTBchCode,FTWahCode,FNLngID,FTWahName)
                SELECT PACKDATA.*
                FROM TCNTImpMasTmp Tmp
                CROSS APPLY (
                    VALUES
                        ...
                        (FTBchCode,'00002','1','คลังจองสินค้า'),
                        (FTBchCode,'00002','2','Reserve Warehouse'),
                        (FTBchCode,'00003','1','คลังเคลมรับ'),
                        (FTBchCode,'00003','2','Cliam Get Customer Warehouse'),
                        (FTBchCode,'00004','1','คลังเคลมเปลี่ยน'),
                        (FTBchCode,'00004','2','Cliam To Supplier Warehouse'),
                        (FTBchCode,'00005','1','คลังของเสีย'),
                        (FTBchCode,'00005','2','Waste Warehouse')
