...
class mAdMessage extends CI_Model {
    ...
    public function FSaMADVSearchByID($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tMSQL      =   "
            ...
            FROM [TCNMAdMsg] ADV
            LEFT JOIN [TCNMAdMsg_L] ADVL ON ADVL.FTAdvCode = ADV.FTAdvCode AND ADVL.FNLngID     = ".$this->db->escape($nLngID)."
            LEFT JOIN [TCNMAgency_L] AGNL ON ADV.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID    = ".$this->db->escape($nLngID)."
            WHERE ADV.FDCreateOn <> ''
            AND ADV.FTAdvCode   = ".$this->db->escape($tDstCode)."
        ";
        ...
        $tMediaSQL  =   "
            ...
            WHERE MDO.FDCreateOn <> ''
            AND MDO.FTMedRefID  = ".$this->db->escape($tDstCode)."
            ORDER BY MDO.FNMedSeq
        ";
    ...
    public function FSaMADVList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL           = "
            SELECT c.* FROM(
                ...
                FROM (
                    SELECT DISTINCT
                        ADS.FTAdvCode   AS rtAdvCodeLeft,
                        ADV.FTAdvCode   AS rtAdvCode,
                        ADVL.FTAdvName  AS rtAdvName,
                        ADVL.FTAdvMsg   AS rtAdvMsg,
                        ADV.FTAdvType   AS rtAdvType,
                    ...
                    LEFT JOIN [TCNMAdMsg_L] ADVL ON ADVL.FTAdvCode = ADV.FTAdvCode AND ADVL.FNLngID     = ".$this->db->escape($nLngID)."
                    LEFT JOIN [TCNMAgency_L] AGNL ON ADV.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID    = ".$this->db->escape($nLngID)."
                    LEFT JOIN (SELECT DISTINCT FTAdvCode FROM TCNMPosAds WITH(NOLOCK) ) ADS ON ADV.FTAdvCode = ADS.FTAdvCode
                    WHERE ADV.FDCreateOn <> ''
        ";
        if($tSesAgnCode != ''){
            $tSQL       .= "AND ADV.FTAgnCode = ".$this->db->escape($tSesAgnCode)." ";
        }
        $tSearchList     = $paData['tSearchAll'];
        if ($tSearchList != ''){
            $tSQL   .= " AND (ADV.FTAdvCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR ADVL.FTAdvName  LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
    ...
    public function FSnMADVGetPageAll(/*$ptWhereCode,*/ $ptSearchList, $ptLngID, $tSesAgnCode){
        $tSQL   = "
            SELECT COUNT (ADV.FTAdvCode) AS counts
            FROM [TCNMAdMsg] ADV
            LEFT JOIN [TCNMAdMsg_L] ADVL ON ADVL.FTAdvCode = ADV.FTAdvCode AND ADVL.FNLngID  = ".$this->db->escape($ptLngID)."
            LEFT JOIN [TCNMAgency_L] AGNL ON ADV.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE ADV.FDCreateOn <> ''
        ";
        if($tSesAgnCode != ''){
            $tSQL   .= "AND ADV.FTAgnCode = ".$this->db->escape($tSesAgnCode)." ";
        ...
        if($ptSearchList != ''){
            $tSQL   .= " AND (ADV.FTAdvCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
            $tSQL   .= " OR ADVL.FTAdvName  LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
        }
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSoMADVCheckDuplicate($ptAdvCode){
        $tSQL   = "
            SELECT COUNT(FTAdvCode) AS counts
            FROM TCNMAdMsg
            WHERE FTAdvCode = ".$this->db->escape($ptAdvCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
    ...
    public function FSoMADVCheckMediaFileNameDuplicate($paData){
        ...
        $tSQL       = "
            SELECT COUNT(FTMedPath) AS counts
            FROM TCNMMediaObj
            WHERE FTMedRefID = ".$this->db->escape($tAdvCode)." AND FTMedPath = ".$this->db->escape($tPath)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
    ...
    public function FSaMADVSearchMediaByID($nMedID, $paData){
        ...
        $tMediaSQL  = "
            ...
            FROM [TCNMMediaObj] MDO
            WHERE MDO.FDCreateOn <> ''
            AND MDO.FNMedID     = ".$this->db->escape($nMedID)."
            AND MDO.FTMedRefID  = ".$this->db->escape($tDstCode)."
            ORDER BY MDO.FNMedSeq
        ";
        $oMediaQuery = $this->db->query($tMediaSQL);
