...
class mSetprinter extends CI_Model {
    ...
    public function FSaMSPRSearchByID($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL       = "
            ...
            LEFT JOIN [TCNMPrinter_L] SPRL ON SPR.FTPrnCode = SPRL.FTPrnCode AND SPRL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN [TSysPortPrn_L] PRL  ON SPR.FTSppCode = PRL.FTSppCode AND PRL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN [TSysPortPrn]   PR   ON SPR.FTSppCode = PR.FTSppCode
            WHERE SPR.FDCreateOn <> ''
        ";
        if($tDstCode!= ""){
            $tSQL   .= "AND SPR.FTPrnCode = ".$this->db->escape($tDstCode)." ";
        }
        ...
        $tSQL       = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FTPrnCode ASC) AS rtRowID,*
                    FROM (
                        SELECT DISTINCT
                            PHW.FTPhwCodeRef AS rtPhwCodeRef,
                            SPR.FTPrnCode,
                            SPR.FTPrnSrcType,
                            SPR.FTSppCode,
                            SPR.FTPrnDriver,
                        ...
                        FROM [TCNMPrinter] SPR
                        LEFT JOIN [TCNMPrinter_L] SPRL ON SPR.FTPrnCode = SPRL.FTPrnCode AND SPRL.FNLngID = ".$this->db->escape($nLngID)."
                        LEFT JOIN (SELECT DISTINCT FTPhwCodeRef FROM TCNMPosHW WITH(NOLOCK) ) PHW ON SPR.FTPrnCode = PHW.FTPhwCodeRef
                        WHERE SPR.FDCreateOn <> ''
        ";
        $tSearchList = $paData['tSearchAll'];
        if ($tSearchList != ''){
            $tSQL   .= " AND (SPRL.FTPrnName LIKE '%".$this->db->escape_like_str($tSearchList)."%' )";
        }
    ...
    public function FSnMSPRGetPageAll($ptSearchList, $ptLngID){
        $tSQL   = "
            SELECT COUNT (SPR.FTPrnCode) AS counts
            FROM [TCNMPrinter] SPR
            LEFT JOIN [TCNMPrinter_L]  SPNL ON SPR.FTPrnCode = SPNL.FTPrnCode AND SPNL.FNLngID = ".$this->db->escape($ptLngID)."
            WHERE SPR.FDCreateOn <> ''
        ";
        if($ptSearchList != ''){
            $tSQL   .= " AND (SPNL.FTPrnName LIKE '%".$this->db->escape_like_str($ptSearchList)."%' )";
        }
        $oQuery      = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSoMSPNCheckDuplicate($ptDstCode){
        $tSQL   = "
            SELECT COUNT(FTPrnCode) AS counts
            FROM TCNMPrinter
            WHERE FTPrnCode = ".$this->db->escape($ptDstCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
