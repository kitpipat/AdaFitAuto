...
class mPosEdc extends CI_Model {
    ...
    public function FSaMGetDataPosEdcList($paDataWhere){
        ...
        $tSQL   = "
            SELECT c.* FROM (
                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , FDCreateOn DESC , FTEdcCode DESC) AS rtRowID,* 
                    FROM (
                        SELECT DISTINCT
                            PHW.FTPhwCodeRef,
                            EDC.FTEdcCode,
                        ...
                        LEFT JOIN TFNMEdc_L  AS EDC_L    WITH(NOLOCK) ON EDC.FTEdcCode = EDC_L.FTEdcCode AND EDC_L.FNLngID  = ".$this->db->escape($nLngID)."
                        ...
                        LEFT JOIN TFNMBank_L AS BNK_L    WITH(NOLOCK) ON BNK.FTBnkCode = BNK_L.FTBnkCode AND BNK_L.FNLngID  = ".$this->db->escape($nLngID)."
                        LEFT JOIN TCNMImgObj AS IMG      WITH(NOLOCK) ON EDC.FTEdcCode = IMG.FTImgRefID  AND FTImgTable = 'TFNMEdc'
                        LEFT JOIN (SELECT DISTINCT FTPhwCodeRef FROM TCNMPosHW WITH(NOLOCK)) PHW ON EDC.FTEdcCode = PHW.FTPhwCodeRef
                        WHERE EDC.FDCreateOn <> ''
        ";

        if($tAgnCode != ''){
            $tSQL   .= "AND EDC.FTAgnCode = ".$this->db->escape($tAgnCode)." ";
        }

        if(isset($tSearchList) && !empty($tSearchList)){
            $tSQL   .= " AND (EDC.FTEdcCode COLLATE THAI_BIN    LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR EDC_L.FTEdcName COLLATE THAI_BIN    LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL   .= " OR TSEDC.FTSedModel COLLATE THAI_BIN   LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
    ...
    public function FSaMPosEdcGetDataByID($paDataWhere){
        ...
        $tSQL           = " 
            SELECT
                ...
                IMG.FTImgObj        AS rtImgObj
            FROM TFNMEdc            AS EDC      WITH(NOLOCK)
            LEFT JOIN TCNMAgency_L  AS AGN_L    WITH(NOLOCK) ON EDC.FTAgnCode = AGN_L.FTAgnCode AND AGN_L.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TFNMEdc_L     AS EDC_L    WITH(NOLOCK) ON EDC.FTEdcCode = EDC_L.FTEdcCode AND EDC_L.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TSysEdc       AS TSEDC    WITH(NOLOCK) ON EDC.FTSedCode = TSEDC.FTSedCode
            LEFT JOIN TFNMBank      AS BNK      WITH(NOLOCK) ON EDC.FTBnkCode = BNK.FTBnkCode
            LEFT JOIN TFNMBank_L    AS BNK_L    WITH(NOLOCK) ON BNK.FTBnkCode = BNK_L.FTBnkCode AND BNK_L.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMImgObj    AS IMG      WITH(NOLOCK) ON EDC.FTEdcCode = IMG.FTImgRefID  AND FTImgTable = 'TFNMEdc'
            WHERE EDC.FDCreateOn <> ''
            AND EDC.FTEdcCode   = ".$this->db->escape($tPosEdcCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0) {
            $aDetail        = $oQuery->row_array();
