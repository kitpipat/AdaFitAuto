...
<script type="text/javascript">
    ...
    // Function : เปิดหน้าจอแสดงรายละเอียดการจอง
    // CreateBy : 04/04/2022 Wasin
    function JSvInvMmtDetailStockBooking(evn){
        var oDataSend   = {
            'tBchCode'  : $(evn).parents('.otrReason').data('bchcode'),
            'tWahCode'  : $(evn).parents('.otrReason').data('wahcode'),
            'tPdtCode'  : $(evn).parents('.otrReason').data('code'),
        };
        $.ajax({
            type    : "POST",
            url     : "mmtINVStkBookDetail",
            data    : oDataSend,
            cache   : false,
            timeout : 0,
            success : function (tResult) {
                if(tResult != ''){
                    $('#odvInvStockBookingDetail .modal-body').html(tResult);
                    $('#odvInvStockBookingDetail').modal('show');
                }
            },
