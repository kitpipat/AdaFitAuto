...
class Rptopenjob_controller extends MX_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model('report/reportFitauto/Rptopenjob_model');
        // Init Report
        $this->init();
        parent::__construct();
    }
    ...
    private function init(){
        $this->aText = [
            'tTitleReport'          => language('report/report/report', 'tRptOpenJobTitle'),
        ...
        $this->aRptFilter       = [
            ...
            'tPosCodeSelect'    => !empty($this->input->post('ocmPosType')) ? $this->input->post('ocmPosType') : "",
            // Filter Document Date (วันที่สร้างเอกสาร)
            'tDocDateFrom'      => (empty($this->input->post('oetRptDocDateFrom'))) ? '' : $this->input->post('oetRptDocDateFrom'),
            'tDocDateTo'        => (empty($this->input->post('oetRptDocDateTo'))) ? '' : $this->input->post('oetRptDocDateTo'),
            //สถานะเปิดเอกสาร
            'tStaOdr' => !empty($this->input->post('ocmRptPhStaOdr')) ? $this->input->post('ocmRptPhStaOdr') : "",
        ];
        // ดึงข้อมูลบริษัทฯ
        $aCompInfoParams = [
            'nLngID' => $this->nLngID,
    ...
    public function index(){
        if (!empty($this->tRptExportType) && !empty($this->tRptCode)) {
            ...
            $this->Rptopenjob_model->FSnMExecStoreReport($this->aRptFilter);
    ...
    public function FSvCCallRptViewBeforePrint($paDataSwitchCase){
        try {
            ...
            $aDataReport = $this->Rptopenjob_model->FSaMGetDataReport($aDataWhere);
            ...
            // Load View Advance Table
            $tRptView = JCNoHLoadViewAdvanceTable('report/datasources/reportFitauto', 'wRptopenjobHtml', $aDataViewRptParams);
    ...
    public function FSvCCallRptViewBeforePrintClickPage(){
        ...
        $aDataReport = $this->Rptopenjob_model->FSaMGetDataReport($aDataWhereRpt);
    ...
    public function FSvCCallRptExportFile(){
        ...
        $aCells = [
            ...
            WriterEntityFactory::createCell(null),
            WriterEntityFactory::createCell(language('report/report/report', 'tRptTotalSub2')),
            WriterEntityFactory::createCell(null),
        ...
        $aDataReport = $this->Rptopenjob_model->FSaMGetDataReport($aDataReportParams);
        ...
        if (isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])) {
            ...
            foreach ($aDataReport['aRptData'] as $nKey => $aValue) {
                $tStaOdr = '';
                if ($aValue['FTXshStaOrd'] == 1) {
                    $tStaOdr = language('report/report/report', 'tRptPhStaOdr1');
                }elseif ($aValue['FTXshStaOrd'] == 2) {
                    $tStaOdr = language('report/report/report', 'tRptPhStaOdr2');
                }elseif ($aValue['FTXshStaOrd'] == 3) {
                    $tStaOdr = language('report/report/report', 'tRptPhStaOdr3');
                }else{
                    $tStaOdr = '';
                }
                ...
                $values = [
                    ...
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell($tStaOdr),
                    WriterEntityFactory::createCell(null),
                    ...
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(number_format($aValue['FCXshTotal'], 2)),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(number_format($aValue['FCXshDiscount'], 2)),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(number_format($aValue['FCXshGrand'], 2)),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(number_format($aValue['FCXshVatable'], 2)),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell(number_format($aValue['FCXshVat'], 2)),
                    WriterEntityFactory::createCell(null),
                ];
                ...
                if (($nKey + 1) == FCNnHSizeOf($aDataReport['aRptData'])) { //SumFooter
                    $values = [
                        ...
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(language('report/report/report', 'tRptTotal')),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        ...
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(number_format($aDataTotal['FCXshTotal_Footer'], 2)),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(number_format($aDataTotal['FCXshDiscount_Footer'], 2)),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(number_format($aDataTotal['FCXshGrand_Footer'], 2)),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(number_format($aDataTotal['FCXshVatable_Footer'], 2)),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(number_format($aDataTotal['FCXshVat_Footer'], 2)),
                        WriterEntityFactory::createCell(null),
                    ];
    ...
    public function FSoCCallRptRenderFooterExcel(){
        ...
        $tPos = '';
        if ($this->aRptFilter['tPosCodeSelect'] == 1) {
            $tPos = language('report/report/report', 'tRptRetail');
        }elseif ($this->aRptFilter['tPosCodeSelect'] == 2) {
        ...
        if (isset($this->aRptFilter['tPosCodeSelect']) && !empty($this->aRptFilter['tPosCodeSelect'])) {
            $aCells = [
                WriterEntityFactory::createCell(language('report/report/report', 'tRptTaxSalePosType') . ' : ' . $tPos),
            ];
            $aMulltiRow[] = WriterEntityFactory::createRow($aCells);
        }else{
            $aCells = [
        ...
        // ฟิวเตอร์ข้อมูล สถานะใบเปิดงาน 
        $tStaOdr = '';
            if ($this->aRptFilter['tStaOdr'] == 1) {
                $tStaOdr = language('report/report/report', 'tRptPhStaOdr1');
            }elseif ($this->aRptFilter['tStaOdr'] == 2) {
                $tStaOdr = language('report/report/report', 'tRptPhStaOdr2');
            }elseif ($this->aRptFilter['tStaOdr'] == 3) {
                $tStaOdr = language('report/report/report', 'tRptPhStaOdr3');
            }else{
                $tStaOdr = language('report/report/report', 'tRptAll');
            }
        if (isset($this->aRptFilter['tStaOdr']) && !empty($this->aRptFilter['tStaOdr'])) {
            $aCells = [
