...
class Rptdebtorbytnf_controller extends MX_Controller 
{
    // ภาษา
    public $aText = [];
    ...
    public function __construct()
    {
        ...
        $this->load->model('report/reportFitauto/Rptdebtorbytnf_model');
        // Init Report
        $this->init();
        parent::__construct();
    }
    ...
    private function init()
    {
        // Array Text Label
        $this->aText    = [
            'tTitleReport'          => language('report/report/report', 'tRptreducedebtbytnf'),
    ...
    public function index()
    {
        if (!empty($this->tRptExportType) && !empty($this->tRptCode)) {
            ...
            $this->Rptdebtorbytnf_model->FSnMExecStoreReport($this->aRptFilter);
    ...
    public function FSvCCallRptViewBeforePrint()
    {
        ...
        $aDataReport    = $this->Rptdebtorbytnf_model->FSaMGetDataReport($aDataWhere);
        ...
        $tRptView   = JCNoHLoadViewAdvanceTable('report/datasources/reportFitauto', 'wRptDebtorByTnf', $aDataViewRptParams);
    ...
    public function FSvCCallRptExportFile()
    {
        ...
        $aDataReport    = $this->Rptdebtorbytnf_model->FSaMGetDataReport($aDataReportParams);
        ...
        $aCells = [
            ...
            WriterEntityFactory::createCell('อ้างอิงใบเบิกออก'),
        ...
        if (isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])) {
            foreach ($aDataReport['aRptData'] as $nKey => $aValue) {
                $values = [
                    WriterEntityFactory::createCell((!empty($aValue['FTSplName']))? $aValue['FTSplName'] : '-'),
                    WriterEntityFactory::createCell($aValue['FTXshDocNo']),
                    WriterEntityFactory::createCell(date("d/m/Y", strtotime($aValue["FDXshDocDate"]))),
                    WriterEntityFactory::createCell((!empty($aValue['FTXshRefDocNo']))? $aValue['FTXshRefDocNo'] : '-' ),
                ...
                $oWriter->addRow($aRow);
                if($aValue['PARTTITIONBYSPL_COUNT'] == $aValue['PARTTITIONBYSPL']){
                    $values = [
                        WriterEntityFactory::createCell('ยอดรวม : ' . $aValue['FTSplName']),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(FCNnGetNumeric($aValue['FCXshAmtNV_SubTotal'])),
                        WriterEntityFactory::createCell(FCNnGetNumeric($aValue['FCXshVatable_SubTotal'])),
                        WriterEntityFactory::createCell(FCNnGetNumeric($aValue['FCXshVat_SubTotal'])),
                        WriterEntityFactory::createCell(FCNnGetNumeric($aValue['FCXshGrand_SubTotal'])),
                    ];
                $aRow   = WriterEntityFactory::createRow($values);
                $oWriter->addRow($aRow);
                }
