...
function JSvCallPageReportCondition(){
    ...
    $.ajax({
        ...
        success: function (tResult) {
            ...
            // $('#ospSpecialDetailReport').text("");
            if( $('#ohdRptRoute').val() == 'rptSalesSumQtyAndPrice' ||
                $('#ohdRptRoute').val() == 'rptCheckSTKAllBch' ||
                $('#ohdRptRoute').val() == 'rptSalesAllBchByUNIT' || 
                $('#ohdRptRoute').val() == 'rptSalesAllBchByPDT' ){
                $('#ospSpecialDetailReport').text("* รายงานนี้แสดงข้อมูล ย้อนหลัง 1 วันเสมอ");
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
