    ...
    public function FSaMGetDataReport($paDataWhere) {
        ...
        $tSQL = "
            SELECT C.*
                FROM (
                    SELECT 
                        ROW_NUMBER() OVER(ORDER BY DATAGRP.FCXsdQty DESC,DATAGRP.FTPdtCode ASC) AS rtRowID,
                        DATAGRP.*
                    FROM(
                        SELECT
                            FTPdtCode,
        ...
        $tSQL .= "
                        GROUP BY FTPdtCode,FTXsdPdtName,FTPgpChainName,FTPunName,FCXsdSetPrice,FTPdtCatCode1,FTPdtCatName1,FTPdtCatCode2,FTPdtCatName2 
                    ) AS DATAGRP 
                ) AS C
