...
class RptPdtCouponPromotion_model extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore     = "{ CALL SP_RPTxPSCouponByBillPmt(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        ...
        $aDataStore = array(
            ...
            // 'ptDocCodeF'      => FCNtAddSingleQuote($paDataFilter['tDocPromotionCodeFrom']),
            'ptCouponCodeF'      => FCNtAddSingleQuote($paDataFilter['tCouponCodeFrom']),
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter = "   
                ...
                FROM TRPTPSCouponByBillTmp WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL   = " SELECT
                    ...
                    FROM (
                        ...
                        FROM TRPTPSCouponByBillTmp A WITH(NOLOCK)
                        /* Calculate Misures */
                        LEFT JOIN (
                            SELECT
                            ...
                            WHERE 1=1
                            AND FTUsrSession    = '$tUsrSession'
                            GROUP BY FTUsrSession,FTPmhDocNo
                        ) AS S ON A.FTPmhDocNo  = S.FTPmhDocNo_SUM AND A.FTUsrSession = S.FTUsrSession_SubTotal
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL   = " UPDATE TRPTPSCouponByBillTmp
                        SET FNRowPartID = B.PartID
                    FROM(
                        SELECT
                            ROW_NUMBER() OVER(PARTITION BY FTPmhDocNo,FTPmhName ORDER BY FTPmhDocNo DESC) AS PartID,
                            FTRptRowSeq
                        FROM TRPTPSCouponByBillTmp TMP WITH(NOLOCK)
                        WHERE TMP.FTUsrSession    = '$tUsrSession'
                    ) AS B
                    WHERE TRPTPSCouponByBillTmp.FTRptRowSeq = B.FTRptRowSeq
                    AND TRPTPSCouponByBillTmp.FTUsrSession   = '$tUsrSession'
                ";
