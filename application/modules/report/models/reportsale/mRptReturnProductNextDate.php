...
class mRptReturnProductNextDate extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTxSalPdtRetNextDate(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID' => $nLangID, 
            'pnComName' => $tComName,
            'ptRptCode' => $tRptCode,
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTxSalPdtRetNextDateTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT
                    ...
                    A.*,
                    S.FNRptGroupMember
                FROM TRPTxSalPdtRetNextDateTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTxSalPdtRetNextDateTmp TSBP WITH(NOLOCK)
            WHERE 1=1
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL = "
            UPDATE TRPTxSalPdtRetNextDateTmp
                SET TRPTxSalPdtRetNextDateTmp.FNRowPartID = B.PartID
            FROM (
                SELECT
            ...
            WHERE 1=1
            AND TRPTxSalPdtRetNextDateTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTxSalPdtRetNextDateTmp.FTComName = '$tComName' 
            AND TRPTxSalPdtRetNextDateTmp.FTRptCode = '$tRptCode'
            AND TRPTxSalPdtRetNextDateTmp.FTUsrSession = '$tUsrSession'
        ";
        $this->db->query($tSQL);
    }
    ...
    public function FSnMCountRowInTemp($paParams)
    {
        ...
        $tSQL = "   
            SELECT
                TMP.FTRptCode
            FROM TRPTxSalPdtRetNextDateTmp TMP WITH(NOLOCK)
