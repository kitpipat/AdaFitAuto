...
class Rptincreasedebtreceivable_model extends CI_Model
{

    // Call Stored
    public function FSnMExecStoreReport($paDataFilter)
    {
        $tCallStore = "{ CALL SP_RPTxRcvAbleDN(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   = "
                ...
                FROM TRPTRcvAbleDNTmp WITH(NOLOCK)
                WHERE FTUsrSession <> '' 
        ...
        $tSQL   = "
            ...
            FROM (
                ...
                ROW_NUMBER() OVER(
                    ...
                    S.FCXshVat_SubTotal
                FROM TRPTRcvAbleDNTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           = "
            ...
            FROM TRPTRcvAbleDNTmp RPT WITH(NOLOCK)
            WHERE 
