...
class Rptdebtorbybills_model extends CI_Model
{

    // Call Stored
    public function FSnMExecStoreReport($paDataFilter)
    {
        $tCallStore = "{ CALL SP_RPTxDebtDNByReceipt(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   = "
                ...
                FROM TRPTDebtDNByReceiptTmp WITH(NOLOCK)
                WHERE FTUsrSession <> '' 
        ...
        $tSQL   = "
            ...
            FROM (
                ...
                ROW_NUMBER() OVER(
                    ...
                    S.*,
                    D.*
                FROM TRPTDebtDNByReceiptTmp A WITH(NOLOCK)
                LEFT JOIN (
                    SELECT
                    ...
                    FROM
                        TRPTDebtDNByReceiptTmp WITH ( NOLOCK ) 
                    WHERE
                        FTUsrSession <> '' 
                        AND FTUsrSession = '$tUsrSession' 
                    GROUP BY
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           = "
            ...
            FROM TRPTDebtDNByReceiptTmp RPT WITH(NOLOCK)
            WHERE 
