...
class Rptcstforcastbycar_model extends CI_Model {
    /**
    ...
    public function FSnMExecStoreReport($paDataFilter){
        ...
        $tCallStore = "{CALL SP_RPTxCstForcastByCar(?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            ...
            'pdLastServDateFrm'     => $paDataFilter['tDocDateFrom'],
            'pdLastServDateTo'      => $paDataFilter['tDocDateTo'],
            'pnLngID'               => $nLangID,
            'pnResult'              => 0,
        );
    ...
    public function FSnMCountRowInTemp($paParams){
        ...
        $tSQL = "
            SELECT
                TMP.FTUsrSession
            FROM TRPTCstForcastByCar TMP WITH(NOLOCK)
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        $tJoinFoooter = "   
            ...
            FROM TRPTCstForcastByCar WITH(NOLOCK)
            WHERE 1=1
        ...
        $tSQL = "SELECT C.*,T.*
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(ORDER BY A.FDFlwDateForcast ASC , A.FNRptRowSeq ASC) AS rtRowID,
                        A.*
                    FROM(
                        SELECT
                            *
                        FROM TRPTCstForcastByCar
    ...
    private function FMaMRPTPagination($paDataWhere){
        ...
        $tSQL = "
            SELECT
                TSPT.FTUsrSession
            FROM TRPTCstForcastByCar TSPT WITH(NOLOCK)
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = "
            UPDATE TRPTCstForcastByCar
                SET TRPTCstForcastByCar.FNRptRowSeq = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY convert(varchar, FDFlwDateForcast, 23) ORDER BY FDFlwDateForcast ASC) AS PartID ,TMP.FNRptRowSeq
        ...
        $tSQL .= "
            ...
            WHERE 1=1
            AND TRPTCstForcastByCar.FNRptRowSeq = B.FNRptRowSeq
            AND TRPTCstForcastByCar.FTUsrSession = '$tUsrSession'
        ";
        $this->db->query($tSQL);
    }
