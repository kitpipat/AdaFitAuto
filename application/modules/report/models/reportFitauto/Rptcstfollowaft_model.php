...
class Rptcstfollowaft_model extends CI_Model {
    /**
    ...
    public function FSnMExecStoreReport($paDataFilter){
        ...
        $tCallStore = "{CALL SP_RPTxCstFollowAFSrvTmp(?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            ...
            'ptCarRegNoFrm'     => $paDataFilter['tRegCodeFrom'],
            'ptCarRegNoTo'      => $paDataFilter['tRegCodeTo'],
            'pdServDateFrm'     => $paDataFilter['tDocDateFrom'],
            'pdServDateTo'      => $paDataFilter['tDocDateTo'],
            'pnResult'          => 0,
        );
    ...
    public function FSnMCountRowInTemp($paParams){
        ...
        $tSQL = "
            SELECT
                TMP.FTUsrSession
            FROM TRPTCstFollowAFSrvTmp TMP WITH(NOLOCK)
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        $tJoinFoooter = "   
            SELECT
                ...
                COUNT(FTXshDocNo)      AS Cst_Total
            FROM TRPTCstFollowAFSrvTmp WITH(NOLOCK)
            WHERE 1=1
        ...
        $tSQL = "SELECT C.*,T.*
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(ORDER BY A.FDXshDocDate ASC , A.FNRptRowSeq ASC) AS rtRowID,
                        A.*
                    FROM(
                        SELECT
                            *
                        FROM TRPTCstFollowAFSrvTmp
    ...
    private function FMaMRPTPagination($paDataWhere){
        ...
        $tSQL = "
            SELECT
                TSPT.FTUsrSession
            FROM TRPTCstFollowAFSrvTmp TSPT WITH(NOLOCK)
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = "
            UPDATE TRPTCstFollowAFSrvTmp
                SET TRPTCstFollowAFSrvTmp.FNRptRowSeq = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY convert(varchar, FDXshDocDate, 23) ORDER BY FDXshDocDate ASC) AS PartID ,TMP.FNRptRowSeq
        ...
        $tSQL .= "
            ...
            WHERE 1=1
            AND TRPTCstFollowAFSrvTmp.FNRptRowSeq = B.FNRptRowSeq
            AND TRPTCstFollowAFSrvTmp.FTUsrSession = '$tUsrSession'
        ";
        $this->db->query($tSQL);
    }
