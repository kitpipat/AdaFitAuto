...
class Rptcreditordebtbyrcv_model extends CI_Model
{

    // Call Stored
    public function FSnMExecStoreReport($paDataFilter)
    {
        $tCallStore = "{ CALL SP_RPTxCreditorDebtByRcv(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   = "
                ...
                FROM TRPTCreditorDebtByRcvTmp WITH(NOLOCK)
                WHERE FTUsrSession <> '' 
        ...
        $tSQL   = "
            ...
            FROM (
                ...
                ROW_NUMBER() OVER(
                    ...
                    S.FCXphVat_SubTotal
                FROM TRPTCreditorDebtByRcvTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           = "
            ...
            FROM TRPTCreditorDebtByRcvTmp RPT WITH(NOLOCK)
            WHERE 
