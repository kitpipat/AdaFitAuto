...
class Rptcreditordebt_model extends CI_Model
{

    // Call Stored
    public function FSnMExecStoreReport($paDataFilter)
    {
        $tCallStore = "{ CALL SP_RPTxCreditorDebt(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   = "
                ...
                FROM TRPTCreditorDebtTmp WITH(NOLOCK)
                WHERE FTUsrSession <> '' 
        ...
        $tSQL   = "
            ...
            FROM (
                ...
                ROW_NUMBER() OVER(
                    ...
                    S.FCXphVat_SubTotal
                FROM TRPTCreditorDebtTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           = "
            ...
            FROM TRPTCreditorDebtTmp RPT WITH(NOLOCK)
            WHERE 
