$(document).ready(function() {
    ...
    $(".xCNInpuCrdFmt").on("keypress keyup blur", function(event) {
        var tInputVal = $(this).val();
        var tInputVals = "";
        for (var i = 0; i < tInputVal.length; i++) {
          if (tInputVal[i]==0 || tInputVal[i] =="*") {
            tInputVals+= tInputVal[i];
          }
        }
        $(this).val(tInputVals);
    });
    ...
    $('body').on("keyup blur change", '.xCNInputNumericWithDecimalReplace1', function(event) {
