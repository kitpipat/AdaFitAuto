...
function JSxImageUplodeResize(poImg, ptRetio) {
    ...
    oImgFrom.append(tCSRFTokenName,tCSRFTokenValue);
...
function JSxChooseImageNEW(oImgBase64, ptImgName, pnBrowseType, ptMasterName) {
    ...
    if (pnBrowseType == '1') {
        ...
        var aImage      = oImgBase64.split(",");
        var aImagePath  = aImage[0];
        var aImageBase  = aImage[1];
        var tImageBase  = aImagePath + '%COMMA%' + aImageBase;
        $('#oetImgInput' + ptMasterName).val(tImageBase).trigger('change');
