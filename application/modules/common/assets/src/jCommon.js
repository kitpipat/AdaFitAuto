...
function JCNxFuncChkSessionExpired() {
    ...
    $.ajax({
        ...
        success: function(nReturn) {
            nStaSession = nReturn;
            if (nStaSession.toString() == 0) {
                var tLogFunction = 'EVENT';
                var tDisplayEvent = 'ตรวจสอบ Session';
                var tErrorStatus = 400;
                var tHtmlError = 'Session Expired';
                var tMsgErrorBody = tHtmlError
...
function FSvCMNSetMsgSucessDialog(tMsgBody) {
    $('#odvModalInfoMessageWarning .modal-body ').html(tMsgBody);
    $('#odvModalInfoMessageWarning').modal({ backdrop: 'static', keyboard: false })
    JCNxCloseLoading();
    $('#odvModalInfoMessageWarning').modal({ show: true });
}
                            ...
                            if (bCheckCompare) {
                                ...
                                if (poParameter.FunctionName != "") {
                                    ...
                                    for (let nI = 0; nI < poParameter.DataAttribute.length; nI++) {
                                        ...
                                        if (aDOCPdtTableTDChildElementAttr !== undefined && aDOCPdtTableTDChildElementAttr != "") {
                                            oParameters.DataAttribute[nI] = {
                                                [poParameter.DataAttribute[nI]]: $(this).attr(poParameter.DataAttribute[nI])
                                            };
                                        }
                                    }
...
function JCNxResponseError(jqXHR, textStatus, errorThrown) {
    ...
    if (jqXHR.status == 404) {
        ...
        var tDisplayEvent = 'Call Page';
        var tErrorStatus  = '404'
        var tLogDocNo     = '';
...
// function JCNxSendMQPageNotFound(jqXHR, ptDocNo) {
//     JCNxCloseLoading();
//     var tHtmlError = $(jqXHR.responseText);
//     var tMsgErrorBody = tHtmlError.find('p:nth-child(2)').text();
//     if (jqXHR.status == 404) {
//         var tLogFunction = 'WARNING';
//         var tDisplayEvent = 'Call Page';
//         var tErrorStatus  = '404'
...
function JCNxClickCheckBox(pnStaClick) {
    ...
    $('.ocbListItem').each(function() {
        if (pnStaClick) { // กรณีติ๊กถูก
            if ($(this).parent().find('span').hasClass('xCNDocDisabled') == true) {
            } else if (($(this).parent().find('span').hasClass('xCNDisabled') == true)){
...
//Functionality : ปุ่ม Side Bar ย่อ/ขยาย หน้าเอกสาร
//Parameters : -
//Creator : 12/12/2021 Worakorn
//Last Update : -
//Return : -
//Return Type : -
function JCNxOpenDiv() {
    $('#odvSideBar').show();
    $('.xCNDivSideBarOpen').addClass('xCNHide');
    // $('.xCNDivSideBarOpen').hide(500);
    $('.xWRight').removeClass('col-xs-12 col-sm-12 col-md-12 col-lg-12');
    $('.xWRight').addClass('col-xs-12 col-sm-9 col-md-9 col-lg-9');
    $('.xWRight').removeClass('xCNDivRightSideBar');
}

function JCNxCloseDiv() {
    $('#odvSideBar').hide();
    // $('.xCNDivSideBarOpen').fadeIn('slow');
    $('.xCNDivSideBarOpen').removeClass('xCNHide');
    $('.xWRight').removeClass('col-xs-12 col-sm-9 col-md-9 col-lg-9');
    $('.xWRight').addClass('col-xs-12 col-sm-12 col-md-12 col-lg-12');
    $('.xWRight').addClass('xCNDivRightSideBar');
}
