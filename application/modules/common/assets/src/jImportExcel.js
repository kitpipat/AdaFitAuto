...
function JSxImportPopUp(poPackdata) {
    ...
    $('#obtIMPConfirmUpload').attr("disabled", false);
    ...
    if (f) {
        ...
        r.onload = e => {
            ...
            switch (tNameModule) {
                ...
                case 'purchaseinvoice':
                    //ตรวจสอบชื่อชิทว่าถูกต้องไหม
                    if (typeof(aJSON['Purcase Invoice']) == 'undefined') {
                        alert('รูปแบบเอกสารไม่ถูกต้อง กรุณาลองใหม่อีกครั้ง');
                        return;
                    }

                    var aJSONData = aJSON["Purcase Invoice"];
                    var nCount = aJSONData.length;
                    var aNewPackData = [];
                    var aError = [];
                    ...
                    for (var j = 1; j < nCount; j++) {
                        ...
                        if (typeof(aJSONData[j][4]) != 'undefined' || null) {
                            if (aJSONData[j][4] == null) {
                                ...
                                if (nValue.match(Letters)) {
                                    ...
                                    aJSONData[j].pop();
                                    aJSONData[j].push(0);
                                    aError.push('3', '[4]' + '$&รูปแบบราคาผิด$&' + tValueOld);
                                }
                            }
                        } else {
                            aJSONData[j][3] = '0';
                        ...
                        if (typeof(aJSONData[j][2]) != 'undefined' || null) {
                            if (aJSONData[j][2] == null) {
                                aJSONData[j][2] = 'N/A';
                                aError.push('7', '[2]' + '$&รหัสหน่วยสินค้าไม่ได้ระบุข้อมูล$&' + 'N/A');
                            } else {
                                if (aJSONData[j][2].length > 25) {
                                    var tValueOld = aJSONData[j][2];
                                    aJSONData[j][2] = aJSONData[j][2].substring(0, 5);
                                    aError.push('4', '[2]' + '$&รหัสหน่วยสินค้ายาวเกินกำหนด$&' + tValueOld);
                                }
                            }
                        } else {
                            aJSONData[j][2] = '00000';
        ...
        //ล้างค่า
        $('#oefFileImportExcel').val("");
        $('#obtIMPConfirmUpload').attr("disabled", true);
    } else {
