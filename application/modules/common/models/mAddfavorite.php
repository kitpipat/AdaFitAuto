...
class mAddfavorite extends CI_Model {
    ...
    public function FSaMGetDataMenu($ptMnuRoute){
        ...
        $tSQL       = "
            ...
            LEFT JOIN [TSysMenuList_L] MNUL WITH(NOLOCK)    ON MNU.FTMnuCode    = MNUL.FTMnuCode AND MNUL.FNLngID = ".$this->db->escape($nLangEdit)."
            LEFT JOIN TCNTMenuFavorite FAV WITH(NOLOCK)     ON MNU.FTMnuCode    = FAV.FTMnuCode
            WHERE 1=1
            AND MNU.FTMnuCtlName    = ".$this->db->escape($ptMnuRoute)."
        ";
