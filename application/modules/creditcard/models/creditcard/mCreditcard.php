        ...
        $tSQL   = "SELECT c.* FROM(
                        SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , FTCrdCode DESC) AS FNRowID,* FROM
                            (SELECT DISTINCT
                                ...
                                IMG.FTImgObj,
                                CDC.FDCreateOn,
                                CDC.FTAgnCode AS rtAgnCode,
                                ISB.FTCrdCode AS rtCrdCodeLef
                            FROM [TFNMCreditCard] CDC
                            LEFT JOIN [TFNMCreditCard_L] CDCL ON CDC.FTCrdCode = CDCL.FTCrdCode AND CDCL.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN [TFNMBank_L] BNKL ON CDC.FTBnkCode = BNKL.FTBnkCode AND BNKL.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN [TCNMImgObj] IMG  ON CDC.FTCrdCode = IMG.FTImgRefID AND IMG.FTImgTable = '$tImgTable'
                            LEFT JOIN (SELECT DISTINCT FTCrdCode FROM TFNMInstallmentSub WITH(NOLOCK) ) ISB ON ISB.FTCrdCode  = CDC.FTCrdCode
                            WHERE 1=1 ";

        if($tAgnCode != ''){
            $tSQL .= "AND CDC.FTAgnCode = '".$this->db->escape($tAgnCode)."'";
        }

        @$tSearchList = $paData['tSearchAll'];
        if(@$tSearchList != ''){
            $tSQL .= " AND (CDC.FTCrdCode LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
            $tSQL .= "      OR CDCL.FTCrdName LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
        }
