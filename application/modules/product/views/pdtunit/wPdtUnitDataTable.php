                        ...
                        <?php foreach($aPunDataList['raItems'] AS $nKey => $aValue):?>
                            <tr class="text-center otrPdtUnit" id="otrPdtUnit<?php echo $nKey?>" data-code="<?php echo $aValue['rtPunCode']?>" data-name="<?php echo $aValue['rtPunName']?>">
                                ...
                                <td class="text-center">
                                    <label class="fancy-checkbox">
                                        <input id="ocbListItem<?php echo $nKey?>" data-chkcode="<?php echo $aValue['rtPunCode']?>" type="checkbox" class="ocbListItem" name="ocbListItem[]" <?php if ($aValue['rtFTUsedCode'] > 0) {
                                                                echo "disabled";
                                                            } ?>>
                                ...
                                <?php if($aAlwEventPdtUnit['tAutStaFull'] == 1 || $aAlwEventPdtUnit['tAutStaDelete'] == 1) : ?>
                                <td><img class="xCNIconTable xCNIconDel <?php if ($aValue['rtFTUsedCode'] > 0) {
                                                                                echo "xCNDocDisabled";
                                                                            } ?>" src="<?php echo  base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSoPdtUnitDel('<?=$nCurrentPage?>','<?php echo $aValue['rtPunCode']?>','<?=$aValue['rtPunName']?>','<?= language('common/main/main','tModalConfirmDeleteItemsYN')?>')"></td>
                                <?php endif; ?>
                                <?php if($aAlwEventPdtUnit['tAutStaFull'] == 1 || ($aAlwEventPdtUnit['tAutStaEdit'] == 1 || $aAlwEventPdtUnit['tAutStaRead'] == 1))  : ?>
...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        var result = jQuery.parseJSON(LocalItemData);

        $.each(result, function( index, value ) {
            $(".ocbListItem").each(function (indexInArray, valueOfElement) { 
                 if($(this).data("chkcode") == value.nCode){
                     $(this).prop("checked", true);
                    //  $(this).parent().parent().parent().find('.xCNIconDel').addClass("xCNDocDisabled");
                 }
            });
        });
