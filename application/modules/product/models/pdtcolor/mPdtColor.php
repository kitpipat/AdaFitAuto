        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtClrCode DESC) AS rtRowID,* FROM
                                    ...
                                    LEFT JOIN [TCNMPdtColor_L]  CLR_L ON CLR.FTClrCode = CLR_L.FTClrCode AND CLR_L.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN (SELECT DISTINCT FTClrCode FROM TCNMPdtPackSize WITH(NOLOCK) ) PDT ON PDT.FTClrCode  = CLR.FTClrCode
                                    WHERE 1=1 ";
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL .= " AND (CLR.FTClrCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL .= " OR CLR_L.FTClrName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
