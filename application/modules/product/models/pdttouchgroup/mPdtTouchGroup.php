    ...
    public function FSaMTCGDataTableList($paDataCondition){
        ...
        $tSQL           = " SELECT c.* FROM (
                                ...
                                (SELECT DISTINCT
                                    ...
                                    LEFT JOIN TCNMPdtTouchGrp_L TCGL WITH(NOLOCK) ON TCG.FTTcgCode = TCGL.FTTcgCode AND TCGL.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN TCNMImgPdt IMG ON IMG.FTImgRefID = TCG.FTTcgCode AND IMG.FTImgTable = 'TCNMPdtTouchGrp'
                                    LEFT JOIN TCNMAgency_L AGNL ON TCG.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = ".$this->db->escape($nLngID)."
        ...
        $tSearchAll     = $paDataCondition['tSearchAll'];
        if ($tSearchAll != ''){
            $tSQL .= " AND (TCG.FTTcgCode LIKE '%".$this->db->escape_like_str($tSearchAll)."%'";
            $tSQL .= " OR TCGL.FTTcgName LIKE '%".$this->db->escape_like_str($tSearchAll)."%')";
        }
