        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtLocCode DESC) AS rtRowID,* FROM
                                    ...
                                    FROM [TCNMPdtLoc] LOC
                                    LEFT JOIN [TCNMPdtLoc_L]  LOC_L ON LOC.FTPlcCode = LOC_L.FTPlcCode AND LOC_L.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN (SELECT DISTINCT FTPlcCode FROM TCNTPdtLocSeq WITH(NOLOCK) ) PDT ON PDT.FTPlcCode  = LOC.FTPlcCode
                                    WHERE 1=1 ";
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL .= " AND (LOC.FTPlcCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL .= " OR LOC_L.FTPlcName COLLATE THAI_BIN  LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
