        ...
        try{
            ...
            $tSQL           = "SELECT c.* FROM(
                                    SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPgpCode DESC) AS rtRowID,* FROM
                                        ...
                                        FROM [TCNMPdtGrp] PGP
                                        LEFT JOIN [TCNMPdtGrp_L]  PGP_L ON PGP.FTPgpChain = PGP_L.FTPgpChain AND PGP_L.FNLngID = ".$this->db->escape($nLngID)."
                                        LEFT JOIN TCNMImgPdt TIP ON PGP.FTPgpCode = TIP.FTImgRefID AND TIP.FTImgTable='TCNMPdtGrp'
                                        LEFT JOIN TCNMAgency_L AGNL ON  PGP.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID  = ".$this->db->escape($nLngID)."
                                        --LEFT JOIN TCNMPdt PDT ON  PGP.FTPgpCode = PDT.FTPgpChain
                                        LEFT JOIN (SELECT DISTINCT FTPgpChain FROM TCNMPdt WITH(NOLOCK) ) PDT ON PDT.FTPgpChain  = PGP.FTPgpCode
                                        WHERE 1=1 ";
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                // $tSQL .= " AND (PGP.FTPgpCode LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                // $tSQL .= " OR PGP.FTPgpChain  LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                // $tSQL .= " OR PGP_L.FTPgpName  LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                // $tSQL .= " OR PGP_L.FTPgpChainName  LIKE '%$tSearchList%')";
                $tSQL .= " AND PGP.FTPgpCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'  OR PGP.FTPgpChain COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'
                    OR PGP_L.FTPgpName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%' OR PGP_L.FTPgpChainName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'
                ";
            }
