...
class Pdtcat_model extends CI_Model
{
    ...
    public function FSaMCATList($paData)
    {
        try {
            ...
            if ($tCatCat == 1) {
                $tLeftJoin = 'LEFT JOIN [TCNMPdtCatInfo] CatInfChk
               WITH(NOLOCK) ON CatInfo.FTCatCode = CatInfChk.FTCatParent AND CatInfChk.FNCatLevel = 2';
                $tSelect = 'CatInfChk.FTCatParent As tCatChk,';
            } else if ($tCatCat == 2) {
                $tLeftJoin = 'LEFT JOIN [TCNMPdtCategory] CatGo WITH(NOLOCK) ON CatInfo.FTCatCode = CatGo.FTPdtCat2 ';
                $tSelect = '  CatGo.FTPdtCat2 As tCatChk,';
            } else {
                $tLeftJoin = '';
                $tSelect = '';
            }
            $tSQL = "SELECT DISTINCT
                     ...
                     WITH(NOLOCK) ON CatInfo.FTCatCode = CatInfo_L.FTCatCode AND CatInfo_L.FNLngID = ".$this->db->escape($nLngID)."
                     $tLeftJoin
                     WHERE ($tConditionAgn) AND CatInfo.FNCatLevel = '$tCatCat'";

            if (isset($tSearchList) && !empty($tSearchList)) {
                $tSQL .= " AND (CatInfo_L.FTCatName COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%' OR CatInfo.FTCatCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
