        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPszCode DESC) AS rtRowID,* FROM
                                    ...
                                    LEFT JOIN [TCNMPdtSize_L]  PSZ_L ON PSZ.FTPszCode = PSZ_L.FTPszCode AND PSZ_L.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN (SELECT DISTINCT FTPszCode FROM TCNMPdtPackSize WITH(NOLOCK) ) PDT ON PDT.FTPszCode  = PSZ.FTPszCode
                                    WHERE 1=1 ";
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL .= " AND (PSZ.FTPszCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL .= " OR PSZ_L.FTPszName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
