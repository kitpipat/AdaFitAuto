        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPmoCode DESC) AS rtRowID,* FROM
                                    ...
                                    LEFT JOIN [TCNMPdtModel_L]  PTY_L ON PTY.FTPmoCode = PTY_L.FTPmoCode AND PTY_L.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN [TCNMAgency_L] AGNL ON PTY.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = ".$this->db->escape($nLngID)."
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL .= " AND (PTY.FTPmoCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL .= " OR PTY_L.FTPmoName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
