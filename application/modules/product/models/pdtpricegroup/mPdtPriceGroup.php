        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPplCode DESC) AS rtRowID,* FROM
                                    ...
                                    LEFT JOIN [TCNMPdtPriList_L]  PPL_L ON PPL.FTPplCode = PPL_L.FTPplCode AND PPL_L.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN [TCNMAgency_L] AGNL ON PPL.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = ".$this->db->escape($nLngID)."
                                    LEFT JOIN [TCNMCstLev] CSL ON PPL.FTPplCode = CSL.FTPplCode
                                    WHERE 1=1 ";
            ...
            if(isset($tSearchList) && !empty($tSearchList)){
                $tSQL .= " AND (PPL.FTPplCode COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                $tSQL .= " OR PPL_L.FTPplName  COLLATE THAI_BIN LIKE '%".$this->db->escape_like_str($tSearchList)."% ' ";
                $tSQL .= ")";
            }
