...
class cTopupVending extends MX_Controller
{
    ...
    public function FSaCTUVTopupVendingAddEvent()
    {
        try {
            ...
            $aDataMaster = array(
                ...
                'FTXthStaDelMQ'         => '',
                'FNXthStaDocAct'        => $this->input->post('ocbTopUpVendingXthStaDocAct'), // สถานะ ประมวลผลสต็อค ว่าง หรือ Null:ยังไม่ทำ, 1:ทำแล้ว
    ...
    public function FSxCTUVTopupVendingUpdatePdtLayoutInTmp()
    {
        ...
        $tUserSessionID = $this->session->userdata("tSesSessionID");
        $tUserLoginCode = $this->session->userdata("tSesUsername");
        $nCheckSTK      = $this->input->post('nCheckSTK'); //สถานะเช็คสต็อคว่าตัดหรือไม่ตัด
        ...
        if ($nQty <= $nStkBal) {
            ...
            //ถ้าเช็คว่าไม่ตัดสต็อค สามารถคีย์เท่่าไหร่ก็ได้
            if( $nCheckSTK == 1){
                $nQTYKey = $nQty;
            }else{
