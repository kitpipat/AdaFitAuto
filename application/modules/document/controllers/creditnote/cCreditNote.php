...
class cCreditNote extends MX_Controller
{
    ...
    public function FSaCCreditNoteAddEvent()
    {
        try {
            ...
            if ($this->input->post('oetCreditNoteRefPICode') != '') {
                $aDataWhereDocRefCDN = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $this->input->post('ohdCreditNoteBchCode'),
                    'FTXshDocNo'        => $aDataMaster['FTXphDocNo'],
                    'FTXshRefType'      => 1,
                    'FTXshRefDocNo'     => $this->input->post('oetCreditNoteRefPICode'),
                    'FTXshRefKey'       => 'IV',
                    'FDXshRefDocDate'   => (!empty($this->input->post('oetCreditNoteXphRefIntDate'))) ? date('Y-m-d H:i:s', strtotime($this->input->post('oetCreditNoteXphRefIntDate'))) : NULL
                );
    
                $aDataWhereDocRefIV = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $this->input->post('ohdCreditNoteBchCode'),
                    'FTXshDocNo'        => $this->input->post('oetCreditNoteRefPICode'),
                    'FTXshRefType'      => 2,
            ...
            if ($this->input->post('oetDOSplRefDocExt') != '') {
                $aDataWhereDocRefCDNExt = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $this->input->post('ohdCreditNoteBchCode'),
                    'FTXshDocNo'        => $aDataMaster['FTXphDocNo'],
                    'FTXshRefType'      => 3,
                    'FTXshRefDocNo'     => $this->input->post('oetCreditNoteXphRefExt'),
                    'FTXshRefKey'       => 'CDN',
                    'FDXshRefDocDate'   => (!empty($this->input->post('oetCreditNoteXphRefExtDate'))) ? date('Y-m-d H:i:s', strtotime($this->input->post('oetCreditNoteXphRefExtDate'))) : NULL
                );
            }else{
                $aDataWhereDocRefCDNExt = '';
            }
            ...
            if ($aDataMaster['FNXphDocType'] == '6') { // ใบลดหนี้แบบมีสินค้า
                ...
                //insert Doc ref
                $this->mCreditNote->FSaMCreditNoteAddUpdateRefDocHD($aDataWhereDocRefCDN, $aDataWhereDocRefIV, $aDataWhereDocRefCDNExt);
            }
...
//===============================================================================================
    ...
    public function FSoCCreditNoteCallRefIntDoc(){
        $tBCHCode   = $this->input->post('tBCHCode');
        ...
        $this->load->view('document/creditnote/refintdocument/wCreditNoteRefDoc', $aDataParam);
    }

    // เลขที่เอกสารอ้างอิงมาแสดงในตาราง browse & Search
    public function FSoCCreditNoteCallRefIntDocDataTable(){
    
        $nPage = $this->input->post('nPiRefIntPageCurrent');
        $tPiRefIntBchCode   = $this->input->post('tPiRefIntBchCode');
        $tPiRefIntDocNo   = $this->input->post('tPiRefIntDocNo');
        $tPiRefIntDocDateFrm   = $this->input->post('tPiRefIntDocDateFrm');
        $tPiRefIntDocDateTo   = $this->input->post('tPiRefIntDocDateTo');
        $tPiRefIntStaDoc   = $this->input->post('tPiRefIntStaDoc');
        ...
        $aDataCondition = array(
         ...
         $aDataParam = $this->mCreditNote->FSoMCreditNoteCallRefIntDocDataTable($aDataCondition);
         ...
         $this->load->view('document/creditnote/refintdocument/wCreditNoteRefDocDataTable', $aConfigView);
    }

    // เอารายการจากเอกสารอ้างอิงมาแสดงในตาราง browse
    public function FSoCCreditNoteCallRefIntDocDetailDataTable(){
        ...
        $aDataParam = $this->mCreditNote->FSoMCreditNoteCallRefIntDocDTDataTable($aDataCondition);

        $aConfigView = array(
            'aDataList' => $aDataParam,
            'nOptDecimalShow' => $nOptDecimalShow
          );
        $this->load->view('document/creditnote/refintdocument/wCreditNoteRefDocDetailDataTable', $aConfigView);
    }

    // เอารายการที่เลือกจากเอกสารอ้างอิงภายในลงตาราง temp dt
    public function FSoCCreditNoteCallRefIntDocInsertDTToTemp(){
        $tPiDocNo       =  $this->input->post('tPiDocNo');
        $tPiFrmBchCode  =  $this->input->post('tPiFrmBchCode');
        ...
        $aDataParam = array(
            'tPiDocNo'       => $tPiDocNo,
            'tPiFrmBchCode'  => $tPiFrmBchCode,
            'tRefIntDocNo'   => $tRefIntDocNo,
            'tRefIntBchCode' => $tRefIntBchCode,
            'aSeqNo'         => $aSeqNo,
        );
        
       $aDataResult = $this->mCreditNote->FSoMCreditNoteCallRefIntDocInsertDTToTemp($aDataParam);
       return  $aDataResult;
    }
