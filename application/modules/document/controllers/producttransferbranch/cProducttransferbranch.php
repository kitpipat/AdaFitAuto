...
class cProducttransferbranch extends MX_Controller {
    ...
    public function FSnTBUpdateHD(){
        ...
        $this->mProducttransferbranch->FSaMTBUpdateOrdHD($Data,$DataWhere); /*ลงตาราง TAPTOrdHD */
    }
    ...
    public function FSvCTBXApprove(){
        ...
        $this->mProducttransferbranch->FSvMTBApprove($aDataUpdate); 
        ...
        $this->mProducttransferbranch->FSxMTBXUpdatePdtStkPrcAll($aDataWhere,'1');
        ...
        try{
            ...
            //ส่ง Noti
            $aDataGetDataHD     =   $this->mProducttransferbranch->FSaMGetHDForNoti(array(
                'tDocNo'        => $tXthDocNo,
                'nLngID'        => $this->session->userdata("tLangEdit"),
                'tDocKey'       => 'TCNTPdtTbxHD',
            ));
            $tNotiID = FCNtHNotiGetNotiIDByDocRef($tXthDocNo);
            $aMQParamsNoti = [
                ...
                "params"        => [
                    "oaTCNTNoti" => array(
                        "FNNotID"       => $tNotiID,
                        "FTNotCode"     => '00012',
                        "FTNotKey"      => 'TCNTPdtTbxHD',
                        "FTNotBchRef"   => $tXthBchCode,
                        "FTNotDocRef"   => $tXthDocNo,
                    ),
                    "oaTCNTNoti_L" => array(
                        0 => array(
                            "FNNotID"       => $tNotiID,
                            "FNLngID"       => 1,
                            "FTNotDesc1"    => 'เอกสารใบโอนสินค้าระหว่างสาขา #'.$aDataGetDataHD['raItems'][0]['FTXthDocNo'],
                            "FTNotDesc2"    => 'รหัสสาขา '.$aDataGetDataHD['raItems'][0]['rtBCHCodeFrm'].' อนุมัติใบโอนสินค้าระหว่างสาขา '.$aDataGetDataHD['raItems'][0]['rtBCHCodeTo'],
                        ),
                        1 => array(
                            "FNNotID"       => $tNotiID,
                            "FNLngID"       => 2,
                            "FTNotDesc1"    => 'Transfer Product Branch #'.$aDataGetDataHD['raItems'][0]['FTXthDocNo'],
                            "FTNotDesc2"    => 'Branch code '.$aDataGetDataHD['raItems'][0]['rtBCHCodeFrm'].' Approve document To Branch Code '.$aDataGetDataHD['raItems'][0]['rtBCHCodeTo'],
                        )
                    ),
                    "oaTCNTNotiAct" => array(
                        0 => array(  
                            ...
                            "FTNoaDesc"      => 'รหัสสาขา '.$aDataGetDataHD['raItems'][0]['rtBCHCodeFrm'].' อนุมัติใบโอนสินค้าระหว่างสาขา '.$aDataGetDataHD['raItems'][0]['rtBCHCodeTo'],
                            "FTNoaDocRef"    => $aDataGetDataHD['raItems'][0]['FTXthDocNo'],
                            "FNNoaUrlType"   =>  1,
                            "FTNoaUrlRef"    => 'TBX/2/0',
                        ),
                    ), 
                    "oaTCNTNotiSpc" => array(
                        0 => array(
                            ...
                            "FTAgnCode"    => '',
                            "FTAgnName"    => '',
                            "FTBchCode"    => $aDataGetDataHD['raItems'][0]['rtBCHCode'],
                            "FTBchName"    => $aDataGetDataHD['raItems'][0]['rtBCHName'],
                        ),
                        1 => array(
                            "FNNotID"       => $tNotiID,
                            "FTNotType"    => '2',
                            ...
                            "FTAgnCode"    => '',
                            "FTAgnName"    => '',
                            "FTBchCode"    => $aDataGetDataHD['raItems'][0]['rtBCHCodeFrm'],
                            "FTBchName"    => $aDataGetDataHD['raItems'][0]['rtBCHNameFrm'],
                        ),
                        2 => array(
                            "FNNotID"       => $tNotiID,
                            "FTNotType"    => '2',
                            ...
                            "FTAgnCode"    => '',
                            "FTAgnName"    => '',
                            "FTBchCode"    => $aDataGetDataHD['raItems'][0]['rtBCHCodeTo'],
                            "FTBchName"    => $aDataGetDataHD['raItems'][0]['rtBCHNameTo'],
                        ),
                    ),
                    "ptUser"        => $this->session->userdata('tSesUsername'),
                ]
    ...
    public function FSvCTBXRemovePdtInDTTmp(){
        ...
        $this->mProducttransferbranch->FSnMTBDelDTTmp($aDataWhere);
    }
    ...
    public function FSvCTBXEditPage(){
        ...
        $this->mProducttransferbranch->FSaMTXDeletePDTInTmp();
    ...
    //คำนวนท้ายบิล และ ประกาศค่าคำนวน
    ...
    public function FSxCTBXCheckViaCodeForApv(){
        ...
        $aResult = $this->mProducttransferbranch->FSxMTBXCheckViaCodeForApv($tDocNo);
        if($aResult){
    ...
    //อัพเดทจำนวน
    public function FSvCTBXEditQtyInDT(){
        try {
            $tDocNo          = $this->input->post('tDocNo');
            ...
            $this->mProducttransferbranch->FSaMCLMUpdateInlineDTTemp($aDataUpdateDT, $aDataWhere);
    ...
    // เช็คว่ามีของในคลังพอไหมอนุมัติ (ใบโอนสินค้าระหว่างสาขา)
    public function FSoCTbxEventCheckProductWahouse(){
        try{
            $tDocNo       = $this->input->post('tDocNo');
            $tBchCode     = $this->input->post('tBchCode');
            ...
            $this->mProducttransferbranch->FSxMTBXUpdatePdtStkPrcAll($aDataWhere,'1');
            $aGetPdtInTmpForSendToAPI = $this->mProducttransferbranch->FSaMTBXGetPdtInTmpForSendToAPI($aDataWhere);
            if( FCNnHSizeOf($aGetPdtInTmpForSendToAPI) > 0 ){

                //API CheckSTK
                $aConfig = $this->mProducttransferbranch->FSaMTBXGetConfigAPI();
                if($aConfig['rtCode'] == '800'){
                    $aReturnData = array(
                        'nStaEvent' => 300,
                        'tStaMessg' => 'เกิดข้อผิดพลาด ไม่พบ API ในการเชื่อมต่อ'
                    );
                    $this->mProducttransferbranch->FSxMTBXUpdatePdtStkPrcAll($aDataWhere,'0');
                    echo json_encode($aReturnData);
                    return false;
                }else{
                ...
                if( $aResult['rtCode'] == '001' ){
                    ...
                    for($i=0; $i<$nCountItem; $i++){
                        ...
                        if($aResult['raItems'][$i]['rtStaPrcStock'] == 2 ){ //stock ไม่พอ
                            ...
                            $aFindTextNamePDTNoStock    = $this->mProducttransferbranch->FSxMTBXFindTextNamePDTNoStock("'".$aResult['raItems'][$i]['rtPdtCode']."'");
                    ...
                    if( FCNnHSizeOf($aHaveItemInWah) > 0 ){
                        $tUpdatePdtStkPrc = $this->mProducttransferbranch->FSxMTBXUpdatePdtStkPrc($aDataWhere,$aHaveItemInWah);
                    }
                    $tChkTsysConfig = $this->mProducttransferbranch->FSxMTBXChkConfig($aDataWhere,$aHaveItemInWah);
