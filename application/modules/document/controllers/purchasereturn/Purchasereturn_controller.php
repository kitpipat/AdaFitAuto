...
class Purchasereturn_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('document/purchasereturn/Purchasereturn_model');
        $this->load->model('document/purchasereturn/Purchasereturndischgmodal_model');
    }
    ...
    public function index($nBrowseType, $tBrowseOption)
    {
        ...
        $aData['aAlwEvent'] = FCNaHCheckAlwFunc('docPN/0/0'); // Controle Event
        $aData['vBtnSave'] = FCNaHBtnSaveActiveHTML('docPN/0/0'); // Load Html ของปุ่ม Save ที่เก็บ Session ปัจจุบัน
        // Get Option Show Decimal
        $aData['nOptDecimalShow'] = FCNxHGetOptionDecimalShow();
        $aData['nOptDecimalSave'] = FCNxHGetOptionDecimalSave();

        $this->load->view('document/purchasereturn/wPurchasereturn', $aData);
    }

    // Function : get ร้านค้า ใน สาขา
    public function FSvCPNGetShpByBch()
    {
    ...
    public function FSvCPNGetPdtBarCode()
    {
    ...
    // public function FSaMPNAddTmpToDT($ptXphDocNo = '')
    // {
    //     $aDataWhere = array(
    //         'FTXphDocNo' => $ptXphDocNo,
    //         'FTXthDocKey' => 'TAPTPnHD',
    //     );
    //     $aResInsDT = $this->Purchasereturn_model->FSaMPNInsertTmpToDT($aDataWhere);
    //     if ($aResInsDT['rtCode'] == '1') {
    //         $this->Purchasereturn_model->FSxMClearPdtInTmp();
    //     }
    // }

    // Function : แก้ไข Pdt DT
    public function FSvCPNEditPdtIntoTableDT()
    {
        ...
        // แก้ไขรายการสินค้า
        $this->Purchasereturn_model->FSaMPNUpdateInlineDTTemp($aDataUpdateDT, $aDataWhere);

        if ($tIsDelDTDis == '1') { // ยืนยันการลบ DTDis ส่วนลดรายการนี้
            $this->Purchasereturndischgmodal_model->FSaMPNDeleteDTDisTemp($aDataWhere);
            $this->Purchasereturndischgmodal_model->FSaMPNClearDisChgTxtDTTemp($aDataWhere);
        }

        // Prorat Call
        FCNaHCalculateProrate('TAPTPnHD', $tDocNo);

        $aCalcDTParams = [
            'tDataDocEvnCall' => '1',
            'tDataVatInOrEx' => $tSplVatType,
            'tDataDocNo' => $tDocNo,
            'tDataDocKey' => 'TAPTPnHD',
            'tDataSeqNo' => ''
        ];
    ...
    public function FSvCPNAddPdtIntoTableDT()
    {
        $tUserLevel = $this->session->userdata('tSesUsrLevel');
        ...
        $nPNOptionAddPdt = $this->input->post('tPNOptionAddPdt');
        ...
        if ($tIsByScanBarCode != '1') { // ทำงานเมื่อไม่ใช่การแสกนบาร์โค้ดมา
            ...
            if ($tIsRefPI == '1') { // หากนำเข้าจากการอ้างอิงใบ PI ต้องลบรายการสินค้าเดิมก่อน
                $this->Purchasereturn_model->FSxMClearPdtInTmp();
                $this->Purchasereturn_model->FSxMClearDTDisTmp();
                $this->Purchasereturn_model->FSxMClearHDDisTmp();
            }

            // $nCounts = $this->Purchasereturn_model->FSaMPNGetCountDTTemp($aDataWhere);
            ...
            for ($nI = 0; $nI < FCNnHSizeOf($aPdtData); $nI++) {
                ...
                $nMaxSeqNo = $this->Purchasereturn_model->FSaMPNGetMaxSeqDTTemp($aDataWhere);
                ...
                $aDataPdtParams = array(
                    ...
                    'tDocKey'                   => 'TAPTPnHD',
                    'nQty'                      => $pcQty,
                    'nPNOptionAddPdt'           => $nPNOptionAddPdt,
                    'tVatrate'                  => $tVatrate,
                    'tVatcode'                  => $tVatcode
                );

                $aDataPdtMaster = $this->Purchasereturn_model->FSaMPNGetDataPdt($aDataPdtParams); // Data Master Pdt ข้อมูลรายการสินค้าที่เพิ่มเข้ามา
                if ($aDataPdtMaster['rtCode'] == '1') {
                    $this->Purchasereturn_model->FSaMPNInsertPDTToTemp($aDataPdtMaster, $aDataPdtParams); // นำรายการสินค้าเข้า DT Temp
                }
            }
        }
        ...
        if ($tIsByScanBarCode == '1') {
            ...
            $aPdtData = $this->Purchasereturn_model->FSaMPNGetPunCodeByBarCode($aGetPunCodeByBarCodeParams);
            ...
            if ($aPdtData['rtCode'] == '1') {
                ...
                $aDataPdtMaster = $this->Purchasereturn_model->FSaMPNGetDataPdt($aDataPdtParams); // Data Master Pdt ข้อมูลรายการสินค้าที่เพิ่มเข้ามา
                if ($aDataPdtMaster['rtCode'] == '1') {
                    $nStaInsPdtToTmp = $this->Purchasereturn_model->FSaMPNInsertPDTToTemp($aDataPdtMaster, $aDataPdtParams); // นำรายการสินค้าเข้า DT Temp
                }
                // Loop
            } else {
                $aStatus = array(
    ...
    public function FSvCPNRemovePdtInDTTmp(){
        $aDataWhere = array(
            'tDocNo'        => $this->input->post('tDocNo'),
        ...
        $aResDel = $this->Purchasereturn_model->FSnMPNDelDTTmp($aDataWhere);
    ...
    public function FSxCPNAddPage()
    {
        ...
        // Clear in temp
        $this->Purchasereturn_model->FSxMClearPdtInTmp();
        $this->Purchasereturndischgmodal_model->FSaMCENDeletePDTInTmp($aParams);
        $this->Purchasereturndischgmodal_model->FSaMPNDeleteHDDisTemp($aParams);
        $this->Purchasereturndischgmodal_model->FSaMPNDeleteDTDisTemp($aParams);
        ...
        $aPermission = FCNaHCheckAlwFunc("docPN/0/0");

        $aDataUserLogin = $this->Purchasereturn_model->FStPNGetUsrByCode($aData);
        ...
        $this->load->view('document/purchasereturn/wPurchasereturnAdd', $aDataAdd);
    }

    // Functionality : Event Add Master
    public function FSaCPNAddEvent(){
        try {
            $nOptDecimalShow = FCNxHGetOptionDecimalShow();
            $tDocDate        = $this->input->post('oetPNXphDocDate') . " " . $this->input->post('oetPNDocTime');
            $tUserLevel      = $this->session->userdata('tSesUsrLevel');
            $tLoginDpt       = $this->session->userdata('tSesUsrDptCode');
            $tBchCode        = $this->input->post("oetPNBchCode");
            $tLoginUser      = $this->session->userdata('tSesUsername');
            $tSessionID      = $this->session->userdata('tSesSessionID');
            $tSplVatType     = $this->input->post('ocmPNXphVATInOrEx');
            $tChkTypeRef     = '';
            if($this->input->post('oetPNRefPICode') != ''){
                $tChkTypeRef     = substr($this->input->post('oetPNRefPICode'),0,2);
            }
            ...
            $aCalDTTempForHD = $this->FSaCPNCalDTTempForHD($aCalDTTempParams);
            $aCalInHDDisTemp = $this->Purchasereturn_model->FSaMPNCalInHDDisTemp($aCalDTTempParams);

            $aDataMaster = array(
                'tIsAutoGenCode' => $this->input->post('ocbPNAutoGenCode'), // ต้องการรัน DocNo อัตโนมัติหรือไม่
                'FTBchCode' => $tBchCode, // รหัสสาขาที่สร้างเอกสาร
                'FTXphDocNo' => $this->input->post('oetPNDocNo'), // เลขที่เอกสาร
                'FNXphDocType' => $this->input->post('ohdPNDocType'), // ประเภทเอกสาร (ใบลดหนี้มีสินค้า หรือ ไม่มีสินค้า)
                'FDXphDocDate' => $tDocDate, // วันที่สร้างเอกสาร
                'FTShpCode' => $this->input->post('oetPNShpCode'), // รหัสร้านค้า
                'FTXphCshOrCrd' => $this->input->post('ocmPNXphCshOrCrd'), // ประเภทการชำระเงิน (เงินสด หรือ เชื่อ)
                'FTXphVATInOrEx' => $tSplVatType, // ประเภทภาษี (แยกนอก หรือ รวมใน)
                'FTDptCode' => $tLoginDpt, // รหัสแผนกผู้ที่สร้างเอกสาร
                'FTWahCode' => $this->input->post('oetPNWahCode'), // รหัสคลังสินค้า
                'FTUsrCode' => $tLoginUser, // รหัสผู้สร้างเอกสาร
                'FTXphApvCode' => NULL, // รหัสผู้อนุมัติ
                'FTSplCode' => $this->input->post('oetPNSplCode'), // รหัสผู้จำหน่าย
                'FTXphRefExt' => $this->input->post('oetPNXphRefExt'), // อ้างอิง เลขที่เอกสาร ภายนอก
                'FDXphRefExtDate' => empty($this->input->post('oetPNXphRefExtDate')) ? NULL : $this->input->post('oetPNXphRefExtDate'), // อ้างอิง วันที่เอกสาร ภายนอก
                'FTXphRefInt' => $this->input->post('oetPNRefPICode'), // อ้างอิง เลขที่เอกสาร ภายใน
                'FDXphRefIntDate' => empty($this->input->post('oetPNXphRefIntDate')) ? NULL : $this->input->post('oetPNXphRefIntDate'), // อ้างอิง วันที่เอกสาร ภายใน
                'FTXphRefAE' => NULL,
                'FNXphDocPrint' => NULL,
                'FTRteCode' => NULL,
                'FCXphRteFac' => NULL,
                ...
                'FCXphPaid' => NULL,
                'FCXphLeft' => NULL,
                'FTXphRmk' => $this->input->post('otaPNXphRmk'),
                'FTXphStaRefund' => NULL,
                'FTXphStaDoc' => '1', // สถานะเอกสาร (1:สมบูรณ์, 2:ไม่สมบูรณ์, 3:ยกเลิก)
                'FTXphStaApv' => NULL,
                'FTXphStaPrcStk' => NULL,
                'FTXphStaPaid' => NULL,
                'FNXphStaDocAct' => $this->input->post('ocbPNXphStaDocAct'),
                'FNXphStaRef' => $this->input->post('ocmPNXphStaRef'), // สถานะอ้างอิง (0:ไม่เคยอ้างอิง, 1:อ้างอิงบางส่วน, 2:อ้างอิงหมดแล้ว)
            ...
            $aDataSpl = array(
                ...
                'FTXphDstPaid'          => $this->input->post('ocmPNHDPcSplXphDstPaid'), // 1:ชำระต้นทาง, 2:ชำระปลายทาง
                'FNXphCrTerm'           => $this->input->post('oetPNHDPcSplXphCrTerm'), // ระยะเครดิต
                'FDXphDueDate'          => empty($this->input->post('oetPNHDPcSplXphDueDate')) ? NULL : $this->input->post('oetPNHDPcSplXphDueDate'), // วันที่ครบกำหนด
                'FDXphBillDue'          => empty($this->input->post('oetPNHDPcSplXphBillDue')) ? NULL : $this->input->post('oetPNHDPcSplXphBillDue'), // วันที่จะรับ/วางบิล
                'FTXphCtrName'          => $this->input->post('oetPNHDPcSplXphCtrName'), // ชื่อผู้ตืดต่อ
                'FDXphTnfDate'          => empty($this->input->post('oetPNHDPcSplXphTnfDate')) ? NULL : $this->input->post('oetPNHDPcSplXphTnfDate'), // วันที่ส่งของ
                'FTXphRefTnfID'         => $this->input->post('oetPNHDPcSplXphRefTnfID'), // เลขที่ ใบขนส่ง
                'FTXphRefVehID'         => $this->input->post('oetPNHDPcSplXphRefVehID'), // อ้างอิง เลขที่ ยานพาหนะ ขนส่ง
                'FTXphRefInvNo'         => $this->input->post('oetPNHDPcSplXphRefInvNo'), // เลขที่บัญชีราคาสินค้า
                'FTXphQtyAndTypeUnit'   => $this->input->post('oetPNHDPcSplXphQtyAndTypeUnit'), // จำนวนและลักษณะหีบห่อ
            ...
            if ($aDataMaster['FNXphDocType'] == '6') { // ใบลดหนี้แบบมีสินค้า
                $this->Purchasereturn_model->FSaMPNAddUpdateHDHavePdt($aDataMaster);
                $this->Purchasereturn_model->FSaMPNInsertTmpToDT($aParams);
                $this->Purchasereturn_model->FSaMPNInsertTmpToDTDis($aParams);
                $this->Purchasereturn_model->FSaMPNInsertTmpToHDDis($aParams);
            }

            if ($aDataMaster['FNXphDocType'] == '7') { // ใบลดหนี้แบบไม่มีสินค้า
                $this->Purchasereturn_model->FSaMPNAddUpdateHDNonePdt($aDataMaster);
                $this->Purchasereturn_model->FSaMPNAddUpdateDTNonePdt($aDataDT);
            }

            $this->Purchasereturn_model->FSaMPNAddUpdatePCHDSpl($aDataSpl);
            ...
            $aTableAddUpdate = array(
                ...
                'tTableRefPN' => 'TAPTPnHDDocRef',
                'tTableRefPI' => 'TAPTPiHDDocRef'
            );

            if ($aDataMaster['FTXphRefInt'] != '') {
                $aDataWhereDocRefPN = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $aDataMaster['FTBchCode'],
                    'FTXshDocNo'        => $aDataMaster['FTXphDocNo'],
                    'FTXshRefType'      => 1,
                    'FTXshRefDocNo'     => $aDataMaster['FTXphRefInt'],
                    'FTXshRefKey'       => 'PN',
                    'FDXshRefDocDate'   => (!empty($aDataMaster['FDXphRefIntDate'])) ? date('Y-m-d H:i:s', strtotime($aDataMaster['FDXphRefIntDate'])) : NULL
                );
    
                $aDataWhereDocRefOther = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $aDataMaster['FTBchCode'],
                    'FTXshDocNo'        => $aDataMaster['FTXphRefInt'],
                    'FTXshRefType'      => 2,
                    'FTXshRefDocNo'     => $aDataMaster['FTXphDocNo'],
                    'FTXshRefKey'       => 'DO',
                    'FDXshRefDocDate'   => (!empty($aDataMaster['FDXphRefIntDate'])) ? date('Y-m-d H:i:s', strtotime($aDataMaster['FDXphRefIntDate'])) : NULL
                );
                if($tChkTypeRef == 'IV'){
                    $aTableAddUpdate['tTableRefOther']      = 'TAPTPiHDDocRef';
                    $aDataWhereDocRefOther['FTXshRefKey']   = 'PN';
                    $aDataWhereDocRefPN['FTXshRefKey']      = 'PI';
                }else{
                    $aTableAddUpdate['tTableRefOther']      = 'TAPTDoHDDocRef';
                    $aDataWhereDocRefOther['FTXshRefKey']   = 'PN';
                    $aDataWhereDocRefPN['FTXshRefKey']      = 'DO';
                }

            }else{
                $aDataWhereDocRefPN = '';
                $aDataWhereDocRefOther = '';
                $aTableAddUpdate['tTableRefOther']      = '';
            }

            if ($aDataMaster['FTXphRefExt'] != '') {
                $aDataWhereDocRefDOExt = array(
                    'FTAgnCode'         => '',
                    'FTBchCode'         => $aDataMaster['FTBchCode'],
                    'FTXshDocNo'        => $aDataMaster['FTXphDocNo'],
                    'FTXshRefType'      => 3,
                    'FTXshRefDocNo'     => $aDataMaster['FTXphRefExt'],
                    'FTXshRefKey'       => 'PN',
                    'FDXshRefDocDate'   => (!empty($aDataMaster['FDXphRefExtDate'])) ? date('Y-m-d H:i:s', strtotime($aDataMaster['FDXphRefExtDate'])) : NULL
                );
            }else{
                $aDataWhereDocRefDOExt = '';
            }

            $this->Purchasereturn_model->FSaMPNaAddUpdateRefDocHD($aDataWhere, $aTableAddUpdate, $aDataWhereDocRefPN, $aDataWhereDocRefOther, $aDataWhereDocRefDOExt);

            // $this->Purchasereturn_model->FSaMPNAddUpdateDocNoInDocTemp($aDataWhere); // Update DocNo ในตาราง Doctemp
            // $this->Purchasereturn_model->FSaMPNInsertTmpToDT($aDataWhere); // Insert DT Temp to DT
            // $this->Purchasereturn_model->FSxMClearPdtInTmp();
            // $this->Purchasereturn_model->FSxMClearDTDisTmp();
            // $this->Purchasereturn_model->FSxMClearHDDisTmp();
    ...
    private function FSaCPNCalDTTempForHD($paParams){
        $aCalDTTemp = $this->Purchasereturn_model->FSaMPNCalInDTTemp($paParams);
        if (!empty($aCalDTTemp)) {
    ...
    private function FSaCPNCalRound($paParams)
    {
    ...
    public function FSvCPNEditPage()
    {
        ...
        $aDataUserLogin = $this->Purchasereturn_model->FStPNGetUsrByCode($aData); // Get ข้อมูลสาขา และร้านค้าของ User ที่ login
        $aPNHD = $this->Purchasereturn_model->FSaMPNGetHD($aData); // Data TAPTPnHD

        $aHDSplParams = [
            'tDocNo' => $aPNHD['raItems']['FTXphDocNo']
        ];
        $aHDSpl = $this->Purchasereturn_model->FSaMPNGetHDSpl($aHDSplParams); // ข้อมูลผู้จำหน่าย TAPTPnHDSpl

        $aSplParams = [
            'tSplCode' => $aPNHD['raItems']['FTSplCode']
        ];
        $aSpl = $this->Purchasereturn_model->FSaMPNGetSplVatCode($aSplParams); // ข้อมูลผู้จำหน่าย TCNMSpl

        $aData['FTUsrCode'] = $aPNHD['raItems']['FTUsrCode'];
        $aUserCreated = $this->mUser->FSaMUSRByID($aData);

        $aData['FTUsrCode'] = $aPNHD['raItems']['FTXphApvCode'];
        $aUserApv = $this->mUser->FSaMUSRByID($aData);

        $aData['FTBchCode'] = $aPNHD['raItems']['FTBchCode'];

        $aData['nRow'] = 10000;
        $aData['nPage'] = 1;
        $aData['FTXthDocKey'] = 'TAPTPnHD';

        // Get Data
        if ($aPNHD['raItems']['FNXphDocType'] == '6') { // ใบลดหนี้มีสินค้า
            $aInsertTmpParams = [
                'tDocNo' => $tDocNo,
                'tBchCode' => $tBchCode,
                'tSessionID' => $tSessionID,
            ...
            $this->Purchasereturn_model->FSaMPNInsertDTToTemp($aInsertTmpParams);
            $this->Purchasereturn_model->FSaMPNInsertDTDisToTmp($aInsertTmpParams);
            $this->Purchasereturn_model->FSaMPNInsertHDDisToTmp($aInsertTmpParams);
        }
        ...
        $this->load->view('document/purchasereturn/wPurchasereturnAdd', $aDataEdit);
    }

    // Functionality : Event Edit Master
    public function FSaCPNEditEvent()
    {
        try {
            // Get Option Show Decimal  
            $nOptDecimalShow = FCNxHGetOptionDecimalShow();

            $tDocNo = $this->input->post('oetPNDocNo');
            $tDocType = $this->input->post('ohdPNDocType');
            $tDocDate = $this->input->post('oetPNXphDocDate') . " " . $this->input->post('oetPNDocTime');
            $tUserLevel = $this->session->userdata('tSesUsrLevel');
            $tLoginDpt = $this->session->userdata('tSesUsrDptCode');
            $tBchCode = $this->input->post("oetPNBchCode"); // ทำรายการได้เฉพาะในสาขาที่เข้าใช้งานเท่านั้น
            $tLoginUser = $this->session->userdata('tSesUsername');
            $tSessionID = $this->session->userdata('tSesSessionID');
            $tSplVatType = $this->input->post('ocmPNXphVATInOrEx');
            $tChkTypeRef     = '';
    ...
    public function FSxCPNDataTable()
    {
        ...
        // Controle Event
        $aAlwEvent = FCNaHCheckAlwFunc('docPN/0/0');
        ...
        $aResList = $this->Purchasereturn_model->FSaMPNList($aData);
        ...
        $this->load->view('document/purchasereturn/wPurchasereturnDataTable', $aGenTable);
    }

    // Function : Adv Table Load Data
    public function FSvCPNPdtAdvTblLoadData()
    {
        ...
        // คำนวน DT ใหม่
        // $aResCalDTTmp = $this->FSnCPNCalulateDTTemp($tXphDocNo, $tXthVATInOrEx);
        ...
        // Get Option Show Decimal
        $nOptDecimalShow = FCNxHGetOptionDecimalShow();
        $aColumnShow = FCNaDCLGetColumnShow('TAPTPnDT');
        ...
        if ($tCalculateAgain == 'CHANGE') {
            ...
            if ($aStaCalcDTTemp === TRUE) {
                FCNaHCalculateProrate('TAPTPnHD', $aPackDataCalCulate['tDocNo']);
                FCNbHCallCalcDocDTTemp($aCalcDTParams);
            }
            $aEndOfBill['aEndOfBillVat']    = FCNaDOCEndOfBillCalVat($aEndOfBillParams);
            $aEndOfBill['aEndOfBillCal']    = FCNaDOCEndOfBillCal($aEndOfBillParams);
        ...
        $aDataDT = $this->Purchasereturn_model->FSaMPNGetDTTempListPage($aDataWhere);
        $aDataDTSum = $this->Purchasereturn_model->FSaMPNSumDTTemp($aDataWhere);
        ...
        $aData['nPage']             = $nPage;
        $tTableHtml = $this->load->view('document/purchasereturn/advancetable/wPurchasereturnPdtAdvTableData', $aData, true);
    ...
    // Function : Adv Table Load Data
    public function FSvCPNNonePdtAdvTblLoadData()
    {
        $tSearchAll     = $this->input->post('tSearchAll');
        $tDocNo         = $this->input->post('tDocNo');
        $tSplVatType    = $this->input->post('tSplVatType');
        ...
        $aColumnShow = FCNaDCLGetColumnShow('TAPTPnDT');
        $aTSysPdtParams = [
            'tPdtCode'      => 'Purchase Return',
            'tPdtSysTable'  => 'TAPTPnHD',
            'nLngID'        => FCNaHGetLangEdit()
        ];
        $aDataDT        = FCNoHDOCGetTSysPdt($aTSysPdtParams); // $this->Purchasereturn_model->FSaMPNGetDTTempListPage($aDataWhere);
        $aDataDTSum     = $this->Purchasereturn_model->FSaMPNSumDTTemp($aDataWhere);
        ...
        $aDataDTNonePdt             = $this->Purchasereturn_model->FSaMPNGetDTNonePdt($aDTNontPdtParams); // Data TAPTPnDT
        $aData['aDataDTNonePdt']    = $aDataDTNonePdt;

        $tTableHtml = $this->load->view('document/purchasereturn/advancetable/wPurchasereturnNonePdtAdvTableData', $aData, true);
    ...
    public function FSvCPNShowColSave()
    {

        FCNaDCLSetShowCol('TAPTPnDT', '', '');
        ...
        if ($nStaSetDef == 1) {
            FCNaDCLSetDefShowCol('TAPTPnDT');
        } else {
            for ($i = 0; $i < FCNnHSizeOf($aColShowSet); $i++) {
                FCNaDCLSetShowCol('TAPTPnDT', 1, $aColShowSet[$i]);
            }
        }

        // Reset Seq
        FCNaDCLUpdateSeq('TAPTPnDT', '', '', '');
        $q = 1;
        for ($n = 0; $n < FCNnHSizeOf($aColShowAllList); $n++) {
            FCNaDCLUpdateSeq('TAPTPnDT', $aColShowAllList[$n], $q, $aColumnLabelName[$n]);
            $q++;
        }
    }

    // Function : Adv Table Show
    public function FSvCPNAdvTblShowColList()
    {

        $aAvailableColumn = FCNaDCLAvailableColumn('TAPTPnDT');
        $aData['aAvailableColumn'] = $aAvailableColumn;
        $this->load->view('document/purchasereturn/advancetable/wPurchaseTableShowColList', $aData);
    }

    // Function : ค้นหา รายการ
    public function FSxCPNFormSearchList()
    {
        ...
        $this->load->view('document/purchasereturn/wPurchasereturnFormSearchList', $aDataMaster);
    }
     ...
     * Return Type : String
     */
    public function FSvCPNPdtMultiDeleteEvent()
    {
        $tDocNo = $this->input->post('tDocNo');
        $tPdtCode  = $this->input->post('tPdtCode');
        ...
        if ($nCount > 1) {
            ...
            for ($i = 0; $i < $nCount; $i++) {
                ...
                $aResDel = $this->Purchasereturn_model->FSaMPNPdtTmpMultiDel($aDataMaster);
            }
        } else {
     ...
     * Parameters : $tSelect "docPNCode"
     * Creator : 28/05/2019 piya
     * Last Modified : -
     * Return : Check status true or false
     * Return Type : String
     */
    public function FSvCPNApprove()
    {
        ...
        $tBchCode = $this->input->post('tBchCode');
        $tDocRefType     = $this->input->post('tChkTypeDocRef');
        ...
        if ($tDocType == '6') { // ใบลดหนี้มีสินค้า
            ...
            $aStaApv = $this->Purchasereturn_model->FSaMPNHavePdtApprove($aDataUpdate);
        ...
        if ($tDocType == '7') { // ใบลดหนี้ไม่มีสินค้า
            $aStaApv = $this->Purchasereturn_model->FSaMPNNonePdtApprove($aDataUpdate);
        }
    }
    ...
    public function FSvCPNCancel()
    {
        ...
        $aStaApv = $this->Purchasereturn_model->FSaMPNCancel($aDataUpdate);
    ...
    /**
     * Functionality : Doc Purchase Return code unique check
     * Parameters : $tSelect "docPNCode"
     * Creator : 28/05/2019 piya
     * Last Modified : -
     * Return : Check status true or false
     * Return Type : String
     */
    public function FStCPNUniqueValidate($tSelect = '')
    {

        if ($this->input->is_ajax_request()) { // Request check
            if ($tSelect == 'docPNCode') {

                $tPNDocCode = $this->input->post('tPNCode');
                $oCPNDoc = $this->Purchasereturn_model->FSnMPNCheckDuplicate($tPNDocCode);

                $tStatus = 'false';
                if ($oCPNDoc[0]->counts > 0) { // If have record
                    $tStatus = 'true';
                }
                echo $tStatus;
            }
     ...
     * Return Type : String
     */
    public function FSoCPNCalEndOfBillNonePdt()
    {
        // Get Option Show Decimal  
        $nOptDecimalShow = FCNxHGetOptionDecimalShow();
     ...
     * Return Type : object
     */
    public function FSoCPNDeleteMultiDoc()
    {
        ...
        foreach ($aDocNo as $aItem) {
            ...
            $this->Purchasereturn_model->FSaMPNDelMaster($aDelMasterParams);
        }
     ...
     * Return Type : Object
     */
    public function FSoCPNDeleteDoc()
    {
     ...
     * Return Type : Array
     */
    public function FSaCPNClearTemp()
    {
     ...
     public function FSoCPNChangeSPLAffectNewVAT(){
        ...
        $this->Purchasereturn_model->FSaMCNChangeSPLAffectNewVAT($aItem);
    }



    
    public function FSoCPNCallRefIntDoc(){
        $tBCHCode   = $this->input->post('tBCHCode');
        $tBCHName   = $this->input->post('tBCHName');
        $tRefType   = $this->input->post('tPNTypeRef');
        $aDataParam = array(
            'tBCHCode' => $tBCHCode,
            'tBCHName' => $tBCHName,
            'tRefType' => $tRefType
        );
        $this->load->view('document/purchasereturn/refintdocument/wPurchasereturnRefDoc', $aDataParam);
    }
     ...
     * ReturnType: Object
     */
    public function FSoCPNCallRefIntDocDataTable(){
    
        $nPage = $this->input->post('nPNRefIntPageCurrent');
        $tPNRefIntBchCode   = $this->input->post('tPNRefIntBchCode');
        $tPNRefIntDocNo   = $this->input->post('tPNRefIntDocNo');
        $tPNRefIntDocDateFrm   = $this->input->post('tPNRefIntDocDateFrm');
        $tPNRefIntDocDateTo   = $this->input->post('tPNRefIntDocDateTo');
        $tPNRefIntStaDoc   = $this->input->post('tPNRefIntStaDoc');
        $tPNTypeRef            = $this->input->post('tPNTypeRef');
        ...
        $aDataCondition = array(
         ...
         $aDataParam = $this->Purchasereturn_model->FSoMPNCallRefIntDocDataTable($aDataCondition);
         ...
         $this->load->view('document/purchasereturn/refintdocument/wPurchasereturnRefDocDataTable', $aConfigView);
    }
     ...
     * ReturnType: Object
     */
    public function FSoCPNCallRefIntDocDetailDataTable(){
        ...
        $aDataParam = $this->Purchasereturn_model->FSoMPNCallRefIntDocDTDataTable($aDataCondition);
        ...
        $this->load->view('document/purchasereturn/refintdocument/wPurchasereturnRefDocDetailDataTable', $aConfigView);
    }
     ...
     * ReturnType: Object
     */
    public function FSoCPNCallRefIntDocInsertDTToTemp(){
        $tPNDocNo               =  $this->input->post('tPNDocNo');
        $tPNFrmBchCode          =  $this->input->post('tPNFrmBchCode');
        ...
        $aDataParam = array(
            'tPNDocNo'          => $tPNDocNo,
            'tPNFrmBchCode'     => $tPNFrmBchCode,
            'tRefIntDocNo'      => $tRefIntDocNo,
            'tRefIntBchCode'    => $tRefIntBchCode,
            'aSeqNo'            => $aSeqNo,
            'nCheckRefBrowse'   => $nCheckRefBrowse,
        );
       $aDataResult = $this->Purchasereturn_model->FSoMPNCallRefIntDocInsertDTToTemp($aDataParam);
       return  $aDataResult;
    }
