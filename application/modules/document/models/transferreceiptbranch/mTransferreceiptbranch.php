...
class mTransferreceiptbranch extends CI_Model{
    ...
    public function FSaMTBIList($paData){
        ...
        $tSQL       = "SELECT TOP ". get_cookie('nShowRecordInPageList')." c.* FROM(
                        ...
                        FROM (
                            ...
                            LEFT JOIN TCNMBranch_L  BCHL WITH (NOLOCK) ON TBI.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN TCNMUser_L    USRL WITH (NOLOCK) ON TBI.FTCreateBy = USRL.FTUsrCode AND USRL.FNLngID = ".$this->db->escape($nLngID)."
                            LEFT JOIN TCNMUser_L    USRLAPV WITH (NOLOCK) ON TBI.FTXthApvCode = USRLAPV.FTUsrCode AND USRLAPV.FNLngID = ".$this->db->escape($nLngID)."
        ...
        if (@$tSearchList != '') {
            $tSQL .= "      AND (
                                (TBI.FTXthDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%') OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%') 
                                OR 
                                (CONVERT(CHAR(10),TBI.FDXthDocDate,103) LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                            )";
        }
        ...
        if (!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeFrom)) {
            $tSQL .= " AND ((TBI.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeFrom)." AND ".$this->db->escape($tSearchBchCodeTo).") 
                        OR (TBI.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeTo)." AND ".$this->db->escape($tSearchBchCodeFrom)."))";
        }
        ...
        if (!empty($tSearchDocDateFrom) && !empty($tSearchDocDateTo)) {
            $tSQL .= "  AND (
                                (TBI.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00").")   AND CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:59:59").")) 
                                OR 
                                (TBI.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 00:00:00").")  AND CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 23:59:59")."))
                            )";
        }
    ...
    public function FSnMTBIGetPageAll($paData){
        ...
        @$tSearchList   = $aAdvanceSearch['tSearchAll'];
        if (@$tSearchList != '') {
            $tSQL .= " AND ((TBI.FTXthDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%') OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%') OR (TBI.FDXthDocDate LIKE '%".$this->db->escape_like_str($tSearchList)."%'))";
        }
    ...
    public function FSxMTBIClearPdtInTmp($ptTblSelectData){
        ...
        $this->db->delete('TSVTTBIDocDTTmp');
        unset($tXthDocKey);
        unset($tSessionID);
    }
    ...
    public function FSaMTBIGetDocDTTempListPage($paDataWhere){
        ...
        $tSQL   = " 
            SELECT
                ...
                DOCTMP.FCXtdAmt,
                DOCTMP.FTXtdRmk
            FROM TSVTTBIDocDTTmp DOCTMP WITH (NOLOCK)
    ...
    public function FSaMTBIGetPriceBYPDT($ptCode){
        ...
        $tSQL .= " ISNULL(PSPL.FCSplLastPrice * ISNULL(P.FCPdtUnitFact,1),NULL) as PDTCostLastPrice,
                ...
                LEFT JOIN TCNMPdtPackSize PPS ON PDT.FTPdtCode = PPS.FTPdtCode AND BAR.FTPunCode = PPS.FTPunCode 
    ...
    public function FSaMTBIInsertPDTToTemp($paDataPdtMaster, $paDataPdtParams){
        ...
        $tSQL   =   "   SELECT
                            ...
                            FCXtdQty,
                            FCXtdFactor
                        FROM TSVTTBIDocDTTmp WITH (NOLOCK)
        ...
        if ($oQuery->num_rows() > 0) {
            ...
            $this->db->insert('TSVTTBIDocDTTmp', $aDataInsert);
    ...
    public function FSnMTBIDelPdtInDTTmp($paDataWhere){
        ...
        $this->db->update('TSVTTBIDocDTTmp');
        return;
    }
    ...
    public function FSaMTBICalInDTTemp($paParams){
        ...
        $tSQL       = " SELECT
                            ...
                            STUFF((
                                ...
                                FROM TSVTTBIDocDTTmp DOCCONCAT
                                WHERE  1=1 
    ...
    public function FSaMTBICalInHDDisTemp($paParams){
        ...
        $tSQL       = " SELECT
                            ...
                            STUFF((
                                ...
                                FROM TCNTDocHDDisTmp DOCCONCAT WITH (NOLOCK)
                                WHERE  1=1 
                                AND DOCCONCAT.FTBchCode 		= '$tBchCode'
                                AND DOCCONCAT.FTXthDocNo		= '$tDocNo'
                                AND DOCCONCAT.FTSessionID		= '$tSessionID'
    ...
    public function FSaMTBIGetDataDocHD($paDataWhere){
        ...
        $tSQL       = " 
            SELECT
                CASE WHEN DOCHD.FTXthRefInt <> '' OR DOCHD.FTXthRefInt IS NOT NULL THEN 2 ELSE 1 END FTXshRefType,
                BCHL.FTBchName,
                SHP.FTMerCode,
                MERL.FTMerName,
                SHP.FTShpType,
            ...
            LEFT JOIN TCNMShipVia_L     SHV_L   WITH (NOLOCK)  ON HDREF.FTViaCode      = SHV_L.FTViaCode   AND SHV_L.FNLngID        = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMBranch_L      BCHL    WITH (NOLOCK)   ON DOCHD.FTBchCode     = BCHL.FTBchCode    AND BCHL.FNLngID         = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMBranch_L      BCHL_F  WITH (NOLOCK)  ON DOCHD.FTXthBchFrm    = BCHL_F.FTBchCode  AND BCHL_F.FNLngID       = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMBranch_L      BCHL_T  WITH (NOLOCK)  ON DOCHD.FTXthBchTo     = BCHL_T.FTBchCode  AND BCHL_T.FNLngID       = ".$this->db->escape($nLngID)."
            ...
            LEFT JOIN TCNMWaHouse_L     WAHL_F  WITH (NOLOCK)   ON DOCHD.FTXthWhFrm     = WAHL_F.FTWahCode	AND DOCHD.FTXthBchFrm   = WAHL_F.FTBchCode AND WAHL_F.FNLngID   = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMWaHouse_L     WAHL_T  WITH (NOLOCK)   ON DOCHD.FTXthWhTo      = WAHL_T.FTWahCode	AND DOCHD.FTXthBchTo    = WAHL_T.FTBchCode AND WAHL_T.FNLngID   = ".$this->db->escape($nLngID)."
            ...
            WHERE DOCHD.FDCreateOn <> '' AND DOCHD.FTXthDocNo = ".$this->db->escape($tTBIDocNo)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $aDetail    = $oQuery->row_array();
    ...
    public function FSxMTBIAddUpdateDocNoToTemp($paDataWhere, $paTableAddUpdate){
        ...
        $this->db->update('TSVTTBIDocDTTmp', array(
            'FTXthDocNo'    => $paDataWhere['FTXthDocNo'],
            'FTBchCode'     => $paDataWhere['FTBchCode']
        ));
    ...
    public function FSaMTBIMoveDtTmpToDt($paDataWhere, $paTableAddUpdate){
        ...
        $tSQL   .=  "   SELECT
                            ...
                            DOCTMP.FTCreateBy
                        FROM TSVTTBIDocDTTmp DOCTMP WITH (NOLOCK)
                        WHERE DOCTMP.FTBchCode    = '$tTBIBchCode'
        ...
        $tSQLUpdateCN     =  "SELECT DOCTMP.FTXtdDocNoRef , DOCTMP.FTBchCode,DOCTMP.FTXthDocNo,DOCTMP.FTPdtCode,DOCTMP.FTCabNameForTWXVD
                        FROM TSVTTBIDocDTTmp DOCTMP WITH (NOLOCK) WHERE DOCTMP.FTBchCode    = '$tTBIBchCode'
                        AND DOCTMP.FTXthDocNo   = '$tTBIDocNo'
                        AND DOCTMP.FTXthDocKey  = '$tTBIDocKey'
                        AND DOCTMP.FTSessionID  = '$tTBISessionID'
    ...
    public function FSxMTBIMoveDTToDTTemp($paDataWhere){
        ...
        $tSQL   = " INSERT INTO TSVTTBIDocDTTmp (
    ...
    public function FSaMTBIUpdatePDTInCN($ptPackData){
        ...
        $tSQL = " SELECT 
                    ...
                    DT.FTPdtCode ,
                    SODT.FNXsdSeqNo
                FROM TSVTTBIDocDTTmp DT WITH (NOLOCK)
