...
class mTaxinvoice extends CI_Model{
    ...
    public function FSaMTAXGetListABB($paData){
        ...
        $tSQL       = "SELECT 			
                        ...
                        FROM ( SELECT c.* FROM( SELECT  ROW_NUMBER() OVER(ORDER BY FTBchCode , FDCreateOn DESC) AS FNRowID,* FROM ( ";
        $tSQLMain   .= "SELECT 
                                TaxHD.FTXshDocNo,
                                TaxHD.FNXshDocType,
                                TaxHD.FTBchCode,
        ...
        if( !empty($tTXIBchCode) ){
            $tSQLMain .= " AND TaxHD.FTBchCode = '$tTXIBchCode' ";
        }
        ...
        if($this->session->userdata("tSesUsrLevel") == 'BCH' || $this->session->userdata("tSesUsrLevel") == 'SHP'){
            ...
            $tSQLMain .= " AND  TaxHD.FTBchCode IN ($tBCH) ";
        }

        $tTXIDocType = $paData['aDataSearch']['tTXIDocType'];
        if( !empty($tTXIDocType) ){
            if($tTXIDocType == '5'){
                $tSQLMain .= " AND TaxHD.FNXshDocType IN ('5','11') ";
            }else{
                $tSQLMain .= " AND TaxHD.FNXshDocType = '$tTXIDocType' ";
            }
        }

        $tTXIDocNo = $paData['aDataSearch']['tTXIDocNo'];
        if( !empty($tTXIDocNo) ){
            $tSQLMain .= " AND ( TaxHD.FTXshDocNo = '$tTXIDocNo' /*OR SalHD.FTXshRefExt = '$tTXIDocNo'*/ ) ";
        }
        ...
        $tSQL .= " ) AS A ";
        $tSQL .= " LEFT JOIN TPSTTaxHDDocRef HDDocRef_in WITH (NOLOCK) ON A.FTXshDocNo = HDDocRef_in.FTXshDocNo AND HDDocRef_in.FTXshRefType = 1 ";
        $tSQL .= " ORDER BY A.FNRowID ASC " ;

        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            $oList      = $oQuery->result();
            $aFoundRow  = $this->FSnMTAXGetListABBPageAll($tSQLMain);
            $nFoundRow  = $aFoundRow;
    ...
    public function FSnMTAXGetListABBPageAll($tSQLMain){
    ...
    public function FSaMTAXGetHDTax($ptData){
        ...
        $tSQL   = " SELECT 
                    ...
                    LEFT JOIN TPSTTaxHDDocRef DOCREF WITH(NOLOCK) ON Tax.FTXshDocNo = DOCREF.FTXshDocNo
