...
class mCreditNote extends CI_Model {
    ...
    public function FSoMCreditNoteCallRefIntDocDataTable($paDataCondition){
        $aRowLen                = FCNaHCallLenData($paDataCondition['nRow'],$paDataCondition['nPage']);
        $nLngID                 = $paDataCondition['FNLngID'];
        $aAdvanceSearch         = $paDataCondition['aAdvanceSearch'];
        // Advance Search
        $tPiRefIntBchCode        = $aAdvanceSearch['tPiRefIntBchCode'];
        $tPiRefIntDocNo          = $aAdvanceSearch['tPiRefIntDocNo'];
        $tPiRefIntDocDateFrm     = $aAdvanceSearch['tPiRefIntDocDateFrm'];
        $tPiRefIntDocDateTo      = $aAdvanceSearch['tPiRefIntDocDateTo'];
        $tPiRefIntStaDoc         = $aAdvanceSearch['tPiRefIntStaDoc'];
        ...
        $tSQLMain = "   SELECT
                            ...
                            LEFT JOIN TCNMSpl_L     SPL_L   WITH (NOLOCK) ON PIHD.FTSplCode     = SPL_L.FTSplCode   AND SPL_L.FNLngID    = $nLngID
                            LEFT JOIN TCNMWaHouse_L WAH_L   WITH (NOLOCK) ON PIHD.FTBchCode     = WAH_L.FTBchCode   AND PIHD.FTWahCode = WAH_L.FTWahCode AND WAH_L.FNLngID	= $nLngID
                            ...
                            WHERE PIHD.FNXphStaRef != 2 AND PIHD.FTXphStaDoc = 1 AND PIHD.FTXphStaApv = 1 AND ISNULL(PIHDREF.FTXshRefType, '') = ''
                    ";
        $tSQLSta = "";

        if(isset($tPiRefIntBchCode) && !empty($tPiRefIntBchCode)){
            $tSQLSta .= " AND (c.FTBchCode = '$tPiRefIntBchCode')";
        }

        if(isset($tPiRefIntDocNo) && !empty($tPiRefIntDocNo)){
            $tSQLSta .= " AND (c.FTXphDocNo LIKE '%$tPiRefIntDocNo%')";
        }

        // ค้นหาจากวันที่ - ถึงวันที่
        if(!empty($tPiRefIntDocDateFrm) && !empty($tPiRefIntDocDateTo)){
            $tSQLSta .= " AND ((c.FDXphDocDate BETWEEN CONVERT(datetime,'$tPiRefIntDocDateFrm 00:00:00') AND CONVERT(datetime,'$tPiRefIntDocDateTo 23:59:59')) OR (c.FDXphDocDate BETWEEN CONVERT(datetime,'$tPiRefIntDocDateTo 23:00:00') AND CONVERT(datetime,'$tPiRefIntDocDateFrm 00:00:00')))";
        }

        // ค้นหาสถานะเอกสาร
        
        if(isset($tPiRefIntStaDoc) && !empty($tPiRefIntStaDoc)){
            if ($tPiRefIntStaDoc == 3) {
                $tSQLSta .= " AND c.FTXphStaDoc = '$tPiRefIntStaDoc'";
            } elseif ($tPiRefIntStaDoc == 2) {
                $tSQLSta .= " AND ISNULL(c.FTXphStaApv,'') = '' AND c.FTXphStaDoc != '3'";
            } elseif ($tPiRefIntStaDoc == 1) {
                $tSQLSta .= " AND c.FTXphStaApv = '$tPiRefIntStaDoc'";
            }
        }
        ...
        $tSQL   =   "       SELECT c.* FROM(
                                ...
                                (  $tSQLMain
                                ) Base) AS c WHERE c.FNRowID > $aRowLen[0] AND c.FNRowID <= $aRowLen[1] AND c.FNXphDocType = 12 $tSQLSta
        ";
    ...
    public function FSoMCreditNoteCallRefIntDocDTDataTable($paData){
    ...
    public function FSoMCreditNoteCallRefIntDocInsertDTToTemp($paData){

        $tPcDocNo        = $paData['tPiDocNo'];
        $tPiFrmBchCode   = $paData['tPiFrmBchCode'];

        // Delect Document DTTemp By Doc No
        $this->db->where('FTBchCode',$tPiFrmBchCode);
        $this->db->where('FTXthDocNo',$tPcDocNo);
        $this->db->delete('TCNTDocDTTmp');
       ...
       $tSQL= "  
                ...
                SELECT
                    '$tPiFrmBchCode' as FTBchCode,
                    '$tPcDocNo' as FTXphDocNo,
                    DT.FNXpdSeqNo,
                    DT.FTPdtCode,
                    DT.FTXpdPdtName,
                    DT.FTPunCode,
        ...
        $tSQL .= "  
            ...
            WHERE DTDIS.FTXphDocNo = '$tRefIntDocNo'  AND DTDIS.FNXpdSeqNo IN $aSeqNo
            ORDER BY DTDIS.FNXpdSeqNo ASC";
        ...
        $tSQL .= "  
            ...
            WHERE HDDIS.FTXphDocNo = '$tRefIntDocNo'
            ORDER BY HDDIS.FDXphDateIns ASC";
    ...
    public function FSaMCreditNoteAddUpdateRefDocHD($aDataWhereDocRefCDN, $aDataWhereDocRefIV, $aDataWhereDocRefCDNExt)
    {
        try { 
            $tTableRefPi        = 'TAPTPiHDDocRef';
            $tTableRefPc        = 'TAPTPcHDDocRef';
