...
class mProducttransferwahouse extends CI_Model
{
    ...
    function FSnMTFWUpdateInlineDTTemp($aDataUpd, $aDataWhere)
    {
        try {
            ...
            $tSQL = "SELECT TMP.FCXtdFactor * TMP.FCXtdQty AS QTY FROM TCNTDocDTTmp TMP WITH(NOLOCK) 
                        WHERE TMP.FTXthDocNo = '".$aDataWhere['FTXthDocNo']."' 
                        AND TMP.FTSessionID = '".$this->session->userdata('tSesSessionID')."' 
                        AND TMP.FTXthDocKey = '".$aDataWhere['FTXthDocKey']."' 
                        AND TMP.FNXtdSeqNo = '".$aDataWhere['FNXtdSeqNo']."' ";
            $oQuery = $this->db->query($tSQL);
