...
class mDeposit extends CI_Model
{
    ...
    //Value ของการปิดรอบ ตามสาขา
    public function FSxMHDDepositGetValue($aParams = [])
    {
        $tBCHCode       = $aParams['tBCHCode'];
        $dDateShift     = $aParams['dDateShift'];

        $tSQL   = "SELECT FTSysStaUsrValue FROM TSysConfig WHERE FTSysCode = 'tPS_RcvCash' AND FTSysApp = 'CN' AND FTSysKey = 'RcvCash' ";
        $oQuery = $this->db->query($tSQL);
        $aData  = $oQuery->row_array();
        if( $aData['FTSysStaUsrValue'] == '' || $aData['FTSysStaUsrValue'] == null){
            $nValue = 0;
        }else{
            $nValue = $aData['FTSysStaUsrValue'];
        }

        $tSQL   = "SELECT SUM(FCRcvPayAmt) AS nValPayAmy FROM TPSTShiftHD HD
                    LEFT JOIN TPSTShiftSSumRcv  RCV ON HD.FTBchCode = RCV.FTBchCode AND HD.FTShfCode = RCV.FTShfCode
                    WHERE CONVERT(date,HD.FDShdSignOut) = '$dDateShift' AND HD.FTBchCode = '$tBCHCode' 
                    AND RCV.FTRcvCode IN ($nValue)  ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $aResult = $oQuery->result_array();
            $nValue  = $aResult[0]['nValPayAmy'];
        }else{
            $nValue  = 0;
        }
        return $nValue;
