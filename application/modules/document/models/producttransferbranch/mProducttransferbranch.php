...
class mProducttransferbranch extends CI_Model {
    ...
    public function FSaMTBXList($paData){
        ...
        $tSQL   = "SELECT TOP ". get_cookie('nShowRecordInPageList')." c.* FROM(
                        ...
                        * FROM
                            (SELECT DISTINCT
                                ...
                                BCHT.FTBchName      AS rtBCHNameTo,
                                BCHF.FTBchName      AS rtBCHNameFrm
                            ...
                            LEFT JOIN TCNMBranch_L  BCHF WITH (NOLOCK) ON TBX.FTXthBchFrm = BCHF.FTBchCode AND BCHF.FNLngID = $nLngID 
                            LEFT JOIN TCNMBranch_L  BCHT WITH (NOLOCK) ON TBX.FTXthBchTo = BCHT.FTBchCode AND BCHT.FNLngID = $nLngID 
                            ...
                            WHERE TBX.FTXthDocNo != '' ";

        // User Branch
        if($this->session->userdata("tSesUsrLevel") != "HQ" ){
            $tWhereBchCodeUsr = $this->session->userdata("tSesUsrBchCodeMulti");
            $tSQL .= " AND TBX.FTBchCode IN ($tWhereBchCodeUsr) OR TBX.FTXthBchFrm IN ($tWhereBchCodeUsr) OR TBX.FTXthBchTo IN ($tWhereBchCodeUsr) ";
        }
        ...
        if(!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeTo)){
            $tSQL .= " OR ((TBX.FTBchCode BETWEEN '$tSearchBchCodeFrom' AND '$tSearchBchCodeTo') OR (TBX.FTBchCode BETWEEN '$tSearchBchCodeTo' AND '$tSearchBchCodeFrom'))";
        }
        ...
        unset($nLngID);
        unset($tSQL);
        unset($tWhereBchCodeUsr);
        unset($oAdvanceSearch);
        unset($tSearchList);
    ...
    public function FSnMTBGetPageAll($paData){
        ...
        if($this->session->userdata("tSesUsrLevel") != "HQ" ){
            $tWhereBchCodeUsr = $this->session->userdata("tSesUsrBchCodeMulti");
            $tSQL .= " AND TB.FTBchCode IN ($tWhereBchCodeUsr) OR TB.FTXthBchFrm IN ($tWhereBchCodeUsr) OR TB.FTXthBchTo IN ($tWhereBchCodeUsr) ";
        }
        ...
        $tSearchBchCodeTo   = $oAdvanceSearch['tSearchBchCodeTo'];
        if(!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeTo)){
            $tSQL .= " OR ((TB.FTBchCode BETWEEN '$tSearchBchCodeFrom' AND '$tSearchBchCodeTo') OR (TB.FTBchCode BETWEEN '$tSearchBchCodeTo' AND '$tSearchBchCodeFrom'))";
        }
    ...
    public function FSaMTBGetCountDTTemp($paDataWhere){
            ...
            $tSQL = "SELECT 
                    ...
                    FROM TSVTTBXDocDTTmp DOCTMP WITH (NOLOCK)
                    WHERE 1 = 1
                    ";
    ...
    public function FSaMTBGetVatDTTemp($paDataWhere){
        ...
        $tSQL = "SELECT DISTINCT FCXtdVatRate, 
                ...
                FROM TSVTTBXDocDTTmp WITH (NOLOCK)
                WHERE 1 = 1";
    ...
    function FSnMTBUpdateInlineDTTemp($aDataUpd,$aDataWhere){
        ...
        try{
            ...
            $this->db->update('TSVTTBXDocDTTmp',$aDataUpd);
    ...
    public function FSaMTBInsertPDTToTemp($paData,$paDataWhere){
        ...
        if($paDataWhere['nTBOptionAddPdt']==1){
            ...
            $tSQL = "SELECT FNXtdSeqNo,FCXtdQty FROM TSVTTBXDocDTTmp WITH (NOLOCK)
            ...
            if($oQuery->num_rows() > 0){
                ...
                $tSQL = "UPDATE TSVTTBXDocDTTmp SET
                ...
                //เพิ่ม
                $this->db->insert('TSVTTBXDocDTTmp',array(
    ...
    function FSnMTBCheckPdtTempForTransfer($tDocNo){
        $tSQL = "SELECT COUNT(FNXtdSeqNo) AS nSeqNo FROM TSVTTBXDocDTTmp WITH (NOLOCK) WHERE FTXthDocKey = 'TCNTPdtTbxHD' AND FTXthDocNo = '".$tDocNo."' AND FTSessionID = '".$this->session->userdata('tSesSessionID')."'";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
    ...
    */
    function FSnMWTOUpdateDTTemp($aDataUpd,$paDataWhere){
        ...
        try{
            ...
            if(is_array($aDataUpd) == 1){
                ...
                $this->db->where('FTXthDocNo', $paDataWhere['FTXthDocNo']);
                $this->db->delete('TSVTTBXDocDTTmp');
    
                foreach($aDataUpd as $key=>$val){
                    //เพิ่ม
                    $this->db->insert('TSVTTBXDocDTTmp',$aDataUpd[$key]);
                    if($this->db->affected_rows() > 0){
                        $aStatus = array(
                            'rtCode' => '1',
                            'rtDesc' => 'Add Success.',
    ...
    public function FSaMTBInsertDTToTemp($paData,$paDataWhere){
        ...
        if($paData['rtCode'] == 1){
            ...
            if($paData[0]['FTXthDocNo'] != ''){
                $this->db->where('FTXthDocNo', $paData[0]['FTXthDocNo']);
                $this->db->where('FTSessionID', $this->session->userdata('tSesSessionID'));
                $this->db->delete('TSVTTBXDocDTTmp');
            }
    ...
    public function FSxMClearPdtInTmp(){
        ...
        $tSQL = "DELETE FROM TSVTTBXDocDTTmp WHERE FTSessionID = '$tSesSessionID' AND FTXthDocKey = '$tDocKey'";
        $this->db->query($tSQL);
    ...
    public function FSnMTBDel($paData){
        try{
            ...
            $this->db->where('FTXthDocNo', $paData['FTXthDocNo']);
            $this->db->delete('TCNTPdtTbxDTFhn');
    ...
    public function FSxMTFXClearDocTemForChngCdt($pInforData){
        $tSQL = "DELETE FROM TSVTTBXDocDTTmp 
    ...
    //Get available column for showing in grid table
    ...
    //Get ข้อมูลเพื่อไปส่งไป Noti
    public function FSaMGetHDForNoti($aParam){
        $ptTextDocRef   = $aParam['tDocNo'];
        $nLngID         = $this->session->userdata("tLangEdit");
        $tSQL = "SELECT 
                    HD.FTXthDocNo ,
                    BCH.FTBchCode       AS rtBCHCode , 
                    BCH.FTBchName       AS rtBCHName ,
                    BCHTO.FTBchCode     AS rtBCHCodeTo , 
                    BCHTO.FTBchName     AS rtBCHNameTo ,
                    BCHFRM.FTBchCode    AS rtBCHCodeFrm , 
                    BCHFRM.FTBchName    AS rtBCHNameFrm
                    FROM TCNTPdtTbxHD HD 
                LEFT JOIN TCNMBranch_L BCH      ON  HD.FTBchCode   = BCH.FTBchCode      AND BCH.FNLngID = $nLngID
                LEFT JOIN TCNMBranch_L BCHTO    ON  HD.FTXthBchTo  = BCHTO.FTBchCode    AND BCHTO.FNLngID = $nLngID
                LEFT JOIN TCNMBranch_L BCHFRM   ON  HD.FTXthBchFrm = BCHFRM.FTBchCode   AND BCHFRM.FNLngID = $nLngID
                WHERE FTXthDocNo = '$ptTextDocRef' ";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
    ...
    public function FSaMCLMUpdateInlineDTTemp($paDataUpdateDT,$paDataWhere){
     ...
     //ล้างข้อมูลใน temp
     public function FSaMTXDeletePDTInTmp(){
    ...
    public function FSaMTBXGetPdtInTmpForSendToAPI($paData){
        ...
        $tSQL       = " SELECT
                            ...
                            '$tDocCode'                 AS ptDocNo,
                            ''                           AS ptRefDocNo
                        FROM TCNTPdtTbxDT               TMP     WITH(NOLOCK)
    ...
    public function FSxMTBXFindTextNamePDTNoStock($ptTextCodePDT){
        $nLngID     = $this->session->userdata("tLangEdit");
    ...
    public function FSxMTBXChkConfig(){
    ...
    //Get ข้อมูล API
    public function FSaMTBXGetConfigAPI(){
    ...
    public function FSxMTBXUpdatePdtStkPrc($paDataWhere,$paHavePdtInWah){
        ...
        $this->db->update('TCNTPdtTbxDT');


        return $this->db->last_query();
    }

    public function FSxMTBXUpdatePdtStkPrcAll($paDataWhere,$ptStatus){
        $tBchCode   = $paDataWhere['FTBchCode'];
        $tDocCode   = $paDataWhere['FTXshDocNo'];
