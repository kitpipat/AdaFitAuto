...
class mPurchaseOrder extends CI_Model {
    ...
    public function FSaMPOGetDataTableList($paDataCondition){
        ...
        $tSearchSplCode         = $aAdvanceSearch['tSearchSplCode'];
        ...
        $tSQL   .=   " SELECT DISTINCT
                    ...
                    LEFT JOIN TCNMUser      USR     WITH ( NOLOCK ) ON POHD.FTCreateBy = USR.FTUsrCode
                WHERE 1=1 ";
        ...
        if(isset($tSearchList) && !empty($tSearchList)){
            $tSQL .= " AND (
                (POHD.FTXphDocNo        LIKE '%$tSearchList%') 
                OR (BCHL.FTBchName      LIKE '%$tSearchList%')
                OR (BCHLTO.FTBchName    LIKE '%$tSearchList%')
                OR (SPLL.FTSplName      LIKE '%$tSearchList%')
                OR (CONVERT(CHAR(10),POHD.FDXphDocDate,103) LIKE '%$tSearchList%')
            )";
        }
        ...
        // Check Supplier Code
        if(isset($tSearchSplCode) && !empty($tSearchSplCode)){
            $tSQL   .= " AND POHD.FTSplCode = '$tSearchSplCode' ";
        }
    ...
    public function FSoMPOCallRefIntDocDataTable($paDataCondition){
        ...
        $tSQLMain = "SELECT
                        ...
                        FROM
                            TCNTPdtReqSplHD PRS WITH(NOLOCK)
                        ...
                        AND (PRS.FTXphStaPrcDoc = 1 OR PRS.FTXphStaPrcDoc = 3)
                        AND PRS.FTXphStaApv = 1 AND ISNULL(POREF.FTXshRefType, '') = '' ";
