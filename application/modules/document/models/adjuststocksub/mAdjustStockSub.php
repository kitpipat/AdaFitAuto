...
class mAdjustStockSub extends CI_Model
{
    ...
    public function FSaMAdjStkSubList($paData)
    {
        ...
        if(!empty($tSearchStaPrcStk) && ($tSearchStaPrcStk != "0")){
            if($tSearchStaPrcStk == 3){
                $tSQL   .= " AND ADJSTK.FTAjhStaPrcStk  = ".$this->db->escape($tSearchStaPrcStk)." OR ADJSTK.FTAjhStaPrcStk = '' ";
            }else{
    ...
    public function FSaMASTEventAddProducts($paDataCondition, $paDataInsert)
    {
        ...
        if($paDataCondition['oetASTFilterPdtCodeFrom'] == $paDataCondition['oetASTFilterPdtCodeTo']){
            // Condition BarCode
            if (isset($paDataCondition['oetASTFilterPdtBarCodeFrom']) && $paDataCondition['oetASTFilterPdtBarCodeFrom'] != "" && isset($paDataCondition['oetASTFilterPdtBarCodeTo']) && $paDataCondition['oetASTFilterPdtBarCodeTo'] != "") {
                $tCondition .= " AND PBAR.FTBarCode BETWEEN '" . $paDataCondition['oetASTFilterPdtBarCodeFrom'] . "' AND '" . $paDataCondition['oetASTFilterPdtBarCodeTo'] . "' ";
            }
        }
