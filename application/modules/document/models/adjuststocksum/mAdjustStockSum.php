...
class mAdjustStockSum extends CI_Model
{
    ...
    public function FSaMAdjStkSumDataTableList($paData)
    {
        ...
        $tSQL       = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC) AS FNRowID,* FROM (
                    ...
                    LEFT JOIN TCNMBranch_L  BCHL WITH (NOLOCK) ON ADJSTK.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID    = ".$this->db->escape($nLngID)."
                    LEFT JOIN TCNMUser_L    USRL WITH (NOLOCK) ON ADJSTK.FTCreateBy = USRL.FTUsrCode AND USRL.FNLngID   = ".$this->db->escape($nLngID)."
                    LEFT JOIN TCNMUser_L    USRLAPV WITH (NOLOCK) ON ADJSTK.FTAjhApvCode = USRLAPV.FTUsrCode AND USRLAPV.FNLngID = ".$this->db->escape($nLngID)."
                    LEFT JOIN TCNMUser_L    HQAPV   WITH (NOLOCK) ON ADJSTK.FTAjhStkApvCode   = HQAPV.FTUsrCode AND HQAPV.FNLngID = ".$this->db->escape($nLngID)."
                    WHERE ADJSTK.FTAjhDocType = 2
        ";
        ...
        if (@$tSearchList != '') {
            $tSQL   .= "
                AND (
                    (ADJSTK.FTAjhDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (CONVERT(CHAR(10),ADJSTK.FDAjhDocDate,103) LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                )
            ";
        }
        // จากสาขา - ถึงสาขา
        ...
        if (!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeFrom)) {
            $tSQL   .= "
                AND (
                    (ADJSTK.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeFrom)."    AND ".$this->db->escape($tSearchBchCodeTo).")
                    OR (ADJSTK.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeTo)."   AND ".$this->db->escape($tSearchBchCodeFrom).")
                )
            ";
        }
        ...
        if (!empty($tSearchDocDateFrom) && !empty($tSearchDocDateTo)) {
            $tSQL   .= " 
                AND (
                    (ADJSTK.FDAjhDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00").")   AND CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:59:59")."))
                    OR (ADJSTK.FDAjhDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 00:00:00").")  AND CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 23:59:59")."))
                )
            ";
        }
        ...
        if (!empty($tSearchStaDoc) && ($tSearchStaDoc != "0")) {
            if ($tSearchStaDoc == 3) {
                $tSQL   .= " AND ADJSTK.FTAjhStaDoc = ".$this->db->escape($tSearchStaDoc)." ";
            } else if ($tSearchStaDoc == 2) {
                $tSQL   .= " AND ISNULL(ADJSTK.FTAjhStaApv,'') = '' AND ADJSTK.FTAjhStaDoc != '3'";
            } else if ($tSearchStaDoc == 1) {
                $tSQL   .= " AND ADJSTK.FTAjhStaApv = ".$this->db->escape($tSearchStaDoc)." ";
            }
        }
        ...
        if (!empty($tSearchStaPrcStk) && ($tSearchStaPrcStk != "0")) {
            ...
            if ($tSearchStaPrcStk == 3) {
                $tSQL .= " AND (ADJSTK.FTAjhStaPrcStk = ".$this->db->escape($tSearchStaPrcStk)." OR ISNULL(ADJSTK.FTAjhStaPrcStk,'') = '') ";
            } else {
                $tSQL .= " AND ADJSTK.FTAjhStaPrcStk  = ".$this->db->escape($tSearchStaPrcStk)." ";
            }
        }
    ...
    public function FSnMAdjStkSumGetPageAll($paData)
    {
        $nLngID = $paData['FNLngID'];
        $tSQL   = "
            SELECT COUNT (ADJSTK.FTAjhDocNo) AS counts
            FROM [TCNTPdtAdjStkHD] ADJSTK WITH (NOLOCK)
            LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK) ON ADJSTK.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($nLngID)." 
            WHERE AND  ADJSTK.FTAjhDocType = 2
        ";
        ...
        if (@$tSearchList != '') {
            $tSQL   .= "
                AND (
                    (ADJSTK.FTAjhDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (ADJSTK.FDAjhDocDate LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                )
            ";
        }
    ...
    public function FSaMAdjStkSumInsertPDTToTemp($paDataWhere)
    {
        try {
            ...
            $tSql = "INSERT INTO TCNTDocDTTmp (
                    ...
                    AND HD.FTAjhCountType = '" . $paDataWhere['FTAjhCountType'] . "'
                    GROUP BY
                        DT.FTPdtName,
                        DT.FTPunName,
                        DT.FTPdtCode,
    ...
    public function FSvMAdjStkSumApprove($paDataUpdate)
    {
        ...
        //Function : Approve Doc
        public function FSvMAdjStkSumHDApprove($paDataUpdate)
        {
            try {
                //DT Dis
                // $this->db->set('FTAjhStaPrcStk', 2);
                $this->db->set('FTAjhStaApv', 1);
                // $this->db->set('FTAjhApvCode', $paDataUpdate['FTXthApvCode']);
                $this->db->set('FTAjhStkApvCode', $paDataUpdate['FTXthApvCode']);
                $this->db->where('FTAjhDocNo', $paDataUpdate['FTAjhDocNo']);
