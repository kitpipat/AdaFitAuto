...
function JSvImpSelectDataInTable(pnPage, poData) {
    ...
    if (typeof (nStaSession) !== 'undefined' && nStaSession == 1) {
        let tSearchAll = $('#oetCMDSearchImport').val();
        let nPageCurrent = (pnPage === undefined || pnPage == '') ? '1' : pnPage;
        if (poData != undefined && poData['nStaChkFile'] == 1) {
            let aDataFile = poData['oDataFile'];
        } else {
            let aDataFile = null;
        }
        let tCmdConsImport  = $('.xWCmdConsImport:checked').val();
        let tCmdDataEntry   = $('#ocmCmdDataEntry').val();
        let tReasonFile     = $('#oetCardReasonCode').val();
        ...
        oFromData.append(tCSRFTokenName,tCSRFTokenValue);
