...
function JSoPOGetAdvanceSearchData(){
    var oAdvanceSearchData  = {
        ...
        tSearchSplCode      : $('#oetPOAdvSearchSplCode').val()
    };
    ...
    $.ajax({
        ...
        success: function (oResult){
            ...
            if (aDataReturnChkTmp['nStaReturn'] == '1'){
                ...
                // $('#ofmPOFormAdd :select').removeAttr("disabled");
                $.ajax({
                    type    : "POST",
                    url     : $("#ohdPORoute").val(),
...
function JCNxPOControlObjAndBtn(){
    ...
    // console.log(nPOStaDoc+'//'+nPOStaApv+'//'+nPOStaRef);
    JSxPONavDefultDocument();
    ...
    if(nPOStaDoc != 1){
        ...
        $("#obtPOExportDoc").hide(); // attr("disabled",true);
    ...
    if(nPOStaDoc == 1 && nPOStaApv == 1 && nPOStaRef == 2){
        // console.log('IF2');
        // Hide/Show Menu Title 
        $("#obtPOExportDoc").show(); 
        $("#oliPOTitleAdd").hide();
...
//ส่งออกไฟล์
function JSxPOSendEmail(){
    var tConcatDocNoRef = $("#oetPODocNo").val();
    var tPOEmailName = $("#ohdPOSplEmail").val();
    if(tPOEmailName != ''){
        $('#odvPOModalExportFile').modal('show');
        $('#odvPOModalExportFile .xCNConfirmExportAndDowload').unbind().click(function(){
            JCNxOpenLoading();
            JSxPOExportFileToMQ(); //ส่งออกเเละส่งเมลล์
        });
    }else{
        FSvCMNSetMsgErrorDialog('ไม่พบการตั้งค่าอีเมลของผู้จำหน่าย กรุณาตรวจสอบ');
    }
}

//ส่งเข้า MQ
function JSxPOExportFileToMQ(){

    //ลบ queue
    var tPODocno = $("#oetPODocNo").val();
    var tPOBchCode = $("#oetPOFrmBchCode").val();
    var tPOEmailType = $("#ocmPOFilterExport").val();
    

    //ส่งเข้า MQ
    $.ajax({
        type    : "POST",
        url     : "docPODocPreOrdBExport",
        data    : {
            tPODocno    : tPODocno,
            tPOBchCode   : tPOBchCode,
            tPOEmailType   : tPOEmailType,
        },
        cache   : false,
        timeout : 5000,
        success : function (oResult) {
