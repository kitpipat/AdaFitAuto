...
$("document").ready(function() {
    ...
    JSxTFWNavDefult();
    $("#obtTWXSubmit").removeAttr("disabled");

    switch (nStaTFWBrowseType) {
        case '2':
            var tAgnCode = $('#oetTFWJumpAgnCode').val();
            var tBchCode = $('#oetTFWJumpBchCode').val();
            var tDocNo = $('#oetTFWJumpDocNo').val();
            JSxRefillVDToTransferWahousePageEdit(tDocNo);
            break;
        case '1':
            JSvCallPageTFWAdd();
            break;
...
function JSxValidateViaOrderBeforeApv(pbIsConfirm) {
    JSxSubmitEventByButton('approve');
    ...
    if (pbIsConfirm) {
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status != 404) {
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'ยกเลิกใบโอนสินค้าระหว่างคลัง';
                    var tErrorStatus = jqXHR.status;
                    var tHtmlError = $(jqXHR.responseText);
                    var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
    ...
    $.ajax({
        ...
        success: function(tResult) {
            ...
            if ($("#ohdCheckDuplicateTFW").val() == 1) {
                //ส่งขอมูลไปรวบรวมที่ Center ก่อนส่ง MQ เพื่อเก็บ LOG (TYPE:ERROR)
                var tLogFunction = 'ERROR';
                var tDisplayEvent = 'เพิ่ม/แก้ไข ใบโอนสินค้าระหว่างคลัง';
                var tErrorStatus = '900'
                var tHtmlError = 'Data Duplicate'
                var tLogDocNo = $("#oetXthDocNo").val();
...
function JSxSubmitEventByButton(ptType = '') {
    var tDocNoSend = ""
    if ($("#ohdTFWRoute").val() != "TFWEventAdd") {
    ...
    $("#obtTWXSubmit").attr('disabled', 'true');
            ...
            if (bReturn) {
                $.ajax({
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if (jqXHR.status != 404) {
                            var tLogFunction = 'ERROR';
                            var tDisplayEvent = 'บันทึก/แก้ไข ใบโอนสินค้าระหว่างคลัง';
                            var tErrorStatus = jqXHR.status;
                            var tHtmlError = $(jqXHR.responseText);
                            var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                            var tLogDocNo = tDocNo;
...
function JSvCallPageTFWEdit(ptXthDocNo) {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.status != 404) {
                var tLogFunction = 'ERROR';
                var tDisplayEvent = 'เรียกดูเอกสารใบโอนสินค้าระหว่างคลัง';
                var tErrorStatus = jqXHR.status;
                var tHtmlError = $(jqXHR.responseText);
                var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
...
function JCNxTFWControlObjAndBtn() {
    ...
    var tRefInType          = $('#ohdTFWRefInType').val();
    var tAutStaCancel       = $('#ohdTFWAutStaCancel').val();
    var tDocDateCreate      = $('#ohdTFWDocDateCreate').val();
    var tDocDateNowToday    = $('#ohdTFWDateNowToday').val();
    ...
    if (ohdXthStaDoc == 1 && ohdXthStaApv == 1 && tRefInType != 2){
        ...
        $("#osmConfirm").on("click", function(evt) {
            ...
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    if (jqXHR.status != 404) {
                        var tLogFunction = 'ERROR';
                        var tDisplayEvent = 'ลบใบโอนสินค้าระหว่างคลัง';
                        var tErrorStatus = jqXHR.status;
                        var tHtmlError = $(jqXHR.responseText);
                        var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                        var tLogDocNo = tIDCode;
                        JCNxPackDataToMQLog(tMsgErrorBody, tErrorStatus, tDisplayEvent, tLogFunction, tLogDocNo);
                    } else {
                        //JCNxSendMQPageNotFound(jqXHR,tIDCode);
                    }
                }
            });
        });
