            ...
            <?php if ($aDataList['rtCode'] == 1) { ?>
                <?php foreach ($aDataList['raItems'] as $key => $aValue) { ?>
                    <tr class="xCNTextDetail2 xCNDepositCashRow" data-seq-no="<?php echo $aValue['FNXtdSeqNo']; ?>">
                        ...
                        <td class="text-left">
                            <!--xCNDatePicker : ใบนำฝากของ fitauto ไม่ต้องใส่ค่าให้เอาค่ามาจากการปิดรอบ-->
                            <input 
                            type="text" 
                            class="form-control xCNDepositCashRefDate xCNApvOrCanCelDisabledCash" 
                            id="oetDepositCashRefDate_<?php echo $key+1; ?>" 
                            name="oetDepositCashRefDate_<?php echo $key+1; ?>" 
                            value="<?php echo empty($aValue['FDBddRefDateForDeposit']) ? '' : date('Y-m-d', strtotime($aValue['FDBddRefDateForDeposit'])); ?>" readonly>
                        </td>
                        <td class="text-right">
                            <input 
                            type="text" 
                            class="xCNDepositCashRefAmt xCNInputNumericWithDecimal xCNApvOrCanCelDisabledCash text-right"
                            value="<?php echo number_format($aValue['FCBddRefAmtForDeposit'], 2, ".", ""); ?>" readonly>
                        </td>
