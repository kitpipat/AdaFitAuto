				...
				<div id="odvCondition" class="panel-collapse collapse in" role="tabpanel">
					<div class="panel-body xCNPDModlue">
						...
						<div class="form-group">
							...
							readonly
							type="text"
							placeholder="<?=Language('document/deposit/deposit', 'tBCH'); ?>"
						...
						<div class="form-group">
							...
							readonly
							type="text"
							placeholder="<?=Language('document/deposit/deposit', 'tBank'); ?>"
							class="form-control xCNInputWithoutSpc xCNApvOrCanCelDisabled"
						...
						<div class="form-group">
							...
							readonly
							type="text"
							placeholder="<?=Language('document/deposit/deposit', 'tAccountType'); ?>"
							class="form-control xCNInputWithoutSpc xCNApvOrCanCelDisabled"
						...
						<div class="form-group">
							...
							readonly
							type="text"
							placeholder="<?=Language('document/deposit/deposit', 'tAccountNumber'); ?>"
							class="form-control xCNInputWithoutSpc xCNApvOrCanCelDisabled"
			...
			<div class="panel panel-default xCNDepositCashContainer" style="margin-bottom: 25px;">
				...
				<div class="panel-collapse collapse in" role="tabpanel">
					...
					<?php if(!$bIsApvOrCanCel) { ?>
                        <div class="row">
                            <div class="col-md-12">
                                <button class="xCNBTNPrimeryPlus pull-right" onclick="JSxDepositFindValue()" data-toggle="modal" data-target="#odvDepositPopupCashAdd" style="margin-bottom: 10px;">+</button>
                            </div>
                        </div>
                    <?php } ?>  
					...
					<div class="row">
						...
						<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
							<div class="form-group">
								...
								<input type="number" class="form-control xCNInputWithoutSingleQuote " id="oetDepositCashAddValue" name="oetDepositCashAddValue" readonly>
							</div>
						</div>
					</div>
				</div>
...
<script>
	...
	//หาว่ายอดปิดรอบของสาขานี้มีเท่าไหร่
	function JSxDepositFindValue(){
		var tBCHCode 	= $('#oetDepositBchCode').val();
		var dDateShift 	= $('#oetDepositCashAddDate').val();
		
		$.ajax({
			type: "POST",
			url	: "depositFindShiftValue",
			data: {
				tBCHCode	: tBCHCode,
				dDateShift	: dDateShift
			},
			cache: false,
			timeout: 5000,
			success: function(tResult) {
				var oResult = JSON.parse(tResult);
				if(oResult.nValue == null){
					$('#oetDepositCashAddValue').val(0);
				}else{
					$('#oetDepositCashAddValue').val(oResult.nValue);
				}
			},
	...
	//ทุกครั้งที่เปลี่ยนวันที่
	$('#oetDepositCashAddDate').change(function() {
		if($(this).val() != ''){
			JSxDepositFindValue();
		}
	});
