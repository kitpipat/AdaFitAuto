        ...
        <table id="otbCLMStep1Point2DocPdtAdvTableList" class="table table-striped">
            <thead>
                <tr class="xCNCenter">
                    ...
                    <th class="xCNTextBold"><?= language('document/invoice/invoice', 'สาขา(ลูกค้า)') ?></th>
                    ...
                    <th class="xCNTextBold"><?= language('document/invoice/invoice', 'สาขาที่ขาย') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoice/invoice', 'อ้างอิงใบกำกับภาษีเต็มรูป') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoice/invoice', 'วันที่อ้างอิงใบกำกับภาษีเต็มรูป') ?></th>
                ...
                <?php if ($aDataList['rtCode'] == 1) : ?>
                    ...
                    if (FCNnHSizeOf($aDataList['raItems']) != 0) {
                        ...
                        foreach ($aDataList['raItems'] as $nKey => $aDataTableVal) : ?>
                            ...
                            if($aDataTableVal['FTSrnCode'] == 'SC'){
                                $SumDiscount = $SumDiscount + $aDataTableVal['FCXtdAmt'];
                            }
                            if($aDataTableVal['FTSrnCode'] == 'SD'){
                            ...
                            <tr class="otr<?= $aDataTableVal['FTPdtCode']; ?> xWPdtItemStep2 xWPdtItemList<?= $nKey ?>" data-key="<?= $nKey ?>" data-pdtcode="<?= $aDataTableVal['FTPdtCode']; ?>" data-seqno="<?= $nKey ?>">
                                ...
                                <?php if($aDataTableVal['FTSrnCode'] == 'SC'){?>  
...
<input type="hidden" id="ohdIVCGrand" name="ohdIVCGrand" value="<?php echo @$SumAmt ?>">
<input type="hidden" id="ohdIVCAlreadyPaid" name="ohdIVCAlreadyPaid" value="<?php echo @$SumAlreadyPaid ?>">
<input type="hidden" id="ohdIVCGrandText" name="ohdIVCGrandText">
                ...
                <tr class="otrSumDetail1">
                    <td>
                        <div class='row'>
                            ...
                            <div class='col-xs-9' id="odvIVCDataTextBath">
                                <script>
                                    var tTextTotal = $('#ohdIVCGrand').val().replace(/,/g, '');
                                    var tThaibath = ArabicNumberToText(tTextTotal.replace("-", ""));
                                    
                                    $('#odvIVCDataTextBath').text(tThaibath);
                                    $('#ohdIVCGrandText').val(tThaibath);
                                </script>
                            </div>
                        </div>
                    </td>
