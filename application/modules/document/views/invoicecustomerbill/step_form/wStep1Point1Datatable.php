...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
    <div class="table-responsive">
        <table id="otbIVCStep1Point1DocPdtAdvTableList" class="table table-striped">
            <thead>
                <tr class="xCNCenter">
                    ...
                    <th class="xCNTextBold" style="width:5%;"><?= language('document/invoicebill/invoicebill', 'ลำดับ') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'สาขา(ลูกค้า)') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'เลขที่เอกสาร') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'วันที่เอกสาร') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'tIVBDocFrmDateSalBch') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'อ้างอิงใบกำกับภาษีเต็มรูป') ?></th>
                    <th class="xCNTextBold"><?= language('document/invoicebill/invoicebill', 'วันที่อ้างอิงใบกำกับภาษีเต็มรูป') ?></th>
                    <!-- <th class="xCNTextBold" style="width:10%;"><?= language('document/invoicebill/invoicebill', 'ครบกำหนด') ?></th> -->
                    <th class="xCNTextBold" style="width:10%;"><?= language('document/invoicebill/invoicebill', 'จำนวนเงิน') ?></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    if (FCNnHSizeOf($aDataList['raItems']) != 0) {
                        foreach ($aDataList['raItems'] as $nKey => $aDataTableVal) : ?>
                            ...
                            <tr class="otr<?= $aDataTableVal['FTPdtCode']; ?> xWPdtItemPoint1 xWPdtItemList<?= $nKey ?>" data-key="<?= $nKey ?>" data-pdtcode="<?= $aDataTableVal['FTPdtCode']; ?>" data-seqno="<?= $nKey ?>">
                                ...
                                <?php 
                                $TypeDoc = '';
                                if($aDataTableVal['FTSrnCode'] == 'Sal'){
                                    $TypeDoc = 'ใบขายสินค้า';    
                                }elseif($aDataTableVal['FTSrnCode'] == 'SC'){
                                    $TypeDoc = 'ใบลดหนี้';   
                                }elseif($aDataTableVal['FTSrnCode'] == 'SD'){
                                    $TypeDoc = 'ใบเพิ่มหนี้';   
                                }
                                ?>
                                <td><?= $aDataTableVal['FTPgpChain']; ?></td>
