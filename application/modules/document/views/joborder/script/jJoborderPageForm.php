...
<script>
    ...
    function JSxStaStock(){  
		var tTextSearch = new RegExp($('#ocmJOBStaStock').val());
        if(tTextSearch =="/all/"){
            JSxClearFilter()
        }else{
            $('.xWContent').hide();
            $('.xWContent').filter(function() {
                return tTextSearch.test($(this).find('.xWStaStock').val());
            }).show();
        }

        var nCNTrHide = $('.xWContent:hidden').length;
        if(nCNTrHide == ($('#otbJORDocPdtAdvTableList tbody tr').length) - 1){
            $('.xCNDataNotFound').show();
        }else{
            $('.xCNDataNotFound').hide();
        }
	}
	
    function JSxClearFilter(){
        $('.xWContent').show();
    }
    ...
    //พิมพ์เอกสาร
    function JSxJobPrintDoc(){
        var aInfor = [
            {"Lang"         : '<?=FCNaHGetLangEdit(); ?>'},
            {"ComCode"      : '<?=FCNtGetCompanyCode(); ?>'},
            {"BranchCode"   : '<?=FCNtGetAddressBranch(@$tJOBBchCode); ?>'},
            {"DocCode"      : '<?=@$tJOBDocNo; ?>'}, // เลขที่เอกสาร
            {"DocBchCode"   : '<?=@$tJOBBchCode;?>'}
        ];
        window.open("<?=base_url(); ?>formreport/Frm_SQL_SMJobOrder?infor=" + JCNtEnCodeUrlParameter(aInfor), '_blank');
    }

    //กดเพื่อที่ไปหน้าประเมินก่อนซ่อม
    function JSxJobJumpPage(ptParamPage)  {
        var nStaSession = JCNxFuncChkSessionExpired();
        if (typeof(nStaSession) !== "undefined" && nStaSession == 1) {
            var tDocNo      = $('#oetJOBDocNo').val();
            var dDocRefDate = $('#ohdJOBCreateDate').val();
            var tCarCode    = $('#ohdJOBCarCode').val();
            var tCarReg     = $('#oetJOBCarNo').val();
            var tCarMile    = $('#oetJOBCarMileAge').val();
            var tBchCode    = $('#ohdJOBBchCode').val();
            var tBchName    = $('#oetJOBBchName').val();
            var tAgnCode    = $('#oetJOBAgnCode').val();
            var tAgnName    = $('#oetJOBAgnName').val();
            if (ptParamPage == 'CHK') {
                var tCstCode        = $('#ohdJOBCstCode').val();
                var tTelephone      = $('#oetJOBCstTel').val();
                var tEmail          = $('#oetJOBCstMail').val();
                var aItemCstCode = [
                    {'tCstCode'      : tCstCode },
                ...
                JSxGotoPageCHK(tDocNo, dDocRefDate, tCarCode, tCarReg, tCarMile, tBchCode , aItemCstCode)
            } else {
                JSxGotoPageIAS(tDocNo, dDocRefDate, tCarCode, tCarReg, tCarMile, tBchCode, tBchName, tAgnCode, tAgnName)
            }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////    
    function JSxGotoPageCHK(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode , poDataCustomer){
        $.ajax({
            type    : "GET",
            url     : "docPreRepairResult/0/0",
            data    : {'ptTypeJump' : '1' },
            cache   : false,
            timeout : 5000,
            success : function (tResult) {
                ...
                JSxChkCHKTypeAddOrUpdate(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode , poDataCustomer);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
            }
    ...
    function JSxChkCHKTypeAddOrUpdate(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode , poDataCustomer) {  
        $.ajax({
            type    : "POST",
            url     : "docPreRepairResultChkTypeAddOrUpdate",
            data    : {'ptDocNo' : ptDocNo},
            cache   : false,
            timeout : 5000,
            success : function (tResult) {
                var aDataFinal = JSON.parse(tResult);
                localStorage.tCheckBackStage = 'PageJobOrder';

                aDataDoc = [
                    {
                        "ptDocNo"      : ptDocNo,
                        "pdDocRefDate" : pdDocRefDate,
                        "ptCarMile"    : ptCarMile,
                        'ptBchCode'    : ptBchCode
                    }
                ];

                aDataCar = [ptCarCode, ptCarReg, ''];
                
                if (aDataFinal.tReturn == 1) {
                    JSvPreSvCallPageEdit(aDataFinal.aRtData[0].FTAgnCode, aDataFinal.aRtData[0].FTBchCode, aDataFinal.aRtData[0].FTXshDocNo);
                }else{
                    JSvPreSvCallPageAdd2(aDataDoc, aDataCar , poDataCustomer);
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
    ...
    function JSxGotoPageIAS(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode, tBchName, ptAgnCode, ptAgnName){
        $.ajax({
            type    : "GET",
            url     : "docIAS/0/0",
            data    : {'ptTypeJump' : '1' },
            ...
            success : function (tResult) {
                $(window).scrollTop(0);
                $('.odvMainContent').html(tResult);
                JSxChkIASTypeAddOrUpdate(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode, tBchName, ptAgnCode, ptAgnName);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
            }
    ...
    function JSxChkIASTypeAddOrUpdate(ptDocNo, pdDocRefDate, ptCarCode, ptCarReg, ptCarMile, ptBchCode, tBchName, ptAgnCode, ptAgnName) {  
        $.ajax({
            type    : "POST",
            url     : "docIASChkTypeAddOrUpdate",
            data    : {'ptDocNo' : ptDocNo },
            cache   : false,
            timeout : 5000,
            success : function (tResult) {
                ...
                if (aDataFinal.tReturn == 1) {
                    JSvIASCallPageEdit(aDataFinal.aRtData[0].FTAgnCode, aDataFinal.aRtData[0].FTBchCode, aDataFinal.aRtData[0].FTXshDocNo);
                }else{
