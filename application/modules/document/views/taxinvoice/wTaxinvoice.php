...
<script>
    ...
    //อนุมัติเอกสาร
    $('#obtApproveDocument').off('click').on('click',function(){
        ...
        $.ajax({
            ...
            success : function (tResult) {
                ...
                if(tTypeABB == 4){//เอกสารขาย
                    ...
                    let tRsnName    = $('#ohdTAXRsnName').val();
                    var aInfor      = [
                        {"Lang"         : '<?=FCNaHGetLangEdit(); ?>'},
                        {"ComCode"      : '<?=FCNtGetCompanyCode(); ?>'},
                        {"BranchCode"   : tBCH},
                    ...
                    window.open("<?=base_url(); ?>formreport/TaxInvoice_refund?StaPrint=0&infor=" + JCNtEnCodeUrlParameter(aInfor) + "&RsnName="+tRsnName + "&Grand="+tGrandText + "&PrintOriginal="+tOrginalRight + "&PrintCopy="+tCopyRight + "&PrintByPage=" + 'ALL', '_blank');
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
            ...
            $.ajax({
                ...
                success : function (tResult) {
                    ...
                    if(tTypeABB == 4){//เอกสารขาย
                        ...
                        // $("#oifPrint").prop('src', "<?=base_url();?>formreport/Frm_PSInvoiceSale?StaPrint=1&infor=" + JCNtEnCodeUrlParameter(aInfor) + "&Grand="+tGrandText + "&PrintOriginal="+tOrginalRight + "&PrintCopy="+tCopyRight + "&PrintByPage=" + nPrintOnlyPage);
                    }else{ //เอกสารคืน
