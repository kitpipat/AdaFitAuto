...
<script>
    ...
    $('.xWEditDocRef').off('click').on('click',function(){
        ...
        $('#ocbPORefType').val(tRefType);
        $('#ocbPORefType').selectpicker('refresh');
        $('#oetPORefDocDate').datepicker({ dateFormat: 'yy-mm-dd' }).val(tRefDocDate);

        if(tRefType == 1){//ภายใน
            $('#oetPODocRefIntName').val(tRefDocNo);
            $('#oetPODocRefInt').val(tRefDocNo);
        }else{ //ภายนอก
            $('#oetPORefDocNo').val(tRefDocNo);
        }

        $('#oetPORefKey').val(tRefKey);
        $('#oetPORefDocNoOld').val(tRefDocNo);
