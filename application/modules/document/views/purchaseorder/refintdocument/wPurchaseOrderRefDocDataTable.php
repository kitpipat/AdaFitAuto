    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="table-responsive">
            <table id="otbSOTblDataDocHDList" class="table table-striped">
                <thead>
                    <tr class="xCNCenter">
                        <th nowrap class="xCNTextBold"><?php echo language('document/purchaseorder/purchaseorder','tPOLabelBranchCreate')?></th>
                        <th nowrap class="xCNTextBold"><?php echo language('document/purchaseorder/purchaseorder','tPOLabelBranchTo')?></th>
                        <th nowrap class="xCNTextBold"><?php echo language('document/purchaseorder/purchaseorder','tPOLabelSplNameTo')?></th>
                    ...
                    <?php if($aDataList['rtCode'] == 1 ):?>
                        <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                            ...
                            <tr style="cursor:pointer;" class="text-center xCNTextDetail2 xWPIDocItems xPurchaseInvoiceRefInt" 
                                    ...
                                    data-bchcodeto="<?php echo $aValue['FTXphBchTo']?>"
                                    data-bchname="<?php echo $aValue['FTBchName']?>"
                                    data-bchnameto="<?php echo $aValue['FTBchNameTo']?>"
                                    data-splcode="<?php echo $aValue['FTSplCode']?>"
                                    data-docdate="<?php echo $aDateDoc[0]?>"
                                    data-splname="<?php echo $aValue['FTSplName']?>"
                                    data-vatcode="<?php echo $aValue['FTVatCode']?>"
                                    data-vatinroex="<?php echo $aValue['FTSplStaVATInOrEx']?>"
                                    data-vatrate="<?php echo $aValue['FCVatRate']?>"
                                    data-tsppaid="<?php echo $aValue['FTSplTspPaid']?>"
                                    data-crlimit="<?php echo $aValue['FCSplCrLimit']?>"
                                    data-crtrem="<?php echo $aValue['FNSplCrTerm']?>"
                            >
                                ...
                                <td nowrap class="text-left"><?php echo (!empty($aValue['FTSplName']))? $aValue['FTSplName']   : '-' ?></td>
...
<div class="">
    ...
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
        <div class="xWPIPageDataTable btn-toolbar pull-right">
            ...
            <button onclick="JSvPORefIntClickPageList('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>>
            ...
            <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataList['rnAllPage'],$nPage+2)); $i++){?>
                ...
                ?>
                <button onclick="JSvPORefIntClickPageList('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
            <?php } ?>

            <?php if($nPage >= $aDataList['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
            <button onclick="JSvPORefIntClickPageList('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>>
...
<div class="">
        <div id="odvPORefIntDocDetail">

        </div>
</div>

<?php include('script/jPurchaseOrderRefDocDataTable.php')?>
