<div class="table-responsive">
    <table class="table table-striped xWPdtTableFont" id="otbPNDOCPdtTable">
        <thead>
            <tr class="xCNCenter">
                ...
                <?php if(empty($tStaApv) && $tStaDoc != 3){?>
                    ...
                    <th class="xWPNDeleteBtnEditButtonPdt"><?= language('document/purchaseorder/purchaseorder', 'tPOTBEdit')?></th>
                <?php }else { ?>
                    <th></th>
                <?php } ?>
            </tr>
        ...
        <input type="hidden" id="ohdPNTempRows" value="<?php echo count(isset($aDataDT['raItems'])?$aDataDT['raItems']:[]); ?>">
        ...
        <?php if($aDataDT['rtCode'] == 1) { ?>
                        ...
                        <td nowrap class="<?=$tAlignFormat?>">
                            <?php if($DataVal->FTShwStaAlwEdit == 1 && in_array($tColumnName, ['FCXtdQty', 'FCXtdSetPrice', 'FTXtdDisChgTxt']) && (empty($tStaApv) && $tStaDoc != 3)) :?>
                                <?php if($tColumnName == 'FTXtdDisChgTxt' && $aDataTableVal['FTXtdStaAlwDis'] == '1'):?>
                                    <div class="xWPNDisChgDTForm">
                                        <button class="xCNBTNPrimeryDisChgPlus" onclick="JCNvPNDisChagDT(this)" type="button" style="margin-right: 5px;">+</button>
                                        <?php 
                                        ...
                                        ?>
                                        <label class="xWPNDisChgDT" id="olbPNDisChgDT<?=$aDataTableVal['rtRowID']?>"><?=$tTextDisChg?></label>
                                    </div>
                                <?php else:?>
                                    <label 
            ...
            <?php } ?>
        <?php }else { ?>
            <tr><td class="text-center xCNTextDetail2 xWPNTextNotfoundDataPdtTable" colspan="100%"><?= language('common/main/main','tCMNNotFoundData')?></td></tr>
        <?php } ?>
...
<?php if($aDataDT['rnAllPage'] > 1) : ?>
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="xWPagePNPdt btn-toolbar pull-right">
                <?php if($nPage == 1){ $tDisabledLeft = 'disabled'; }else{ $tDisabledLeft = '-';} ?>
                <button onclick="JSvPNPdtClickPage('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>> 
                ...
                <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataDT['rnAllPage'],$nPage+2)); $i++){?> 
                    ...
                    ?>
                    <button onclick="JSvPNPdtClickPage('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
                <?php } ?>
                <?php if($nPage >= $aDataDT['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
                <button onclick="JSvPNPdtClickPage('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>> 
...
<!-- Modal Delete Items -->
<div class="modal fade" id="odvModalDelPdtPN">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="osmConfirm" type="button" class="btn xCNBTNPrimery" onClick="JSoPNPdtDelChoose('<?= @$nCurrentPage ?>')"><?= language('common/main/main', 'tModalConfirm') ?></button>
...
<div class="modal fade" id="odvModalConfirmDeleteDTDis" style="z-index: 7000;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="obtPNConfirmDeleteDTDis" type="button" class="btn xCNBTNPrimery"><?= language('common/main/main', 'tModalConfirm') ?></button>
                <button id="obtPNCancelDeleteDTDis" type="button" class="btn xCNBTNDefult" data-dismiss="modal"><?= language('common/main/main', 'tCancel') ?></button>
            </div>
        </div>
    </div>
</div>
...
<?php  include("script/jPurchasereturnPdtAdvTableData.php");?>
