...
$('.xDocuemntRefInt').click(function(){
    ...
    JSxCLMCallRefIntDocDetailDataTable(tBchCode,tDocNo);
    $('.xDocuemntRefInt').removeClass('active');
    $(this).addClass('active');
})

// Function Check Data Search And Add In Tabel DT Temp
function JSvCLMRefIntClickPageList(ptPage){
    var nPageCurrent = '';
    switch (ptPage) {
...
// Function Check Data Search And Add In Tabel DT Temp
function JSxCLMCallRefIntDocDetailDataTable(ptBchCode,ptDocNo){
    JCNxOpenLoading();
    $.ajax({
        type    : "POST",
        url     : "docClaimRefIntDocDetailDataTable",
        data    : {
            'ptBchCode'     : ptBchCode,
            'ptDocNo'       : ptDocNo
        },
