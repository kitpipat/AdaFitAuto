            ...
            <table class="table table-striped">
                <thead>
                    <tr class="xCNCenter">
                        ...
                        <th nowrap class="xCNTextBold"><?php echo language('document/purchaseorder/purchaseorder', 'อ้างอิงเลขที่เอกสารภายนอก') ?></th>
                ...
                <tbody>
                    <?php if ($aDataList['rtCode'] == 1) : ?>
                        <?php foreach ($aDataList['raItems'] as $nKey => $aValue) : ?>
                            ...
                            $aDateDoc3 = explode(" ", $aDateDoc[2]);
                            $tDateDoc =   $aDateDoc[0] . '-' . $aDateDoc[1] . '-' . $aDateDoc3[0];

                            $tClassPrcStk = 'text-success';
                            $bIsApvOrCancel = ($aValue['FTXphStaApv'] == 1 || $aValue['FTXphStaApv'] == 2) || ($aValue['FTXphStaDoc'] == 3);

                            ?>
                            <tr style="cursor:pointer;" class="text-center xCNTextDetail2 xWPIDocItems xDocuemntRefInt" id="otrPurchaseInvoiceRefInt<?php echo $nKey ?>" data-docno="<?= $aValue['FTXphDocNo'] ?>" data-bchcode="<?= $tDOBchCode ?>" data-docdate="<?= $tDateDoc ?>">
                                ...
                                <td nowrap class="text-center"><?php echo (!empty($tDateDoc)) ? $tDateDoc : '-' ?></td>
...
<?php include('script/jInvoiceRefDocDataTable.php') ?>