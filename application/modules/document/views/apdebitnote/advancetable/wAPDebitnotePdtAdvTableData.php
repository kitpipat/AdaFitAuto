<div class="table-responsive">
    <table class="table table-striped xWPdtTableFont" id="otbAPDDOCPdtTable">
        <thead>
            <tr class="xCNCenter">
                ...
                <?php if(empty($tStaApv) && $tStaDoc != 3){?>
                    ...
                    <th class="xWAPDDeleteBtnEditButtonPdt"><?= language('document/purchaseorder/purchaseorder', 'tPOTBEdit')?></th>
                <?php }else { ?>
                    <th></th>
                <?php } ?>
            </tr>
        ...
        <input type="hidden" id="ohdAPDTempRows" value="<?php echo count(isset($aDataDT['raItems'])?$aDataDT['raItems']:[]); ?>">
        ...
        <?php if($aDataDT['rtCode'] == 1) { ?>
                        ...
                        <td nowrap class="<?=$tAlignFormat?>">
                            <?php if(!$DataVal->FTShwStaAlwEdit == 1 && in_array($tColumnName, ['FCXtdQty', 'FCXtdSetPrice', 'FTXtdDisChgTxt']) && (empty($tStaApv) && $tStaDoc != 3)) :?>
                                <?php if($tColumnName == 'FTXtdDisChgTxt' && $aDataTableVal['FTXtdStaAlwDis'] == '1'):?>
                                    <div class="xWAPDDisChgDTForm">
                                        <button class="xCNBTNPrimeryDisChgPlus" onclick="JCNvAPDDisChagDT(this)" type="button" style="margin-right: 5px;">+</button>
                                        <?php 
                                        ...
                                        ?>
                                        <label class="xWAPDDisChgDT" id="olbAPDDisChgDT<?=$aDataTableVal['rtRowID']?>"><?=$tTextDisChg?></label>
                                    </div>
                                <?php else:?>
                                    <label 
                ...
                <?php if(empty($tStaApv) && $tStaDoc != 3){?>
                    <td nowrap class="text-center">
                        <lable class="xCNTextLink">
                            <img class="xCNIconTable" src="<?php echo  base_url('application/modules/common/assets/images/icons/delete.png'); ?>" title="Remove" onclick="JSnAPDRemoveDTRow(this)">
                        </lable>
                    </td>    
                    <td></td>
                <?php }else { ?>
            ...
            <?php } ?>
        <?php }else { ?>
            <tr><td class="text-center xCNTextDetail2 xWAPDTextNotfoundDataPdtTable" colspan="100%"><?= language('common/main/main','tCMNNotFoundData')?></td></tr>
        <?php } ?>
...
<?php if($aDataDT['rnAllPage'] > 1) : ?>
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="xWPageAPDPdt btn-toolbar pull-right">
                <?php if($nPage == 1){ $tDisabledLeft = 'disabled'; }else{ $tDisabledLeft = '-';} ?>
                <button onclick="JSvAPDPdtClickPage('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>> 
                ...
                <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataDT['rnAllPage'],$nPage+2)); $i++){?> 
                    ...
                    ?>
                    <button onclick="JSvAPDPdtClickPage('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
                <?php } ?>
                <?php if($nPage >= $aDataDT['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
                <button onclick="JSvAPDPdtClickPage('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>> 
...
<!-- Modal Delete Items -->
<div class="modal fade" id="odvModalDelPdtAPD">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="osmConfirm" type="button" class="btn xCNBTNPrimery" onClick="JSoAPDPdtDelChoose('<?= @$nCurrentPage ?>')"><?= language('common/main/main', 'tModalConfirm') ?></button>
...
<div class="modal fade" id="odvModalConfirmDeleteDTDis" style="z-index: 7000;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="obtAPDConfirmDeleteDTDis" type="button" class="btn xCNBTNPrimery"><?= language('common/main/main', 'tModalConfirm') ?></button>
                <button id="obtAPDCancelDeleteDTDis" type="button" class="btn xCNBTNDefult" data-dismiss="modal"><?= language('common/main/main', 'tCancel') ?></button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    // Function : Check All Event Check Box
    // Creator  : 04/03/2022 Wasin
...
<?php  include("script/jAPDebitnotePdtAdvTableData.php");?>