<script>
    ...
    var nStaTWOBrowseType   = $('#oetTWOStaBrowse').val();
    var tCallTRNBackOption  = $('#oetTWOCallBackOption').val();
    ...
    $('document').ready(function() {
        ...
        switch(nStaTWOBrowseType){
            case '2':
                var tAgnCode = $('#oetTWOJumpAgnCode').val();
                var tBchCode = $('#oetTWOJumpBchCode').val();
                var tDocNo = $('#oetTWOJumpDocNo').val();
                JSvTWOCallPageEdit(tDocNo);
            break;
            default:
            JSvTWOCallPageTransferwarehouseout();
        }
    ...
    function JSxTWOTransferwarehouseoutDocCancel(pbIsConfirm) {
        ...
        if (pbIsConfirm) {
            $.ajax({
                ...
                data    : {
                    'tTWODocNo'   : $('#oetTWODocNo').val(),
                    'tTWODocType' : $('#oetTWODocType').val(),
                    'tTWOBchCode' : $('#oetSOFrmBchCode').val(),
                },
                cache   : false,
                timeout : 5000,
                success : function (tResult) {
        ...
        if(typeof nStaSession !== "undefined" && nStaSession == 1){
            try{
                if(pbIsConfirm) {
                    ...
                    $.ajax({
                        type    : "POST",
                        url     : "TWOTransferwarehouseoutCheckProductWahouse",
                        data    : {
                            'tDocNo'        : tXthDocNo,
