<script type="text/javascript">
    ...
    var tTWOCheckIn         = '<?= $tTWOCheckIn; ?>'; // ค่าที่ได้จากการเชคเอกสารอ้่างอิงภายใน  แปลงเป็นตัวแปลของ javascript
    ...
    var tRefInType          = $('#ohdTWORefInType').val();
    var tAutStaCancel       = $('#ohdTWOAutStaCancel').val();
    var tDocDateCreate      = $('#ohdTWODocDateCreate').val();
    var tDocDateNowToday    = $('#ohdTWODateNowToday').val();
    ...
    $(document).ready(function() {
        ...
        if (tTWORsnType == 3) {
            ...
            tTwoRsnTypeCode = 'ETC';
        } else if (tTWORsnType == 5) {
            tTwoRsnTypeCode = 'CUS';
        }
        $("#obtTWOSubmitFromDoc").removeAttr("disabled");
        $('#ocmSelectTransTypeIN').val(tTwoRsnTypeCode).change();

        // เอกสารถูกยกเลิก
        if(tTWOStaDoc == 3) {
            $('#otaTWOFrmInfoOthRmk').removeAttr('disabled', true);
            ...
            $('.xCNControllDisabled').attr('disabled', true);
            $('.xWDropdown').hide();
        ...
        } else if (tTWOStaDoc == 1 && tTWOStaApvDoc == 1 && tRefInType != 2) {
    ...
    var oBrowseTROutFromWah_BCH = function(paDataparamiter) {
        ...
        var oOptionReturn = {
            ...
            Where: {
                Condition: ["AND TCNMWaHouse.FTWahRefCode = '" + tBchCode + "' AND TCNMWaHouse.FTWahCode !='" + tWahCodeTo + "'"]
            },
            GrideView: {
                ColumnPathLang: 'company/shop/shop',
                ColumnKeyLang: ['tWahCode', 'tWahName'],
    ...
    $('#obtBrowseTROutFromWah').click(function() {
        ...
        oBrowseTROutFromWah_BCH_Option.Where.Condition = [" AND TCNMWaHouse.FTBchCode = '" + tBCH + "' " + tFindWahouse + " "];
    ...
    //เลือกคลังสินค้าปลายทาง (แบบมี shop)
    ...
    //เลือกคลังสินค้าปลายทาง (แบบไม่มี shop)
    var oBrowseTROutToWah_BCH = function(paParam) {
        ...
        var oOptionReturn = {
            ...
            Where: {
                Condition: [" AND TCNMWaHouse.FTWahRefCode = '" + tBchCode + "' AND TCNMWaHouse.FTWahCode != '" + tWahCodeFrom + "' "]
            },
            GrideView: {
                ColumnPathLang: 'company/shop/shop',
                ColumnKeyLang: ['tWahCode', 'tWahName'],
    ...
    $('#obtBrowseTROutToWah').click(function() {
        if ($('#oetTROutShpToCode').val() != '') {
            ...
            oBrowseTROutToWah_BCH_Option.Where.Condition = [" AND TCNMWaHouse.FTBchCode = '" + tBCH + "' " + tFindWahouse + " "];
            ...
            if (tTypeDocument == 'IN') { //เอกสารรับโอน
                ...
                if (tIN == 0) {
                    ...
                    if (tTypeDocumentIN == 'SPL') {
                        ...
                        if ($('#oetTranferOutCusCode').val() == '') {
                            $('#odvWTIModalWahIsEmpty').modal('show');
                            $('#ospWahIsEmpty').html('<?= language('document/transferwarehouseout/transferwarehouseout', 'กรุณาเลือกลูกค้าปลายทางให้ถูกต้อง') ?>');
                            return;
                        }
                    }
                }
            ...
            if ($('#ohdTWOStaDoc').val() != 3) {
                if (tTypeDocument == 0) {
                    ...
                    if (tTypeDocument == 4) { //เอกสารรับโอน
                        ...
                        if (tIN == '') {
                            $('#odvWTIModalTypeIsEmpty').modal('show');
                            $('#ospTypeIsEmpty').html('<?= language('document/transferwarehouseout/transferwarehouseout', 'tINDocumentISEmptyDetail2') ?>');
                            return;
                        } else {
    ...
    function JSxTransferwarehouseoutEventAddEdit(ptRoute) {
        $('#ofmTransferwarehouseoutFormAdd').validate({
            ...
            submitHandler: function(form) {
                ...
                //วันที่ + เวลา
                $('#oetTWODocDate').removeAttr('disabled', true);
                $('#oetTWODocTime').removeAttr('disabled', true);
                $('.xCNControllDisabled').removeAttr('disabled', true);
                $('#obtTWOFrmBrowseShipAdd').removeAttr('disabled', true);
                ...
                $("#obtTWOSubmitFromDoc").attr('disabled', 'true');
    ...
    // เบาวน์  Customer
    $('#obtBrowseTranferOutCus').click(function() {
        JCNxBrowseData('oBrowseTRINFromCus')
    });
    var oBrowseTRINFromCus = {
        ...
        CallBack: {
            ReturnType: 'S',
            Value: ["oetTranferOutCusCode", "TCNMCst.FTCstCode"],
            Text: ["oetTranferOutCusName", "TCNMCst_L.FTCstName"],
        }
    }
</script>