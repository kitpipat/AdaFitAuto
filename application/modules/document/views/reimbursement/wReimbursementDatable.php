            ...
            <table id="" class="table table-striped">
                <thead>
                    <tr class="xCNCenter">
                        ...
                        <?php endif; ?>
                        <th nowrap class="xCNTextBold" style="width:5%;"><?php echo language('document/deliveryorder/deliveryorder','tDOBchName')?></th>
                        ...
                        <th nowrap class="xCNTextBold" style="width:10%;"><?php echo language('document/reimbursement/reimbursement','tSALDocRefIntNo')?></th>
                        <th nowrap class="xCNTextBold" style="width:10%;"><?php echo language('document/reimbursement/reimbursement','tSALDocRefIntDate')?></th>
                ...
                <tbody>
                    <?php if($aDataList['rtCode'] == 1 ):?>
                        <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                            <?php
                                $tSALAgnCode  = $aValue['FTAgnCode'];
                                $tSALBchCode  = $aValue['FTBchCode'];
                                $tSALDocCode  = $aValue['FTXshDocNo'];
                                $tSALDocRefCode  = $aValue['FTXshRefDocNo'];
                                ...
                                if(!empty($aValue['FTXshStaApv']) || $aValue['FTXshStaDoc'] == 3){
                                    ...
                                    $tClassDisabled = '';
                                    $tTitle = '';
                                    $tOnclick = "onclick=JSoSALDelDocSingle('".$nCurrentPage."','".$tSALDocCode."','".$tSALAgnCode."','".$tSALBchCode."','".$tSALDocRefCode."')";
                                }
                            ...
                            ?>
                            <tr class="text-center xCNTextDetail2 xWPIDocItems" id="otrPurchaseInvoice<?php echo $nKey?>" data-code="<?php echo $tSALDocCode?>" data-name="<?php echo $tSALDocCode?>">
                                <?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
                                    <td nowrap class="text-center">
                                        <label class="fancy-checkbox ">
                                            <input id="ocbListItem<?php echo $nKey?>" type="checkbox" class="ocbListItem" name="ocbListItem[]" value="<?=$tSALDocCode?>"
                                                    data-code="<?php echo $tSALDocCode?>"
                                                    data-name="<?php echo $tSALDocCode?>"
                                                    data-agn="<?php echo $tSALAgnCode?>"
                                                    data-bch="<?php echo $tSALBchCode?>"
                                                    data-docref="<?php echo $tSALDocRefCode?>"
                                                    disabled
                                            >
                                ...
                                <?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaRead'] == 1) : ?>
                                    <td nowrap>
                                        <img class="xCNIconTable" style="width: 17px;" src="<?= base_url('application/modules/common/assets/images/icons/view2.png'); ?>" onClick="JSvSALCallPageEdit('<?= $aValue['FTAgnCode'] ?>','<?= $aValue['FTBchCode'] ?>','<?= $aValue['FTXshDocNo'] ?>','<?= $aValue['FTCstCode'] ?>')">
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach;?>
...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
        <div class="xWPIPageDataTable btn-toolbar pull-right">
            ...
            <button onclick="JSvSALClickPageList('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>>
            ...
            <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataList['rnAllPage'],$nPage+2)); $i++){?>
                ...
                ?>
                <button onclick="JSvSALClickPageList('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
            <?php } ?>

            <?php if($nPage >= $aDataList['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
            <button onclick="JSvSALClickPageList('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>>
...
<!-- ===================================================== Modal Delete Document Single ===================================================== -->
    <div id="odvSALModalDelDocSingle" class="modal fade" tabindex="-1" role="dialog">
...
<!-- ===================================================== Modal Delete Document Multiple =================================================== -->
    <div id="odvSALModalDelDocMultiple" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header xCNModalHead">
                    <label class="xCNTextModalHeard"><?php echo language('common/main/main','tModalDelete')?></label>
...
<!-- ======================================================================================================================================== -->
<?php include('script/jReimbursementDataTable.php')?>
