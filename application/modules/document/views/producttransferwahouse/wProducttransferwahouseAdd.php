<?php
	...
	if ($aResult['rtCode'] == "1") {
		...
		// Status Ref Key Type
        $tRefInType = $aResult['raItems']['FTXthRefInType'];
	} else {
		$tRoute 			= "TFWEventAdd";
		$tXthDocNo 			= "";
		$dXthDocDate 		= "";
...
<form class="contact100-form validate-form" action="javascript:void(0)" method="post" enctype="multipart/form-data" autocorrect="off" autocapitalize="off" autocomplete="off" id="ofmAddTFW">
	...
	<input type="hidden" id="ohdTFWRefInType"       name="ohdTFWRefInType"      value="<?=@$tRefInType;?>">
    <input type="hidden" id="ohdTFWAutStaCancel"    name="ohdTFWAutStaCancel"   value="<?=@$aAlwEvent['tAutStaCancel'];?>">
    <input type="hidden" id="ohdTFWDocDateCreate"   name="ohdTFWDocDateCreate"  value="<?=date("m", strtotime(@$dXthDocDate))?>">
    <input type="hidden" id="ohdTFWDateNowToday"    name="ohdTFWDateNowToday"   value="<?=date('m');?>">
	<div class="row">
		<div class="col-md-4">
