                ...
                <tbody>
                    <?php if ($aDataList['rtCode'] == 1) : ?>
                        <?php foreach ($aDataList['raItems'] as $nKey => $aValue) : ?>
                            <?php
                            $tPreSvAgnCode  = $aValue['FTAgnCode'];
                            $tPreSvBchCode  = $aValue['FTBchCode'];
                            $tPreSvDocCode  = $aValue['FTXshDocNo'];
                            if (!empty($aValue['FTXshStaApv']) || $aValue['FTXshStaDoc'] == 3) {
                                ...
                                $tClassDisabled = '';
                                $tTitle = '';
                                $tOnclick = "onclick=JSoPreDelDocSingle('" . $nCurrentPage . "','" . $tPreSvDocCode . "')";
                            }
                            ...
                            ?>
                            <tr class="text-center xCNTextDetail2" data-code="<?php echo $tPreSvDocCode ?>" data-name="<?php echo $tPreSvDocCode ?>">
                                <?php if ($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
                                    <td nowrap class="text-center">
                                        <label class="fancy-checkbox ">
                                            <input id="ocbListItem<?php echo $nKey ?>" type="checkbox" class="ocbListItem" name="ocbListItem[]" value="<?= $tPreSvDocCode ?>" data-code="<?php echo $tPreSvDocCode ?>" data-name="<?php echo $tPreSvDocCode ?>" <?php echo $tCheckboxDisabled; ?>>
                                ...
                                <td nowrap>
                                    <?php if ($bIsApvOrCancel) { ?>
                                        <img class="xCNIconTable" style="width: 17px;" src="<?= base_url('application/modules/common/assets/images/icons/view2.png'); ?>" onClick="JSvPreSvCallPageEdit('<?= $aValue['FTAgnCode'] ?>','<?= $aValue['FTBchCode'] ?>','<?= $aValue['FTXshDocNo'] ?>')">
                                    <?php } else { ?>
                                        <img class="xCNIconTable xCNIconEdit" onClick="JSvPreSvCallPageEdit('<?= $aValue['FTAgnCode'] ?>','<?= $aValue['FTBchCode'] ?>','<?= $aValue['FTXshDocNo'] ?>')">
                                    <?php } ?>
                                </td>
                                <?php //endif; 
                                ?>
...
<div class="row">
    ...
    <!-- <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
        <div class="xWPIPageDataTable btn-toolbar pull-right">
            ...
            <button onclick="JSvPreClickPageList('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>>
            ...
            <?php for ($i = max($nPage - 2, 1); $i <= max(0, min($aDataList['rnAllPage'], $nPage + 2)); $i++) { ?>
                ...
                ?>
                <button onclick="JSvPreClickPageList('<?php echo $i ?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i ?></button>
            <?php } ?>
            ...
            <button onclick="JSvPreClickPageList('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>>
...
<div id="odvPreModalDelDocSingle" class="modal fade" tabindex="-1" role="dialog">
...
<div id="odvPreModalDelDocMultiple" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header xCNModalHead">
                <label class="xCNTextModalHeard"><?php echo language('common/main/main', 'tModalDelete') ?></label>
...
<?php include('script/jPreRepairResultDataTable.php') ?>