...
<div class="row">
    ...
    <div class="table-responsive" style="min-height: 300px; max-height: 300px; overflow-y: scroll;">
            <table id="otbPXDisChgDataDocDTList" class="table">
                <thead>
                    <tr class="xCNCenter">
			            <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','ลำดับ')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','ก่อนลด')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','มูลค่า ลด/ชาร์จ')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','หลังลด')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','ประเภท')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','ส่วนลด/ชาร์จ')?></th>
                        <th class="xCNTextBold"><?php echo language('document/expenserecord/expenserecord','ลบ')?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if($aDataList['rtCode'] == 1 ):?>
                        ...
                        <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                            <tr class="xWPXDisChgTrTag">
                                ...
                                <td nowrap class="text-right"><label class="xWPXDisChgValue"><?php echo $aValue['FCXtdValue']; ?></label></td>
                                <td nowrap class="text-right"><label class="xWPXDisChgAfterDisChg"><?php echo $aValue['FCXtdNet']; ?></label></td>
                    ...
                    <?php else: ?>
                        <?php $tHideClass = ''; ?>
                    <?php endif;?>
                        <tr id="otrPXDisChgDTNotFound" class="<?php echo $tHideClass;?>"><td class='text-center xCNTextDetail2' colspan='100%'><?php echo language('common/main/main','tCMNNotFoundData')?></td></tr>
                </tbody>
            </table>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <div class="xWPage btn-toolbar pull-right">
                ...
                <button onclick="JSvPXDisChgDTClickPage('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>>
                ...
                <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataList['rnAllPage'],$nPage+2)); $i++){?>
                    ...
                    ?>
                    <button onclick="JSvPXDisChgDTClickPage('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
                <?php } ?>

                <?php if($nPage >= $aDataList['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
                <button onclick="JSvPXDisChgDTClickPage('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>>
