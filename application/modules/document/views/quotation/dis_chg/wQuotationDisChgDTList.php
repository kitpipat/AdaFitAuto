...
<div class="row">
    ...
    <div class="table-responsive" style="min-height: 300px; max-height: 300px; overflow-y: scroll;">
            <table id="otbQTDisChgDataDocDTList" class="table">
                <thead>
                    <tr class="xCNCenter">
                ...
                <tbody>
                    <?php if($aDataList['rtCode'] == 1 ):?>
                        ...
                        <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                            <tr class="xWQTDisChgTrTag">
                                ...
                                <td nowrap class="text-right"><label class="xWQTDisChgValue"><?=$aValue['FCXtdValue']; ?></label></td>
                                <td nowrap class="text-right"><label class="xWQTDisChgAfterDisChg"><?=$aValue['FCXtdNet']; ?></label></td>
                    ...
                    <?php else: ?>
                        <?php $tHideClass = ''; ?>
                    <?php endif;?>
                        <tr id="otrQTDisChgDTNotFound" class="<?=$tHideClass;?>"><td class='text-center xCNTextDetail2' colspan='100%'><?=language('common/main/main','tCMNNotFoundData')?></td></tr>
                </tbody>
            </table>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <div class="xWPage btn-toolbar pull-right">
                ...
                <button onclick="JSvQTDisChgDTClickPage('previous')" class="btn btn-white btn-sm" <?=$tDisabledLeft ?>>
                ...
                <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataList['rnAllPage'],$nPage+2)); $i++){?>
                    ...
                    ?>
                    <button onclick="JSvQTDisChgDTClickPage('<?=$i?>')" type="button" class="btn xCNBTNNumPagenation <?=$tActive ?>" <?=$tDisPageNumber ?>><?=$i?></button>
                <?php } ?>

                <?php if($nPage >= $aDataList['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
                <button onclick="JSvQTDisChgDTClickPage('next')" class="btn btn-white btn-sm" <?=$tDisabledRight ?>>
