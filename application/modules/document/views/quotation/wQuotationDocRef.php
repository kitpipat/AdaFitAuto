...
<script>
    ...
    $('.xWDelDocRef').off('click').on('click',function(){
        ...
        $.ajax({
            type    : "POST",
            url     : "docQuotationEventDelHDDocRef",
            data:{
    ...
    $('.xWEditDocRef').off('click').on('click',function(){
        ...
        $('#ocbQTRefType').val(tRefType);
        $('#ocbQTRefType').selectpicker('refresh');
        $('#oetQTRefDocDate').datepicker({ dateFormat: 'yy-mm-dd' }).val(tRefDocDate);

        if(tRefType == 1){//ภายใน
            $('#oetQTDocRefIntName').val(tRefDocNo);
            $('#oetQTDocRefInt').val(tRefDocNo);
        }else{ //ภายนอก
            $('#oetQTRefDocNo').val(tRefDocNo);
        }

        $('#oetQTRefKey').val(tRefKey);
        $('#oetQTRefDocNoOld').val(tRefDocNo);
